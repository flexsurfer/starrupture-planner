...
export function buildBaseProductionTable(
  ...
  const recipeDemands = new Map<
    string,
    { card: BaseProductionRecipeCard; required: number; rate: number }[]
  >();
  for (const building of base.buildings) {
    ...
    if (
      ...
      (definition && !isBuildingAvailableForSection(definition, "production"))
    ) {
      capacityOwned.set(
        building.buildingTypeId,
        (capacityOwned.get(building.buildingTypeId) ?? 0) + 1,
      );
    }
  }
  const getItem = (id: string) =>
    itemsById[id] ?? { id, name: id, type: "unknown" };
  const demand = (
    plan: PlanSummaryRow,
    amount: number,
  ): CoveragePlanDemand => ({
    ...
    if (
      !flow.nodes.some(
        (node) =>
          ...
          node.outputItem === plan.selectedItemId,
      )
    ) {
      unavailable.push({
        kind: "unavailable",
        id: `unavailable:${plan.id}`,
        item: getItem(plan.selectedItemId),
        rate: plan.targetAmount,
        itemCoverage: {
          required: plan.targetAmount,
          covered: 0,
          planDemands: [{ ...demand(summary, plan.targetAmount), covered: 0 }],
        },
      });
    }
    for (const node of flow.nodes) {
      ...
      if (existing) {
        ...
        recipes.set(id, {
          kind: "recipe",
          id,
          item: getItem(node.outputItem),
          node,
          rate,
          targetRate,
          requiredBuildings: required,
          coverage: null,
      ...
      if (!building || !isBuildingAvailableForSection(building, "production"))
        continue;
  ...
  for (const row of coverage.values()) {
    ...
    row.planDemands = plans
      .filter((plan) => row.perPlan[plan.id] > 0)
      .map((plan) => demand(plan, row.perPlan[plan.id]));
  }
  for (const card of recipes.values())
    card.coverage = coverage.get(card.node.buildingId) ?? null;
  ...
  const allocationOrder = [...base.productions].sort(
  ...
  const coverInput = (balance: MaterialBalanceRow): ProductionItemCoverage => {
    ...
    const planDemands = allocationOrder.flatMap((plan) => {
      const entry = balance.planDemands.find(
    ...
    return {
      required: balance.totalRequired,
  ...
  const cards: BaseProductionTableCard[] = [
    ...recipes.values(),
    ...unavailable,
    ...materials.map((balance) => ({
      kind: "input" as const,
      id: `input:${balance.itemId}`,
      item: balance.item,
      balance,
      itemCoverage: coverInput(balance),
    })),
  ];
  ...
  for (const card of cards) {
    const type =
      ...
      (card.kind === "recipe" && card.targetRate > 0)
        ? "target"
        : card.kind === "recipe" && card.node.nodeType === "launcher"
          ? "launcher"
          : card.item.type || "unknown";
  ...
  const rank = (type: string) =>
    order.includes(type) ? order.indexOf(type) : order.indexOf("unknown");
  return {
    groups: [...grouped]
      ...
      .map(([type, entries]) => ({
        ...
        cards: entries.sort(
          (a, b) =>
            a.item.name.localeCompare(b.item.name) || a.id.localeCompare(b.id),
        ),
      })),
    requiredBuildings: [...coverage.values()].reduce(
      (sum, row) => sum + row.totalRequired,
      0,
    ),
    missingBuildings: [...coverage.values()].reduce(
