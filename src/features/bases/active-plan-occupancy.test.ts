...
function createPlan(
  partial: Partial<Production> & { id: string; name: string },
): Production {
  return {
    id: partial.id,
    name: partial.name,
...
describe("buildActivePlanOccupancy", () => {
  it("assigns free buildings first and overlaps only when required", () => {
    ...
    expect(planABuildings ? Array.from(planABuildings) : []).toEqual([
      "s1",
      "s2",
    ]);
    expect(planBBuildings ? Array.from(planBBuildings) : []).toEqual([
      "s3",
      "s1",
    ]);
  ...
  it("prioritizes buildings with selected items when allocating requirements", () => {
    ...
    const occupancy = buildActivePlanOccupancy(base);
    expect(
      Array.from(occupancy.assignedPlanBuildingIds.get("plan_1") || []),
    ).toEqual(["b_with_item"]);
  });

  it("does not reserve explicit input buildings for occupancy counts", () => {
    const input1 = createBaseBuilding(
      "input_1",
      "ore_excavator",
      "inputs",
      "ore_iron",
    );
    const input2 = createBaseBuilding(
      "input_2",
      "ore_excavator",
      "inputs",
      "ore_copper",
    ...
    const occupancy = buildActivePlanOccupancy(base);
    expect(
      Array.from(occupancy.assignedPlanBuildingIds.get("plan_inputs") || []),
    ).toEqual([]);
    expect(
      Array.from(occupancy.assignedPlanBuildingIds.get("plan_required") || []),
    ).toEqual(["input_1", "input_2"]);
  });
  ...
  it("overlaps production assignments when free buildings are exhausted", () => {
    ...
    const occupancy = buildActivePlanOccupancy(base);
    expect(
      Array.from(occupancy.assignedPlanBuildingIds.get("plan_a") || []),
    ).toEqual(["s1"]);
    expect(
      Array.from(occupancy.assignedPlanBuildingIds.get("plan_b") || []),
    ).toEqual(["s1"]);
  ...
  it("can exclude the current plan from occupancy calculations", () => {
    ...
    const occupancy = buildActivePlanOccupancy(base, {
      excludePlanId: "plan_b",
    });
    expect(Array.from(occupancy.assignedPlanBuildingIds.keys())).toEqual([
      "plan_a",
    ]);
    expect(occupancy.occupiedBuildingTypeCounts.get("smelter")).toBe(1);
  });
