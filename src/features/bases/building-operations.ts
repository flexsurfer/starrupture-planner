...
export function createBaseBuilding({
  ...
  return {
    ...
    ...(requestedRatePerMinute && requestedRatePerMinute > 0
      ? { requestedRatePerMinute }
      : {}),
    ...(capacityPerMinute && capacityPerMinute > 0
      ? { capacityPerMinute }
      : {}),
...
export function getLinkedInputBuildingTypeId(
  buildings: Building[],
): string | undefined {
  const packageReceiver = buildings.find(
    (building) => building.id === PACKAGE_RECEIVER_BUILDING_ID,
  );
  if (packageReceiver) return packageReceiver.id;

  const fallback = buildings.find(
    (building) =>
      isBuildingAvailableForSection(building, "inputs") &&
      !isRawExtractor(building),
  );
  return fallback?.id;
}

export function getOutputBuilding(
  base: Base,
  outputBuildingId: string,
): BaseBuilding | undefined {
  return base.buildings.find(
    (building) =>
...
export function takeOverPlanningEndpoint(
  _base: Base,
...
export function unlinkInputsLinkedToOutput(
  ...
  draftState.basesList.forEach((base) => {
    base.buildings.forEach((building) => {
      ...
      if (
        exceptInputRef?.baseId === base.id &&
        exceptInputRef.buildingId === building.id
      )
        return;
...
export function linkInputToOutput(
  ...
  const inputBuilding = inputBase.buildings.find(
    (building) => building.id === inputRef.buildingId,
  );
  ...
  const snapshotItemId =
    resolvedOutput.selectedItemId || inputBuilding.selectedItemId;
  const snapshotRatePerMinute =
    resolvedOutput.ratePerMinute && resolvedOutput.ratePerMinute > 0
      ? resolvedOutput.ratePerMinute
