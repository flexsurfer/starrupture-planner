...
export const registerBasesEvents: UkladModule<UkladRegistrar<AppContracts>> = (
  registrar,
) => {
  ...
  registrar.regEvent(
    appIds.events.BASES_SET_DETAILS_EXPANDED,
    ({ draftState }, expanded) => {
  ...
  registrar.regEvent(
    appIds.events.BASES_CREATE_BASE,
    ({ draftState }, name) => {
  ...
  registrar.regEvent(
    appIds.events.BASES_SET_CORE_LEVEL,
    ({ draftState }, level) => {
  ...
  registrar.regEvent(
    appIds.events.BASES_DELETE_BASE,
    ({ draftState }, baseId) => {
      draftState.basesList = draftState.basesList.filter(
        (base) => base.id !== baseId,
      );
  ...
  registrar.regEvent(
    appIds.events.BASES_OPEN_BASE,
    ({ draftState }, baseId, tab = "base") => {
      draftState.basesSelectedBaseId = baseId;
      draftState.basesSelectedDetailTab = tab;
      draftState.baseLayoutTransferMode =
        getBaseById(draftState.basesList, baseId)?.layout?.transferMode ??
        "virtual";
    },
  );

  registrar.regEvent(
    appIds.events.BASES_SET_SELECTED_BASE,
    ({ draftState }, baseId) => {
      draftState.basesSelectedBaseId = baseId;
      draftState.basesSelectedDetailTab = "base";
      draftState.baseLayoutTransferMode =
  ...
  registrar.regEvent(
    ...
    ({ draftState }, baseId, groupId) => {
      ...
      const groupExists = draftState.energyGroups.some(
  ...
  registrar.regEvent(
    ...
    (
      ...
      const base = getBaseById(draftState.basesList, baseId);
      if (!base) return;
      const building = draftState.buildingsList.find(
        (candidate) => candidate.id === buildingTypeId,
      );

      const normalizedCount =
        building && isBuildingCountAvailable(building)
          ? sanitizeBulkBuildingCount(count)
          : 1;
      const normalizedRatePerMinute =
        typeof ratePerMinute === "number" && ratePerMinute > 0
          ? ratePerMinute
          : undefined;
      const normalizedLinkedOutput = linkedOutput || undefined;
      const sourcePlan =
        sectionType === "outputs" && sourceProductionId
      ...
      const normalizedAllocationMode = normalizedSourceProductionId
        ? allocationMode === "fixed"
          ...
          : "auto"
        : undefined;
      const normalizedRequestedRatePerMinute =
        normalizedAllocationMode === "fixed" &&
      ...
      const normalizedPriority = normalizedSourceProductionId
        ...
        : undefined;
      const normalizedLinkedInputRef =
        sectionType === "outputs" &&
        linkedInputRef?.baseId &&
        linkedInputRef?.buildingId
          ? linkedInputRef
          : null;
      ...
      for (let index = 0; index < normalizedCount; index += 1) {
        const newBuilding = createBaseBuilding({
          ...
          description,
          selectedItemId:
            sourcePlan?.selectedItemId || selectedItemId || undefined,
          ratePerMinute: normalizedSourceProductionId
            ? undefined
            : normalizedRatePerMinute,
          linkedOutput: normalizedSourceProductionId
            ? undefined
            : normalizedLinkedOutput,
          ...
          priority:
            typeof normalizedPriority === "number"
              ? normalizedPriority + index
              : undefined,
        });
        ...
        if (normalizedLinkedInputRef) {
          ...
          const targetBase = getBaseById(
            draftState.basesList,
            normalizedLinkedInputRef.baseId,
          );
          const targetInput = targetBase?.buildings.find(
            (candidate) => candidate.id === normalizedLinkedInputRef.buildingId,
          );
          if (targetBase && targetInput)
            takeOverPlanningEndpoint(targetBase, targetInput);
        }
      ...
      selectAddedProductionPlanInputs(
        draftState as AppState,
        baseId,
        addedInputIds,
      );
  ...
  registrar.regEvent(
    ...
    ({ draftState }, baseId, buildingTypeId, sectionType, rawTargetCount) => {
      ...
      const nextBuildings = reconcileBaseBuildingSectionTypeCount({
        ...
        targetCount,
        createId: () =>
          `building_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
      });
  ...
  registrar.regEvent(
    ...
    ({ draftState }, buildingId) => {
      ...
      const base = getBaseById(draftState.basesList, baseId);
      if (!base) return;
      const removed = base.buildings.find(
      ...
      if (removed?.sectionType === "inputs" && removed.planningOwnerPlanId) {
        ...
        draftState.productionPlanModalState.selectedInputIds =
          draftState.productionPlanModalState.selectedInputIds.filter(
            (id) => id !== buildingId,
          );
      }
    },
  ...
  registrar.regEvent(
    ...
    (
      ...
      const resolvedSourceOutput = resolveOutputBuilding(
        sourceOutput,
        sourceBase,
      );
      unlinkInputsLinkedToOutput(
  ...
  registrar.regEvent(
    ...
    ({ draftState }, baseId, buildingId, payload) => {
      ...
      const output = base.buildings.find(
        (building) => building.id === buildingId,
      );
      ...
      const sourcePlan = base.productions.find(
        (plan) => plan.id === sourceProductionId,
      );
      if (!sourcePlan) return;
      takeOverPlanningEndpoint(base, output);

      output.sourceProductionId = sourceProductionId;
      output.allocationMode =
        payload.allocationMode === "fixed" ? "fixed" : "auto";
      ...
      if (
        ...
        output.capacityPerMinute = getDefaultOutputCapacityPerMinute(
      ...
      if (
        ...
        output.priority = base.buildings.filter(
          (building) =>
