...
const MyBasesPage = () => {
  ...
  const handleDeleteBase = useCallback((baseId: string) => {
    ...
    if (base) {
      runtime.dispatch([appIds.events.UI_SHOW_CONFIRMATION_DIALOG,
        t("Delete Base"),
      t("Are you sure you want to delete {name}? This action cannot be undone.", { name: base.name }),
      () => {
  ...
  return (
    <div className={`h-full min-h-0 px-2 lg:px-3 flex flex-col ${visibleView === 'bases' ? 'pb-2 lg:pb-3' : ''}`}>
      <NavigationHeader title={t("My Bases")} actions={<MyBasesSettings />} summary={<>
        <MyBasesStats />
        {advanced && <button
          type="button"
        ...
        >
          ...
          </svg>{t("Grids")}{energyGroups.length > 0 && (
      ...
      <div className="mb-2 flex shrink-0 items-center gap-1">
        <div
          ...
          aria-label={t("My Bases sections")}
        >
          <button
            type="button"
            role="tab"
        ...
        >
          ...
          <span className="hidden sm:inline">{t("Create Base")}</span>
        </button>
      </div>
