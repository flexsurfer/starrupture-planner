...
const MyBasesPage = () => {
  ...
  const handleOpenBase = useCallback(
    (baseId: string, tab: BaseDetailTab = "base") => {
  ...
  return (
    <div
      className={`h-full min-h-0 px-2 lg:px-3 flex flex-col ${visibleView === "bases" ? "pb-2 lg:pb-3" : ""}`}
    >
      <NavigationHeader
        title="My Bases"
      ...
      {/* Content */}
      <div
        className={`flex-1 min-h-0 ${visibleView === "logistics" ? "overflow-hidden" : "overflow-auto"}`}
      >
        {visibleView === "bases" && (
          <div
