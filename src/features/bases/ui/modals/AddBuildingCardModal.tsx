...
} from "@/features/bases/building-counts";
import {
...
function getBuildingGroupId(
  ...
  if (sectionType === "inputs") {
    ...
    if (
      ...
      TRANSPORT_BUILDING_IDS.has(building.id)
    )
      return "transport";
    return "other";
  }
...
function groupBuildingsForSection(
  buildings: Building[],
  sectionType: BuildingSectionType,
): BuildingGroup[] {
...
export const AddBuildingCardModal: React.FC<AddBuildingCardModalProps> = ({
  ...
  const [selectedLinkedOutput, setSelectedLinkedOutput] =
    useState<LinkableOutputItem | null>(null);
  ...
  const [configurationMode, setConfigurationMode] =
    useState<ConfigurationMode>("manual");
  ...
  const supportsItemConfiguration =
    sectionType === "inputs" || sectionType === "outputs";
  const mustConfigureItem =
    supportsItemConfiguration && requireItemConfiguration;
  const selectedItem = selectedItemId
    ? itemsById[selectedItemId] || null
    : null;
  const selectedPlan = selectedPlanId
    ? plans.find((plan) => plan.id === selectedPlanId) || null
    : null;
  const supportsCount =
    sectionType === "production" || sectionType === "energy";
  const selectedBuildingSupportsCount = !!selectedBuilding && supportsCount;
  const selectedBuildingSupportsLinking =
    !!selectedBuilding &&
    sectionType === "inputs" &&
    !isRawExtractor(selectedBuilding);
  const selectedBuildingSupportsPlanLinking =
    !!selectedBuilding && sectionType === "outputs" && plans.length > 0;
  const selectedBuildingSupportsInputTargets =
    !!selectedBuilding &&
    sectionType === "outputs" &&
  ...
  const linkableInputTargets = useMemo<LinkableInputTarget[]>(() => {
    ...
    for (const base of allBases) {
      for (const input of base.buildings) {
        ...
        const itemId =
          input.selectedItemId || input.linkedOutput?.itemIdSnapshot;
        const item = itemId
          ? itemsById[itemId] || { id: itemId, name: itemId, type: "unknown" }
          : undefined;
        const linkedBase = input.linkedOutput
          ? allBases.find(
              (candidate) => candidate.id === input.linkedOutput?.baseId,
            )
          : undefined;
        const linkedOutput =
          linkedBase && input.linkedOutput
            ? linkedBase.buildings.find(
                (candidate) => candidate.id === input.linkedOutput?.buildingId,
              )
            : undefined;
        ...
        targets.push({
          ...
          item,
          ratePerMinute:
            input.ratePerMinute || input.linkedOutput?.ratePerMinuteSnapshot,
          linkedOutputLabel,
        });
      }
    }
    ...
    return targets.sort((left, right) => {
      const currentBaseDelta =
        Number(right.baseId === selectedBase?.id) -
        Number(left.baseId === selectedBase?.id);
  ...
  const selectedLinkedInputTarget = selectedLinkedInputKey
    ? linkableInputTargets.find(
        (target) => target.key === selectedLinkedInputKey,
      ) || null
    : null;
  ...
  const getDefaultPriorityForPlan = (planId: string): number => {
    ...
    return selectedBase.buildings.filter(
      (building) =>
  ...
  const getDefaultCapacityForSelectedBuilding = (): string => {
    ...
    const defaultCapacity = getDefaultOutputCapacityPerMinute(
      selectedBuilding.id,
    );
  ...
  const handlePlanModeClick = () => {
    ...
    setRatePerMinute("");
    setCapacityPerMinute(
      (current) => current || getDefaultCapacityForSelectedBuilding(),
    );
    setPriority(
      (current) =>
        current ||
        (selectedPlanId
          ? String(getDefaultPriorityForPlan(selectedPlanId))
          : ""),
    );
  ...
  const handleLinkedInputTargetChange = (
  ...
  const allConfigurationModeOptions: ConfigurationModeOption[] = [
    {
      ...
      detail: "Item + rate",
      isAvailable:
        selectedBuildingSupportsLinking || selectedBuildingSupportsPlanLinking,
  ...
  const configurationModeOptions = allConfigurationModeOptions.filter(
  ...
  const handleConfirm = () => {
    ...
    const hasLinkedOutput =
      configurationMode === "linked" && selectedLinkedOutput !== null;
    const hasPlanLinkedOutput =
      configurationMode === "plan" && selectedPlan !== null;
    const hasConfiguredItem =
      configurationMode === "manual" && !!selectedItemId && normalizedRate > 0;
    const hasLinkedOutputItem =
      hasLinkedOutput && !!selectedItemId && normalizedRate > 0;
    ...
    onAdd({
      ...
      selectedItemId:
        hasConfiguredItem || hasLinkedOutputItem || hasPlanLinkedOutput
          ? selectedItemId
          : undefined,
      ratePerMinute:
        hasConfiguredItem || hasLinkedOutputItem ? normalizedRate : undefined,
      ...
      capacityPerMinute:
        hasPlanLinkedOutput && normalizedCapacity > 0
          ? normalizedCapacity
          : undefined,
      priority:
        hasPlanLinkedOutput &&
        Number.isFinite(normalizedPriority) &&
        normalizedPriority >= 0
          ? normalizedPriority
          : undefined,
      linkedInputRef:
        selectedBuildingSupportsInputTargets && selectedLinkedInputTarget
          ? {
  ...
  return (
    <div className="modal modal-open">
      <div
        ...
        className="modal-box max-w-4xl max-h-[90vh] flex flex-col overflow-hidden p-0"
      >
        {/* Header - fixed */}
            ...
            <div className="flex gap-3 mb-4">
              <div className="form-control w-28">
                ...
                <input
                  ...
                  onBlur={() =>
                    setCount(String(sanitizeBulkBuildingCount(Number(count))))
                  }
                />
              </div>
            </div>
            ...
            <div className="mb-4 rounded-lg border border-base-300 bg-base-200/30 p-4">
              <div className="flex flex-col gap-4">
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="min-w-0">
                              ...
                              <span className="block truncate text-[10px] text-base-content/55">
                                {option.detail}
                              </span>
                            </span>
                          </label>
                ...
                <div className="rounded-md border border-base-300/70 bg-base-100/70 p-3">
                          ...
                          {selectedLinkedOutput ? (
                            <div className="min-w-0 text-xs">
                              <div className="truncate font-medium text-base-content/85">
                                {selectedLinkedOutput.baseName} /{" "}
                                {selectedLinkedOutput.item.name}
                              </div>
                  ...
                  ) : configurationMode === "plan" &&
                    selectedBuildingSupportsPlanLinking ? (
                          ...
                          <>
                            <label className="form-control">
                              ...
                              <input
                                ...
                                onChange={(event) =>
                                  setCapacityPerMinute(event.target.value)
                                }
                                placeholder="Auto"
                              />
                            </label>
                        ...
                        >
                          ...
                          {linkableInputTargets.map((target) => (
                            ...
                            >
                              ...
                              {target.linkedOutputLabel
                                ? ` · linked to ${target.linkedOutputLabel}`
                                : ""}
                            </option>
                          ))}
                        </ClippedSelect>
      ...
      {selectedBuilding && supportsItemConfiguration && (
        <SelectItemModal
          ...
          currentRatePerMinute={
            Number(ratePerMinute) > 0 ? Number(ratePerMinute) : undefined
          }
