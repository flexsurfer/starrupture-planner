...
vi.mock(
  "@/features/production-plans/ui/components/EmbeddedFlowDiagram",
  () => ({ EmbeddedFlowDiagram: () => <div>Production diagram</div> }),
);
...
function setup(overrides: Partial<AppState> = {}) {
  ...
  const items = [
    ...
    { id: "bar", name: "Bar", type: "processed" },
  ];
...
it("asks for a mode on first opening, including for existing bases, then allows changing it", async () => {
  ...
  const picker = screen.getByRole("dialog", {
    name: "How would you like to use My Bases?",
  });
  fireEvent.click(
    within(picker).getByRole("button", { name: /Planning mode/ }),
  );
  await waitFor(() =>
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument(),
  );
  expect(
    screen.queryByRole("tab", { name: "Logistics" }),
  ).not.toBeInTheDocument();
  expect(
    screen.queryByRole("button", { name: /Grids/ }),
  ).not.toBeInTheDocument();
  expect(
  ...
  await waitFor(() =>
    expect(screen.getByRole("tab", { name: "Logistics" })).toBeVisible(),
  );
...
it("keeps building and resource requirements visible and safely switches away from a hidden tab", async () => {
  const { harness } = setup({
    ...
    basesSelectedDetailTab: "buildings",
  });
  ...
  await waitFor(() =>
    expect(
      screen.queryByRole("tab", { name: /^Buildings/ }),
    ).not.toBeInTheDocument(),
  );
  ...
  expect(
    within(screen.getByRole("article", { name: "Ore input" })).getByText(
      "60/min",
    ),
  ).toBeVisible();
  ...
  expect(
    screen.queryByRole("button", {
      name: /owned count|Manage inputs|Manage buildings/,
    }),
  ).not.toBeInTheDocument();
  ...
  expect(
    screen.queryByText(/Requirements need attention/),
  ).not.toBeInTheDocument();
  fireEvent.click(screen.getByRole("button", { name: "Add Plan" }));
  await waitFor(() =>
    expect(harness.getState().productionPlanModalState.isOpen).toBe(true),
  );
...
it("offers the first-time picker with no bases, then switches directly without reopening settings", async () => {
  ...
  expect(
    screen.queryByRole("button", { name: "My Bases settings" }),
  ).not.toBeInTheDocument();
  fireEvent.click(screen.getByRole("switch", { name: "Advanced" }));
...
it("navigates from bases through plans and editing, preserves edits, and returns to each breadcrumb destination", async () => {
  ...
  expect(
    await screen.findByRole("heading", { name: "Outpost", level: 1 }),
  ).toBeVisible();
  expect(
    screen.queryByRole("button", { name: "Close base" }),
  ).not.toBeInTheDocument();
  fireEvent.click(screen.getByRole("tab", { name: /^Plans/ }));
  ...
  expect(
    screen.queryByRole("tablist", { name: "Base sections" }),
  ).not.toBeInTheDocument();
  const trail = screen.getByRole("navigation", { name: "Breadcrumb" });
  expect(within(trail).getByText("Bar plan")).toHaveAttribute(
    "aria-current",
    "page",
  );
  ...
  expect(await screen.findByRole("tab", { name: /^Plans/ })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  expect(harness.getState().basesList[0].productions[0].name).toBe(
    "Updated plan",
  );
  ...
  expect(
    await screen.findByRole("heading", { name: "My Bases" }),
  ).toBeVisible();
  expect(harness.getState().productionPlanModalState.isOpen).toBe(false);
  fireEvent.click(screen.getByRole("button", { name: "Open" }));
  fireEvent.click(
    await screen.findByRole("button", { name: "Back to My Bases" }),
  );
  expect(
    await screen.findByRole("heading", { name: "My Bases" }),
  ).toBeVisible();
