...
export function buildLogisticsCanvasData({
  ...
  for (const model of models) {
    ...
    if (brokenInputs.length > 0) {
      warnings.push(
        `${brokenInputs.length} broken link${brokenInputs.length > 1 ? "s" : ""}`,
      );
    }
    const unassignedOutputs = model.outputs.filter((output) => !output.itemId);
    if (unassignedOutputs.length > 0) {
      warnings.push(
        `${unassignedOutputs.length} unassigned output${unassignedOutputs.length > 1 ? "s" : ""}`,
      );
    }
    ...
    const nodeData: BaseNetworkNodeData = {
      ...
      inputs: model.incomingInputs.map((input) => ({
        ...
        hasBrokenLink:
          !!input.linkedOutputStatus && input.linkedOutputStatus !== "ok",
      })),
    };
  ...
  if (showEnergy) {
    const groupMap = new Map<
      string,
      { generation: number; consumption: number; baseCount: number }
    >();
    for (const model of models) {
      const stats = baseStats[model.baseId];
    ...
    for (const [groupId, pooled] of groupMap.entries()) {
      ...
      for (const model of models) {
        ...
        const net =
          (stats.localEnergyGeneration ?? 0) - (stats.energyConsumption ?? 0);
        if (net === 0) continue;
    ...
    for (const model of models) {
      for (const output of model.outputs) {
        for (const input of output.linkedInputs) {
          ...
          if (isBroken) {
            coveredBrokenInputIds.add(
              `${input.baseId}:${input.baseBuildingId}`,
            );
          }
  ...
  if (showBroken) {
    for (const model of models) {
      for (const input of model.incomingInputs) {
        if (!input.linkedOutputStatus || input.linkedOutputStatus === "ok")
          continue;
  ...
  gridNodes.forEach((node, index) => {
    node.position = {
      x:
        (index % columns) * BASE_CELL_SIZE +
        (BASE_NODE_WIDTH - GRID_NODE_WIDTH) / 2,
