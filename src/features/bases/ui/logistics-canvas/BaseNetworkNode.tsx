  ...
  return (
    <div className="rounded-xl border-2 border-base-300 bg-base-100 shadow-lg w-[260px] overflow-hidden">
      ...
      <Handle
        ...
        className="!w-2.5 !h-2.5 !bg-warning"
        style={{ left: "42%" }}
      />
      <Handle
        id="energy-out"
      ...
      {/* Header */}
      <div
        className={`px-3 py-2 border-b border-base-300 ${d.hasWarnings ? "bg-warning/10" : "bg-base-200/50"}`}
      >
      ...
      <div className="px-3 py-2 space-y-1.5 text-xs">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <span
            className={`font-mono font-medium ${d.energyBalance < 0 ? "text-error" : "text-success"}`}
          >
            {d.energyBalance >= 0 ? "+" : ""}
            {formatRate(d.energyBalance)} MW
          </span>
        </div>
            ...
            {d.outputs.slice(0, 4).map((output) => (
              ...
              >
                <span className="truncate max-w-[140px]">
                  {output.itemName || "unassigned"}
                </span>
                <span className="font-mono text-[10px]">
                  {formatRate(output.ratePerMinute)}/min
                </span>
              </div>
            ))}
            {d.outputs.length > 4 && (
              <div className="text-base-content/50 pl-1">
                +{d.outputs.length - 4} more
              </div>
            )}
          </div>
        )}
        ...
        {d.inputs.length > 0 && (
          <div className="pt-1 border-t border-base-300/50">
            ...
            {d.inputs.slice(0, 4).map((input) => (
              ...
              >
                <span
                  className={`truncate max-w-[140px] ${input.hasBrokenLink ? "text-error" : ""}`}
                >
                  {input.itemName || "empty"}
                  {input.hasBrokenLink ? " ✗" : ""}
                </span>
                <span className="font-mono text-[10px]">
                  {formatRate(input.ratePerMinute || 0)}/min
                </span>
              </div>
            ))}
            {d.inputs.length > 4 && (
              <div className="text-base-content/50 pl-1">
                +{d.inputs.length - 4} more
              </div>
            )}
          </div>
        )}
        ...
        {d.hasWarnings && (
          <div className="pt-1 border-t border-base-300/50">
            {d.warnings.slice(0, 2).map((warning, idx) => (
              <div key={idx} className="text-warning truncate">
                ⚠ {warning}
              </div>
            ))}
          </div>
