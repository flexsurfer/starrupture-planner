...
import {
  buildLogisticsCanvasData,
  type LayerFilter,
} from "./logisticsCanvasUtils";
...
const EdgeDetailPanel: React.FC<EdgeDetailPanelProps> = ({
  ...
  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-50 bg-base-100 border border-base-300 rounded-lg shadow-xl p-3 min-w-[280px] max-w-[380px]">
    ...
    <div className="space-y-1 text-xs">
      ...
      {edgeData.ratePerMinute != null && (
        <div className="flex justify-between">
          ...
          <span className="font-mono">
            {Math.round(edgeData.ratePerMinute)}/min
          </span>
        </div>
      )}
      {edgeData.isBroken && (
      ...
      {edgeData.type === "energy" &&
        (edgeData.groupName || edgeData.groupId) && (
          <div className="flex justify-between">
...
const LogisticsCanvasInner: React.FC = () => {
  ...
  const models =
    useSubscription([appIds.subscriptions.BASES_LOGISTICS_VIEW_MODELS]) ||
    EMPTY_MODELS;
  const energyGroups =
    useSubscription([appIds.subscriptions.ENERGY_GROUPS_LIST]) ||
    EMPTY_ENERGY_GROUPS;

  // Compute base stats from bases (inline to avoid new subscription)
  const baseStatsMap = useSubscription([
    appIds.subscriptions.BASES_ALL_DETAIL_STATS,
  ]);

  const [selectedEdgeData, setSelectedEdgeData] =
    useState<EdgeDetailData | null>(null);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (nodes.length > 0)
        fitView({ duration: 300, padding: 0.15, minZoom: 0.1 });
    }, 50);
        ...
        <Panel position="top-right">
          <button
            ...
            className={`btn btn-sm gap-1.5 shadow ${
              showEnergy
                ? "btn-warning"
                : "bg-base-100 border border-base-300 text-base-content/80"
            }`}
          >
            <span>⚡</span>
