...
export const BuildingCountControl: React.FC<BuildingCountControlProps> = ({
  ...
  const [pendingCommittedValue, setPendingCommittedValue] = useState<
    number | null
  >(null);
  ...
  const isDirty = isValidDraft
    ? parsedDraftValue !== value
    : draftValue !== String(value);
  const hasPendingCommit =
    isValidDraft && pendingCommittedValue === parsedDraftValue;
  const canSave =
    isValidDraft && parsedDraftValue !== value && !hasPendingCommit;
  ...
  const stepClass = cardLayout
    ...
    : compact
      ? "join-item btn-sm h-8 min-h-8 w-8 p-0 font-normal text-base-content/65"
      : "join-item btn-xs";
  ...
  return (
    <div
      className={
        cardLayout
          ? "grid w-full min-w-0 grid-cols-[1.75rem_minmax(0,1fr)_1.75rem] sm:grid-cols-[2rem_minmax(0,1fr)_2rem]"
          : "join shrink-0"
      }
    >
      {!compact && !cardLayout && (
      ...
      <input
        ...
        onBlur={() => {
          if (parsedDraftValue !== null) {
            setDraftOverride(
              parsedDraftValue === value ? null : String(parsedDraftValue),
            );
          }
        }}
      />
