  ...
  return <>
    ...
    {open && <dialog ref={dialog} className="modal" aria-labelledby={titleId}
      ...
      <div className="modal-box max-w-xl">
        <h2 id={titleId} className="text-lg font-semibold">How would you like to use My Bases?</h2>
        <p className="mt-2 text-sm text-base-content/65">Choose a mode for all your bases. Use the Advanced switch to change it anytime.</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={() => selectMode('planning')} aria-pressed={mode === 'planning'}
            ...
            <span className="mt-2 block text-sm text-base-content/70">Create production plans and see the buildings and resources you need.</span>
          </button>
          <button type="button" onClick={() => selectMode('advanced')} aria-pressed={mode === 'advanced'}
            ...
            <span className="mt-2 block text-sm text-base-content/70">Keep your bases in sync with what you build in game. Manage buildings, energy, transport, logistics, inputs and outputs.</span>
          </button>
        </div>
        <p className="mt-4 text-xs text-base-content/55">Changing mode only changes what is shown. Your plans and base configuration are kept.</p>
      </div>
    </dialog>}
  </>;
};
