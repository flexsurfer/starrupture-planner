...
vi.mock("@/app/uklad/bindings", () => ({
  ...
  useSubscription: ([id]: [string]) => {
    switch (id) {
      ...
      case appIds.subscriptions.ITEMS_AVAILABLE_ITEMS_BY_BUILDING_ID:
        return items;
      case appIds.subscriptions.BUILDINGS_BY_ID_MAP:
        return { receiver, dispatcher, smelter };
      case appIds.subscriptions.BASES_BASE_BY_ID:
        return base;
      case appIds.subscriptions.BASES_LIST:
        return [base];
      case appIds.subscriptions.BASES_BUILDING_SECTION_BUILDINGS:
        return [grouped];
      case appIds.subscriptions.BASES_BUILDING_SECTION_STATS:
        return {
          buildingCount: 3,
          totalHeat: 9,
          totalPowerConsumption: 15,
...
const grouped: BuildingSectionBuilding = {
  ...
  count: 3,
  isGrouped: true,
  activePlanNames: ["Wolfram bars"],
};
const input: BaseBuilding = {
  id: "input",
...
const output: BaseBuilding = {
  id: "output",
  buildingTypeId: "dispatcher",
  sectionType: "outputs",
  sourceProductionId: "plan",
...
const entry = (baseBuilding: BaseBuilding): BuildingSectionBuilding => ({
  ...
  sectionType: baseBuilding.sectionType as "inputs" | "outputs",
  count: 1,
  isGrouped: false,
  activePlanNames: [],
});
...
it("keeps Add accessible when a section is collapsed without toggling it", () => {
  ...
  expect(
    screen.queryByRole("heading", { name: "Smelter" }),
  ).not.toBeInTheDocument();
  fireEvent.click(
    screen.getByRole("button", { name: "Add production building" }),
  );
...
it("preserves count adjustments, explicit draft saving, and removal confirmation", () => {
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Increase Smelter production count" }),
  );
  ...
  dispatch.mockClear();
  fireEvent.change(
    screen.getByRole("textbox", { name: "Smelter production count" }),
    { target: { value: "12" } },
  );
  expect(dispatch).not.toHaveBeenCalled();
  fireEvent.click(
    screen.getByRole("button", { name: "Save Smelter production count" }),
  );
  expect(dispatch).toHaveBeenLastCalledWith([
    appIds.events.BASES_SET_BUILDING_SECTION_TYPE_COUNT,
    "base",
...
it("opens the item editor from a manual input and saves its new rate", () => {
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Edit Wolfram Bar item and rate" }),
  );
  expect(
    screen.getByRole("heading", { name: "Select Item for Receiver" }),
  ).toBeVisible();
  ...
  expect(
    screen.queryByRole("heading", { name: "Select Item for Receiver" }),
  ).not.toBeInTheDocument();
});
...
it("shows live linked supply and lets the source return to manual", () => {
  ...
  const { rerender } = render(
    <BuildingSectionCard sectionBuilding={entry(linked)} baseId="base" />,
  );
  expect(screen.getByText("Wolfram Wire")).toBeVisible();
  expect(screen.getByText("150/min")).toBeVisible();
  expect(
    screen.queryByRole("button", { name: /Edit .* item and rate/ }),
  ).not.toBeInTheDocument();
  ...
  rerender(
    <BuildingSectionCard
      sectionBuilding={entry({
        ...linked,
        linkedOutput: { ...linked.linkedOutput, baseId: "missing" },
      })}
      baseId="base"
    />,
  );
  expect(
    screen.getByTitle("Broken linked output: Missing base / output"),
  ).toHaveTextContent("Broken link");
...
it("keeps plan-linked output capacity and target controls available", () => {
  ...
  fireEvent.change(screen.getByRole("spinbutton", { name: "Capacity/min" }), {
    target: { value: "120" },
  });
  expect(dispatch).toHaveBeenLastCalledWith([
