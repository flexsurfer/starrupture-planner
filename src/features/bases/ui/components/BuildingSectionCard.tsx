...
import {
  ...
  isRawExtractor,
} from "@/features/bases/building-section";
...
const useLinkedInputData = (baseBuilding: BaseBuilding): LinkedInputData => {
  ...
  const hasError =
    !!resolved.linkedOutput && resolved.linkedOutputStatus !== "ok";
...
const BuildingItemSummary = ({
  item,
  rate,
  status,
  hasError,
}: BuildingItemSummaryProps) => (
  <span className="flex min-w-0 flex-1 items-center gap-2">
    {item && (
      <span className="shrink-0 [&>div]:size-8 [&_img]:size-8">
        <ItemImage itemId={item.id} item={item} size="small" />
      </span>
    )}
    <span className="min-w-0 flex-1">
      <span className="block text-sm font-normal leading-snug text-base-content/75 break-words">
        {item?.name || "No item selected"}
      </span>
      {item && (
        <span
          className="block text-xs font-medium tabular-nums"
          style={{ color: getItemCategoryColor(item.type) }}
        >
          {formatRate(rate)}/min
        </span>
      )}
    ...
    {status && (
      <span
        className={`shrink-0 text-[11px] ${hasError ? "text-error" : "text-base-content/60"}`}
      >
        {status}
      </span>
    )}
...
const LinkedInputItemSummary = ({
  ...
  const selectedItem = resolved.selectedItemId
    ? itemsMap[resolved.selectedItemId]
    : null;

  return (
    <div
      className={`rounded-md border bg-base-content/5 p-2 ${hasError ? "border-error/50" : "border-transparent"}`}
      title={`${hasError ? "Broken linked output" : "Linked output"}: ${label}`}
    >
      <BuildingItemSummary
        item={selectedItem}
        rate={resolved.ratePerMinute}
        status={hasError ? "Broken link" : "Linked"}
        hasError={hasError}
      />
    </div>
  );
  ...
  return useMemo(() => {
    ...
    for (const base of allBases) {
      for (const output of base.buildings) {
        ...
        if (isLogisticsExcludedOutputBuildingId(output.buildingTypeId))
          continue;

        const resolvedOutput = resolveOutputBuilding(output, base);
        if (
          !resolvedOutput.selectedItemId ||
          !isConfiguredPositiveRate(resolvedOutput.ratePerMinute)
        )
          continue;
    ...
    return outputs.sort((left, right) => {
      if (left.isCurrentBase !== right.isCurrentBase)
        return left.isCurrentBase ? -1 : 1;
...
const InputOutputLinkControls: React.FC<InputOutputLinkControlsProps> = ({
  baseId,
  baseBuilding,
}) => {
  ...
  const selectedOutput = selectedOutputKey
    ? outputs.find(
        (output) =>
          getLinkableOutputKey(output.baseId, output.baseBuildingId) ===
          selectedOutputKey,
      ) || null
    : null;
  const selectedOutputExists =
    !selectedOutputKey ||
    outputs.some(
      (output) =>
        getLinkableOutputKey(output.baseId, output.baseBuildingId) ===
  ...
  const handleSourceChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (!nextKey) {
      runtime.dispatch([
        ...
        isConfiguredPositiveRate(resolved.ratePerMinute)
    ...
    const output = outputs.find(
      (candidate) =>
        getLinkableOutputKey(candidate.baseId, candidate.baseBuildingId) ===
        nextKey,
    );
    if (!output) return;
  ...
  return (
    <div className="min-w-0">
      <label className="flex min-w-0 items-center gap-2">
        <span className="w-10 shrink-0 text-xs text-base-content/60">
          Source
        </span>
        <ClippedSelect
          size="sm"
        ...
        >
          ...
          {outputs.map((output) => {
            const key = getLinkableOutputKey(
              output.baseId,
              output.baseBuildingId,
            );
  ...
  return useMemo(() => {
    ...
    for (const base of allBases) {
      for (const input of base.buildings) {
        ...
        const itemId =
          resolvedInput.selectedItemId || input.linkedOutput?.itemIdSnapshot;
        const item = itemId
          ? itemsById[itemId] || { id: itemId, name: itemId, type: "unknown" }
          : undefined;
        const resolution = input.linkedOutput
          ? resolveLinkedOutput(input, allBases)
          : null;
        const sourceOutputBuilding = resolution?.sourceOutput
          ? buildingsById[resolution.sourceOutput.buildingTypeId]
          : null;
        ...
        inputs.push({
          ...
          item,
          ratePerMinute:
            resolvedInput.ratePerMinute ||
            input.linkedOutput?.ratePerMinuteSnapshot,
    ...
    return inputs.sort((left, right) => {
      const currentBaseDelta =
        Number(right.baseId === currentBaseId) -
        Number(left.baseId === currentBaseId);
...
const OutputInputLinkControls: React.FC<OutputInputLinkControlsProps> = ({
  ...
  const handleRemoveInput = (input: LinkableInputItem) => {
    runtime.dispatch([
      ...
      input.baseBuildingId,
      input.item?.id || null,
      isConfiguredPositiveRate(input.ratePerMinute)
        ? input.ratePerMinute
        : null,
    ]);
  };
...
const OutputPlanLinkControls: React.FC<OutputPlanLinkControlsProps> = ({
  ...
  const updatePlanLink = (payload: {
    ...
    const hasSourceProductionId = Object.prototype.hasOwnProperty.call(
      payload,
      "sourceProductionId",
    );
    runtime.dispatch([
      ...
      {
        sourceProductionId:
          (hasSourceProductionId
            ? payload.sourceProductionId
            : selectedPlanId) || null,
        allocationMode: "auto",
        capacityPerMinute:
          payload.capacityPerMinute ?? baseBuilding.capacityPerMinute ?? null,
...
export const BuildingSectionCard: React.FC<BuildingSectionCardProps> = ({
  ...
  const isOutputBuilding =
    !isGrouped && baseBuilding?.sectionType === "outputs";
  ...
  const setGroupedCount = useCallback(
    (nextCount: number) => {
      runtime.dispatch([
  ...
  return (
    <>
      <article
        className={`min-w-0 rounded-lg border bg-base-200 p-2 sm:p-3 ${isInActivePlan ? "border-primary/60" : "border-base-300"}`}
      >
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-medium leading-snug text-base-content/80 break-words">
              {displayName}
            </h3>
          ...
          <button
            type="button"
            className="btn btn-sm btn-ghost h-8 min-h-8 w-8 shrink-0 p-0 text-base-content/50 hover:text-error"
        ...
        {description && (
          <p className="mt-2 text-xs leading-relaxed text-base-content/60 break-words">
            {description}
          </p>
        )}
        {baseBuilding?.planningOwnerPlanId && (
          <p
            className="mt-2 text-xs text-base-content/55"
            title="Created for this plan in Planning mode. Editing this endpoint's configuration makes it manually managed."
          >
            Managed by plan:{" "}
            {base?.productions.find(
              (plan) => plan.id === baseBuilding.planningOwnerPlanId,
            )?.name || "Unknown plan"}
          </p>
        )}
        {isInActivePlan && (
          <p className="mt-2 text-xs leading-snug text-base-content/60 break-words">
            <span
              aria-hidden="true"
        ...
        {isGrouped && (
          <div className="mt-2 flex flex-wrap items-center justify-between gap-1 border-t border-base-300 pt-2">
          ...
          <div className="mt-2 space-y-2">
            {isLinkedInput ? (
              ...
              <button
                ...
                aria-label={
                  selectedItem
                    ? `Edit ${selectedItem.name} item and rate`
              ...
              >
                {selectedItem ? (
                  ...
                  <span className="flex-1 text-sm text-base-content/65">
                    Select item & rate
                  </span>
                )}
                <svg
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={1.5}
                    d="m16.862 4.487 1.687-1.688a1.875 1.875 0 0 1 2.652 2.652L9.832 16.82a4.5 4.5 0 0 1-1.897 1.13L5.25 18.75l.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.862 4.487Zm0 0 2.651 2.652"
                  />
                </svg>
              </button>
            )}

            {isLinkableInputBuilding && (
              <InputOutputLinkControls
                baseId={baseId}
                baseBuilding={baseBuilding}
              />
            )}
            {isOutputBuilding && resolvedOutput && (
              <div className="space-y-2">
