...
export const BuildingSection: React.FC<BuildingSectionProps> = ({
  ...
  const sectionBuildings = useSubscription([
  ...
  return (
    <ExpandableSection
      ...
      summary={
        <>
          ...
          {stats.totalPowerGeneration > 0 && (
            <span title="Power generation">
              ⚡ +{stats.totalPowerGeneration} MW
            </span>
          )}
      ...
      actions={
        <button
          type="button"
          className="btn btn-sm btn-ghost h-8 min-h-8 gap-1 px-2 text-xs font-normal text-base-content/65"
          aria-label={`Add ${title.toLowerCase()} building`}
          onClick={onAdd}
        >
          <svg
            aria-hidden="true"
