...
export const BaseCoreInfo: React.FC<{ onRename?: () => void }> = ({
  onRename,
}) => {
  const runtime = useRuntime();
  const advanced =
    useSubscription([appIds.subscriptions.BASES_MODE]) !== "planning";
  const expanded = useSubscription([
    appIds.subscriptions.BASES_DETAILS_EXPANDED,
  ]);
  const detailStats = useSubscription([
  ...
  const onCoreLeveChange = useCallback(
    (level: number) => {
  ...
  return (
    <div className="relative bg-base-200 rounded-lg p-2 pr-12 sm:p-3 sm:pr-12">
        ...
        {advanced && (
          <button
            ...
            title={
              expanded
                ? "Hide base details"
                : isHeatOverCapacity || isEnergyInsufficient
                  ? "Show base details — heat or energy needs attention"
                  : "Show base details"
            }
      ...
      <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-3">
            ...
            {advanced && expanded && (
              <>
                <p className="hidden sm:block text-xs text-base-content/70 mt-1">
                  The Core defines the buildable area for this Base. Buildings
                  can only be placed inside the Core area.
                </p>
                {/* Core Level Selector */}
        ...
        {/* Stats */}
        {advanced && expanded && (
          <div className="flex w-full flex-col gap-1 self-stretch sm:w-auto">
            <div className="flex flex-wrap justify-between gap-3 sm:justify-start sm:gap-4">
              <div className="flex-shrink-0">
                <div className="text-xs text-base-content/70 mb-0.5">
                  Buildings
                </div>
