  ...
  return (
    <div>
      {table.groups.length === 0 ? (
        <p className="rounded-lg border border-base-300 bg-base-100 p-4 text-sm text-base-content/60">
          No production plans yet. Add a plan to see its production chain and
          requirements.
        </p>
      ) : (
        <div className="overflow-hidden rounded-lg border border-base-300 bg-base-100">
          <table className="w-full table-fixed">
            <caption className="sr-only">
              Base production cards ordered by target and item category
            </caption>
            {table.groups.map((group) => (
              ...
              >
                <tr>
                  <th
                    scope="rowgroup"
                    className="bg-base-200 px-2 py-1.5 text-left text-xs font-medium text-base-content/75 sm:px-3 sm:py-2 sm:text-sm"
                  >
                    {group.type === "target"
                      ? "Targets"
                      : group.type === "launcher"
                        ? "Delivery"
                        : getCategoryDisplayName(group.type)}
                  </th>
                </tr>
                <tr>
