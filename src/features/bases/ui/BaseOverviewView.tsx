...
export const BaseOverviewView: React.FC = () => {
  ...
  return (
    <div>
      {table.groups.length === 0 ? <p className="rounded-lg border border-base-300 bg-base-100 p-4 text-sm text-base-content/60">{t("No production plans yet. Add a plan to see its production chain and requirements.")}</p> : (
        <div className="overflow-hidden rounded-lg border border-base-300 bg-base-100">
          <table className="w-full table-fixed">
            <caption className="sr-only">{t("Base production cards ordered by target and item category")}</caption>
