...
const buildings: Building[] = [
  {
    ...
    recipes: [
      {
        output: { id: "bar", amount_per_minute: 30 },
  ...
  },
  {
    ...
    recipes: [
      {
        output: { id: "wire", amount_per_minute: 15 },
        inputs: [{ id: "bar", amount_per_minute: 60 }],
      },
    ],
  },
...
const base: Base = {
  ...
  productions: [
    {
      id: "wire-plan",
      name: "Wire for Future Health Solutions",
      selectedItemId: "wire",
      targetAmount: 15,
      active: true,
      requiredBuildings: [
        { buildingId: "smelter", count: 2 },
        { buildingId: "assembler", count: 1 },
      ],
    },
    {
      id: "bar-plan",
  ...
  buildings: [
    ...
    ...Array.from({ length: 4 }, (_, index) => ({
      id: `output-${index}`,
      name: `Wire dispatch ${index + 1}`,
      buildingTypeId: "package_dispatcher",
      sectionType: "outputs",
      sourceProductionId: "wire-plan",
      capacityPerMinute: 5,
    })),
  ],
};
...
function setup(
  ...
  buildingData: Building[] = buildings,
  fullDetail = false,
) {
  ...
  render(
    <UkladProvider runtime={runtime}>
      {fullDetail ? <BaseDetailView /> : <BaseOverviewView />}
    </UkladProvider>,
  );
  return harness;
}
...
it("groups targets and shared ingredients like the planner while retaining inactive demand", () => {
  const harness = setup();
  const table = harness.getSubscriptionValue([
  ...
  expect(bar).toMatchObject({
    ...
    targetRate: 30,
    requiredBuildings: 3,
    coverage: { owned: 3, totalRequired: 3, missing: 0 },
  });
  const ore = within(
    screen.getByRole("article", { name: "Wolfram Ore input" }),
  );
  ...
  expect(ore.getByRole("meter")).toHaveAttribute(
    "aria-valuetext",
    "150/min of 180/min covered",
  );
...
it("updates shortages when owned counts are changed and only saves typed counts explicitly", async () => {
  ...
  expect(wire.getByRole("meter", { name: "Item coverage" })).toHaveAttribute(
    "aria-valuenow",
    "0",
  );
  fireEvent.click(
    wire.getByRole("button", { name: "Increase Assembler owned count" }),
  );
  await waitFor(() =>
    expect(
      wire.getByRole("textbox", { name: "Assembler owned count" }),
    ).toHaveValue("1"),
  );
  await waitFor(() =>
    expect(
  ...
  expect(wire.getByRole("meter", { name: "Base coverage" })).toHaveAttribute(
    "aria-valuenow",
    "100",
  );
  fireEvent.change(
  ...
  expect(
    harness
      ...
      .find((row) => row.buildingId === "assembler")?.owned,
  ).toBe(1);
  fireEvent.click(
    wire.getByRole("button", { name: "Save Assembler owned count" }),
  );
...
it("shares building coverage across different recipes and keeps every count control in sync", async () => {
  ...
  multiRecipeBuildings[0].recipes!.push({
    output: { id: "plate", amount_per_minute: 30 },
    inputs: [{ id: "ore", amount_per_minute: 60 }],
  });
  const expandedBase = structuredClone(base);
  expandedBase.productions.push({
    id: "plate-plan",
    name: "Plate reserves",
  ...
  expect(
    harness.getSubscriptionValue([appIds.subscriptions.BASES_PRODUCTION_TABLE])
      .missingBuildings,
  ).toBe(2);
  fireEvent.click(
    bar.getByRole("button", { name: "Increase Smelter owned count" }),
  );
  await waitFor(() =>
    expect(
      plate.getByRole("textbox", { name: "Smelter owned count" }),
    ).toHaveValue("4"),
  );
  expect(bar.getByRole("meter", { name: "Base coverage" })).toHaveAttribute(
    "aria-valuenow",
    "100",
  );
  expect(plate.getByRole("meter", { name: "Base coverage" })).toHaveAttribute(
    "aria-valuenow",
    "100",
  );
  fireEvent.click(
    plate.getByRole("button", { name: "Increase Smelter owned count" }),
  );
  await waitFor(() =>
    expect(
      bar.getByRole("textbox", { name: "Smelter owned count" }),
    ).toHaveValue("5"),
  );
...
it("rounds whole buildings per plan before merging the same recipe", () => {
  ...
  expect(
    ...
    expect.objectContaining({
      rate: expect.closeTo(25),
      targetRate: 25,
      requiredBuildings: 2,
      coverage: expect.objectContaining({ totalRequired: 2 }),
    }),
  ]);
});
...
it("keeps different recipes for the same item separate", () => {
  ...
  variants[0].recipes!.push({
    id: "efficient",
    variant: "alternative",
    output: { id: "bar", amount_per_minute: 20 },
  ...
  const cards = harness
    .getSubscriptionValue([appIds.subscriptions.BASES_PRODUCTION_TABLE])
    .groups.flatMap((group) => group.cards)
    .filter((card) => card.item.id === "bar");
  ...
  expect(cards).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        ...
        rate: 30,
        requiredBuildings: 2,
        node: expect.objectContaining({ recipeIndex: 1 }),
      }),
      expect.objectContaining({
        targetRate: 0,
        rate: 60,
        requiredBuildings: 2,
        node: expect.objectContaining({ recipeIndex: 0 }),
      }),
    ]),
  );
});
...
it("shows required and covered rates with direct neutral plan editing and no overview extras", async () => {
  ...
  fireEvent.click(
    plan.getByRole("button", { name: "Edit Wire for Future Health Solutions" }),
  );
  await waitFor(() =>
    expect(harness.getState().productionPlanModalState).toMatchObject({
      isOpen: true,
      editSectionId: "wire-plan",
    }),
  );
  expect(
    screen.queryByRole("button", { name: "Plan details" }),
...
it("allocates scarce item capacity and input supply to active plans first without double counting", () => {
  ...
  scarce.buildings = scarce.buildings.filter(
    (building) => !["smelter-1", "smelter-2"].includes(building.id),
  );
  ...
  const cards = harness
    ...
    .groups.flatMap((group) => group.cards);
  expect(
    cards.find((card) => card.item.id === "bar")?.itemCoverage,
  ).toMatchObject({
    required: 90,
    covered: 30,
    planDemands: [
  ...
  expect(
    cards.find((card) => card.item.id === "ore")?.itemCoverage,
  ).toMatchObject({
    required: 180,
    covered: 50,
    planDemands: [
...
it("keeps one Add Plan action on every base tab", async () => {
  const harness = setup(
    { id: "base", name: "Outpost", buildings: [], productions: [] },
    buildings,
    true,
  );
...
it("puts Delivery before Targets in the base production table", () => {
  ...
  delivery.productions[0].corporationLevel = {
    corporationId: "corp",
    level: 1,
  };
  const harness = setup(delivery, [
    ...buildings,
    {
      id: ORBITAL_CARGO_LAUNCHER_BUILDING_ID,
...
it("keeps invalid plans accessible and handles an empty base", async () => {
  ...
  const unavailable = within(
    screen.getByRole("article", { name: "removed-item" }),
  );
  expect(unavailable.getByText("Review plan setup")).toBeVisible();
  expect(
    unavailable.getByRole("button", { name: "Edit Old plan" }),
  ).toBeVisible();
  cleanup();
  const harness = setup({
    id: "base",
  ...
  expect(
    screen.getByText(
      "No production plans yet. Add a plan to see its production chain and requirements.",
    ),
  ).toBeVisible();
  expect(
    screen.queryByRole("button", { name: "Add Plan" }),
  ).not.toBeInTheDocument();
  await act(async () => {
