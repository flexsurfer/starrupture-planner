...
export const BaseDetailView: React.FC = () => {
  ...
  const { isOpen: isEditingPlan } = useSubscription([
    appIds.subscriptions.PRODUCTION_PLAN_MODAL_OPEN_STATE,
  ]);
  const advanced =
    useSubscription([appIds.subscriptions.BASES_MODE]) !== "planning";
  const selectedTab =
    useSubscription([appIds.subscriptions.BASES_SELECTED_DETAIL_TAB]) || "base";
  const advancedOnlyTabs: BaseDetailTab[] = ["buildings", "balance", "layout"];
  const activeTab =
    !advanced && advancedOnlyTabs.includes(selectedTab) ? "base" : selectedTab;
  ...
  if (activeTab === "layout") {
    return (
      <div className="h-full min-h-0 flex flex-col">
        <BaseLayoutView onBack={() => setActiveTab("base")} />
      </div>
    );
  }
  ...
  return (
    <div className="h-full min-h-0 px-2 pb-2 lg:px-3 lg:pb-3 flex flex-col">
      <div
        hidden={isEditingPlan}
        className={isEditingPlan ? "hidden" : "flex min-h-0 flex-1 flex-col"}
      >
        <NavigationHeader
          title={selectedBase.name}
          breadcrumbs={[
            { label: "My Bases", onClick: backToBases },
        ...
        <div className="flex-1 flex flex-col min-h-0">
            ...
            >
              ...
              {advanced && hasLayout && (
                <button
                  ...
                  id="base-tab-balance"
                  aria-controls="base-panel-balance"
                  className={`tab shrink-0 px-2 sm:px-3 text-xs sm:text-sm font-semibold flex flex-nowrap items-center gap-1 sm:gap-1.5 whitespace-nowrap ${activeTab === "balance" ? "tab-active" : ""}`}
              ...
              {advanced && (
                <button
                  ...
                  aria-selected={false}
                  id="base-tab-layout"
                  className="tab shrink-0 px-2 sm:px-3 text-xs sm:text-sm font-semibold flex flex-nowrap items-center gap-1 sm:gap-1.5 whitespace-nowrap"
          ...
          <div className="flex-1 overflow-auto min-h-0">
            ...
            {activeTab === "balance" && hasLayout && (
              <div
                id="base-panel-balance"
                role="tabpanel"
                aria-labelledby="base-tab-balance"
                className="rounded-lg bg-base-200 p-3 sm:p-4"
              >
                <BaseLayoutBalanceSummary
