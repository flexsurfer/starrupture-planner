...
export type BasesStatsShareResult =
  "shared" | "copied" | "downloaded" | "cancelled";
...
const formatPercent = (value: number): string =>
  `${clampPercentage(value).toFixed(1)}%`;
  ...
  for (const base of bases) {
    for (const plan of base.productions || []) {
      ...
      if (plan.corporationLevel) {
        const corporation = corporationsById.get(
          plan.corporationLevel.corporationId,
        );
        const corpLevel = corporation?.levels.find(
          (lvl) => lvl.level === plan.corporationLevel?.level,
        );
        const component = corpLevel?.components.find(
          (c) => c.id === plan.selectedItemId,
        );
        if (component) {
...
export const createBasesStatsShareText = (
  ...
  const topBasesLine =
    topBases.length > 0
      ? topBases
          .map((base) => `${base.name} (${base.buildings.length})`)
          .join(", ")
      : "No bases yet";
  const topItemsLine =
    topProducedItems.length > 0
      ? topProducedItems
          .map(
            (item) =>
              `${item.itemName} (${numberFormatter.format(item.totalRatePerMinute)}/min)`,
          )
          .join(", ")
...
export const renderBasesStatsCard = async (
  ...
  if (topBases.length === 0) {
    ...
    topBases.forEach((base, index) => {
      ...
      ctx.fillText(
        `${numberFormatter.format(base.buildings.length)} b`,
        1088,
        rowY + 5,
      );
      ctx.textAlign = "left";
  ...
  if (topProducedItems.length === 0) {
    ...
    topProducedItems.slice(0, 4).forEach((item, index) => {
      ...
      ctx.fillText(
        `${numberFormatter.format(item.totalRatePerMinute)}/m`,
        1088,
        rowY,
      );
      ctx.textAlign = "left";
...
export const shareBasesStats = async (
  ...
  const shareFile = new File([cardBlob], SHARE_FILE_NAME, {
    type: "image/png",
  });
