...
export const LOGISTICS_DEFAULT_OUTPUT_BUILDING_ID =
  PACKAGE_DISPATCHER_BUILDING_ID;
...
const formatFallbackName = (building: BaseBuilding): string =>
  building.name || building.buildingTypeId;

export function isLogisticsOutput(baseBuilding: BaseBuilding): boolean {
  return (
    baseBuilding.sectionType === "outputs" &&
    !isLogisticsExcludedOutputBuildingId(baseBuilding.buildingTypeId)
  );
}

export function isLogisticsInput(
...
export function getLogisticsOutputCapacityPerMinute(
  baseBuilding: BaseBuilding,
): number | undefined {
...
function buildInputLink(
  ...
  const ratePerMinute =
    resolved.ratePerMinute ?? input.linkedOutput?.ratePerMinuteSnapshot;
...
function getOutputLinkedInputs(
  ...
  for (const base of allBases) {
    for (const building of base.buildings) {
      ...
      inputs.push(
        buildInputLink(base, building, buildingsById, itemsById, allBases),
      );
    }
  }
...
function buildOutputs(
  ...
  return selectedBase.buildings.filter(isLogisticsOutput).map((output) => {
...
export function buildBaseLogisticsViewModel(params: {
  ...
  return {
    ...
    incomingInputs: buildIncomingInputs(
      selectedBase,
      bases,
      buildingsById,
      itemsById,
