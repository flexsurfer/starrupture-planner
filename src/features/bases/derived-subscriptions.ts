...
export function collectConfiguredSectionItems(
  base: Base,
  ...
  for (const baseBuilding of base.buildings) {
    ...
    const itemId =
      resolvedBuilding.selectedItemId ??
      baseBuilding.linkedOutput?.itemIdSnapshot;
    const ratePerMinute =
      resolvedBuilding.ratePerMinute ??
      baseBuilding.linkedOutput?.ratePerMinuteSnapshot;
    const hasRate =
      typeof ratePerMinute === "number" && Number.isFinite(ratePerMinute);
    const includeZeroRateOutput =
      sectionType === "outputs" && !!baseBuilding.sourceProductionId && hasRate;
    const includeZeroRateInput =
      sectionType === "inputs" && !!baseBuilding.linkedOutput && hasRate;
    if (
      !itemId ||
      (!(hasRate && ratePerMinute > 0) &&
        !includeZeroRateOutput &&
        !includeZeroRateInput)
    )
      continue;
    ...
    const linkedResolution = baseBuilding.linkedOutput
    ...
    items.push({
      ...
      linkedOutput: baseBuilding.linkedOutput
        ? {
            ...
            outputName:
              linkedResolution?.sourceOutput?.name ||
              sourceBuilding?.name ||
...
function calculateBaseDetailStats(
  ...
  if (
    base.energyGroupId &&
    energyGroupsById?.[base.energyGroupId] &&
    allBases
  ) {
    pooledGeneration = 0;
    pooledConsumption = 0;
    for (const groupBase of allBases) {
      ...
      for (const baseBuilding of groupBase.buildings) {
        ...
        if (building.type === "generator")
          pooledGeneration += building.power || 0;
  ...
  const baseCoreHeatCapacity = calculateBaseCoreHeatCapacity(
    base.coreLevel ?? DEFAULT_BASE_CORE_LEVEL,
    base.buildings,
    buildingsById,
  );
  ...
  return {
    ...
    isEnergyInsufficient:
      localConsumption > 0 &&
      (energyGeneration === 0 || localConsumption > energyGeneration),
    energyGroupId: base.energyGroupId,
    energyGroupName:
      base.energyGroupId && energyGroupsById
        ? energyGroupsById[base.energyGroupId]?.name
        : undefined,
  };
}
...
function comparePlanSummaryRows(
  left: PlanSummaryRow,
  right: PlanSummaryRow,
): number {
...
export const registerBasesDerivedSubscriptions: UkladModule<
...
> = (registrar) => {
  registrar.regSub(
    ...
    ([base, bases, buildings, itemsById, plans, materials], ..._params) => (
      void _params,
      buildBaseProductionTable(
        base,
        bases,
        buildings,
        itemsById,
  ...
  registrar.regSub(
    ...
    ([base, buildingsById, groupsById, bases], ..._params) => (
      void _params,
      base
        ? calculateBaseDetailStats(base, buildingsById, groupsById, bases)
        : null
    ),
  );
  ...
  registrar.regSub(
    ...
    () => [[appIds.subscriptions.BUILDINGS_BY_ID_MAP]],
    ([buildingsById], ..._params) => (
      void _params,
      getCoreLevels(buildingsById)
    ),
  );
  ...
  registrar.regSub(
    ...
    ([bases, buildingsById, itemsById], baseId) =>
      buildBaseLogisticsViewModel({
  ...
  registrar.regSub(
    ...
    ([bases, buildingsById, itemsById], ..._params) => (
      void _params,
      buildAllBaseLogisticsViewModels({ bases, buildingsById, itemsById })
    ),
  );
  ...
  registrar.regSub(
    ...
    ([bases, buildingsById, groupsById], ..._params) => (
      void _params,
      Object.fromEntries(
        bases.map((base) => [
          base.id,
  ...
  registrar.regSub(
    ...
    ([base, buildingsById], _baseId, sectionType) => {
      if (!base) return [];
      const sectionBuildings = base.buildings.filter(
        (building) => building.sectionType === sectionType,
      );
      ...
      const addPlanName = (buildingId: string, planName: string) => {
        ...
        const names =
          planNamesByBuildingId.get(buildingId) || new Set<string>();
        names.add(planName);
        planNamesByBuildingId.set(buildingId, names);
      };
      buildActivePlanOccupancy(base).assignedPlanBuildingIds.forEach(
        (buildingIds, planId) => {
      ...
      const baseBuildingIds = new Set(
        base.buildings.map((building) => building.id),
      );
      for (const plan of base.productions.filter(
        (candidate) => candidate.active,
      )) {
        for (const input of plan.inputs || []) {
          if (input.id && baseBuildingIds.has(input.id))
            addPlanName(input.id, plan.name);
        }
      }
      const entries = sectionBuildings.flatMap(
        (baseBuilding): BuildingSectionBuilding[] => {
          const building = buildingsById[baseBuilding.buildingTypeId];
          return building
            ? [
                {
                  ...
                  count: 1,
                  isGrouped: false,
                  activePlanNames: Array.from(
                    planNamesByBuildingId.get(baseBuilding.id) || [],
                  ),
                },
              ]
            : [];
      ...
      const grouped = new Map<
        string,
        Omit<BuildingSectionBuilding, "baseBuilding">
      >();
      for (const entry of entries) {
        const existing = grouped.get(entry.buildingTypeId);
        if (existing) {
          existing.count += 1;
          existing.activePlanNames = Array.from(
            new Set([...existing.activePlanNames, ...entry.activePlanNames]),
          );
        } else {
  ...
  registrar.regSub(
    ...
    ([basesById, buildingsById], baseId, sectionType) => {
      const baseBuildings =
        basesById[baseId]?.buildings.filter(
          (building) => building.sectionType === sectionType,
        ) || [];
      const stats: BuildingSectionStats = {
      ...
      for (const baseBuilding of baseBuildings) {
        ...
        if (!isAmplifierBuilding(building.id))
          stats.totalHeat += building.heat || 0;
        if (building.type === "generator") {
  ...
  registrar.regSub(
    ...
    ([buildings], sectionType) =>
      getAvailableBuildingsForSection(buildings, sectionType),
  );
  ...
  registrar.regSub(
    ...
    ([bases, buildingsById], ..._params) => {
      ...
      const summary: MyBasesStats = {
        ...
        heatPercentage: 0,
        energyPercentage: 0,
        isHeatOverCapacity: false,
        isEnergyInsufficient: false,
      };
      for (const base of bases) {
        summary.totalBuildings += base.buildings.length;
        summary.totalPlans += base.productions.length;
        summary.totalHeatCapacity += calculateBaseCoreHeatCapacity(
          base.coreLevel ?? DEFAULT_BASE_CORE_LEVEL,
          base.buildings,
          buildingsById,
        );
        for (const baseBuilding of base.buildings) {
          ...
          if (!building) continue;
          if (!isAmplifierBuilding(building.id))
            summary.totalHeat += building.heat || 0;
          if (building.type === "generator")
            summary.totalEnergyProduced += building.power || 0;
          else summary.totalEnergyUsed += building.power || 0;
        }
      }
      summary.heatPercentage =
        summary.totalHeatCapacity > 0
          ? Math.min((summary.totalHeat / summary.totalHeatCapacity) * 100, 100)
          : 0;
      summary.energyPercentage =
        summary.totalEnergyProduced > 0
          ? Math.min(
              (summary.totalEnergyUsed / summary.totalEnergyProduced) * 100,
              100,
            )
          : summary.totalEnergyUsed > 0
            ? 100
            : 0;
      summary.isHeatOverCapacity =
        summary.totalHeat > summary.totalHeatCapacity;
      summary.isEnergyInsufficient =
        summary.totalEnergyUsed > 0 &&
        (summary.totalEnergyProduced === 0 ||
          summary.totalEnergyUsed > summary.totalEnergyProduced);
      return summary;
    },
  );
    ...
    ([base, corporations, itemsById], ..._params) => {
      ...
      return base.productions
        .map((plan) => ({
          ...
          requiredBuildingCount: (plan.requiredBuildings || []).reduce(
            (sum, building) => sum + building.count,
            0,
          ),
          inputCount: (plan.inputs || []).length,
          corporationLabel: plan.corporationLevel
            ? `${names.get(plan.corporationLevel.corporationId) || "Corporation"} Lv.${plan.corporationLevel.level}`
            : "None",
        }))
  ...
  registrar.regSub(
    ...
    ([base, bases, buildings, itemsById, plans], ..._params) => {
      ...
      for (const input of base.buildings) {
        ...
        if (resolved.linkedOutput && resolved.linkedOutputStatus !== "ok")
          continue;
        if (
          resolved.selectedItemId &&
          resolved.ratePerMinute &&
          resolved.ratePerMinute > 0
        ) {
          coverage.set(
            resolved.selectedItemId,
            (coverage.get(resolved.selectedItemId) || 0) +
              resolved.ratePerMinute,
          );
        }
      }
      ...
      const add = (planId: string, itemId: string, required: number) => {
        ...
        if (existing) {
          existing.perPlan[planId] = (existing.perPlan[planId] || 0) + required;
      ...
      for (const plan of base.productions) {
        ...
        const flow = buildProductionFlow(
          {
            ...
            targetAmount: plan.targetAmount,
            inputBuildings: getFlowInputBuildings(plan.inputs || [], bases),
            rawProductionDisabled: true,
            includeLauncher:
              plan.corporationLevel !== null &&
              plan.corporationLevel !== undefined,
            recipeSelections: plan.recipeSelections || {},
          },
          buildings,
        );
        for (const deficit of flow.rawMaterialDeficits || []) {
          rawKeys.add(`${plan.id}:${deficit.itemId}`);
          add(plan.id, deficit.itemId, deficit.required);
        }
        ...
        for (const node of flow.nodes) {
          if (
            ...
            continue;
          inputRequirements.set(
            node.outputItem,
            (inputRequirements.get(node.outputItem) || 0) +
              node.buildingCount * node.outputAmount,
          );
        }
        for (const [itemId, required] of inputRequirements)
          if (required > 0 && !rawKeys.has(`${plan.id}:${itemId}`))
            add(plan.id, itemId, required);
      }
      for (const [itemId, available] of coverage)
        if (!rows.has(itemId))
          rows.set(itemId, {
            itemId,
            item: itemsById[itemId] || {
      ...
      return Array.from(rows.values())
        .map((row) => {
          ...
          return {
            ...row,
            planDemands: getPlanDemands(row.perPlan, plans),
            available,
            covered: Math.min(row.totalRequired, available),
            missing: Math.max(0, row.totalRequired - available),
          };
        })
        .sort(
          (left, right) =>
            right.missing - left.missing ||
            right.totalRequired - left.totalRequired ||
  ...
  registrar.regSub(
    ...
    ([base, buildings, plans], ..._params) => {
      ...
      for (const building of base.buildings)
        if (building.sectionType === "production")
          owned.set(
            building.buildingTypeId,
            (owned.get(building.buildingTypeId) || 0) + 1,
          );
      const byId = new Map(
      ...
      for (const plan of base.productions)
        for (const requirement of plan.requiredBuildings || []) {
          ...
          if (existing) {
            existing.perPlan[plan.id] = requirement.count;
            existing.totalRequired += requirement.count;
          } else
            rows.set(requirement.buildingId, {
              buildingId: requirement.buildingId,
              building,
              perPlan: { [plan.id]: requirement.count },
      ...
      return Array.from(rows.values())
        .map((row) => {
          ...
          return {
            ...
            planDemands: getPlanDemands(row.perPlan, plans),
            owned: count,
            covered: Math.min(row.totalRequired, count),
            missing: Math.max(0, row.totalRequired - count),
          };
        })
        .sort(
          (left, right) =>
