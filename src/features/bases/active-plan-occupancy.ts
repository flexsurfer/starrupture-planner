...
function buildPrioritizedBuildingsByType(
  base: Base,
): Map<string, BaseBuilding[]> {
...
export function buildActivePlanOccupancy(
  ...
  const baseBuildingsById = new Map<string, BaseBuilding>(
    base.buildings.map((b) => [b.id, b]),
  );
  const availableBuildingsByType = buildPrioritizedBuildingsByType(base);

  const activePlans = base.productions.filter(
    (plan) => plan.active && plan.id !== options.excludePlanId,
  );

  const assignUniqueBuilding = (
    planId: string,
    buildingId: string,
  ): boolean => {
    ...
    const buildingTypeCount =
      occupiedBuildingTypeCounts.get(baseBuilding.buildingTypeId) || 0;
    occupiedBuildingTypeCounts.set(
