...
function createBuilding(
  partial: Partial<Building> & Pick<Building, "id" | "name">,
): Building {
  return {
    recipes: [],
    ...partial,
...
describe("buildingSectionUtils", () => {
  it("allows count controls only for buildings available in energy or production", () => {
    ...
    expect(
      isBuildingCountAvailable(
        createBuilding({ id: "heater", name: "Heater", type: "temperature" }),
      ),
    ).toBe(true);
    expect(
      isBuildingCountAvailable(
    ...
    expect(
      isBuildingCountAvailable(
        createBuilding({ id: "storage", name: "Storage", type: "storage" }),
      ),
    ).toBe(true);
    expect(
      isBuildingCountAvailable(
        createBuilding({
          ...
          type: "production",
          recipes: [
            { inputs: [], output: { id: "ore", amount_per_minute: 60 } },
          ],
        }),
      ),
    ).toBe(false);
    ...
    expect(
      isBuildingCountAvailable(
        createBuilding({ id: "habitat", name: "Habitat", type: "habitat" }),
      ),
    ).toBe(false);
  });
});
