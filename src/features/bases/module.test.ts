...
describe("bases Uklad module", () => {
  ...
  it("owns base selection, editing, and collapsed-card state", () => {
    ...
    const [base] = harness.getSubscriptionValue([
    ...
    expect(
      harness.getSubscriptionValue([appIds.subscriptions.BASES_SELECTED_BASE]),
    ...
    ).toMatchObject({ name: "Outpost One" });
  ...
  it("owns bulk additions, section count reconciliation, and removal", () => {
    ...
    harness.restoreState({
      ...
      basesList: [
        { id: "base-1", name: "Outpost", buildings: [], productions: [] },
      ],
