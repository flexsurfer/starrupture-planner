...
describe("buildProductionFlow", () => {
  ...
  describe("Simple Linear Chain", () => {
    it("should build a simple chain from raw material to processed material", () => {
      ...
      const oreNode = result.nodes.find(
        (n) => n.buildingId === "ore_excavator",
      );
    ...
    it("should handle fractional building counts correctly", () => {
      ...
      const launcherNode = result.nodes.find(
        (n) => n.buildingId === "orbital_cargo_launcher",
      );
  ...
  describe("Alternative Recipe Selection", () => {
    ...
    it("uses the slow-rate recipe by default when alternatives exist", () => {
      ...
      const slowNode = result.nodes.find(
        (n) => n.buildingId === "fabricator" && n.outputItem === "rotor",
      );
      const fastNode = result.nodes.find(
        (n) => n.buildingId === "craftertier2" && n.outputItem === "rotor",
      );
    ...
    it("identifies upgraded and alternative recipes in the same building", () => {
      ...
      expect(
        defaultResult.nodes.find((node) => node.outputItem === "tube")
          ?.recipeIndex,
      ).toBe(1);
      ...
      const alternativeNode = alternativeResult.nodes.find(
        (node) => node.outputItem === "tube",
      );
  ...
  describe("Complex Multi-Input Chain", () => {
    it("should build a complex chain with multiple inputs", () => {
      ...
      const nodesByBuilding = result.nodes.reduce(
        (acc, node) => {
    ...
    it("should create separate nodes for different recipes of the same building", () => {
      ...
      const fabricatorNodes = result.nodes.filter(
        (n) => n.buildingId === "fabricator",
      );
      expect(fabricatorNodes).toHaveLength(2);

      const beamNode = fabricatorNodes.find(
        (n) => n.outputItem === "titanium_beam",
      );
      const sheetNode = fabricatorNodes.find(
        (n) => n.outputItem === "titanium_sheet",
      );
  ...
  describe("Multiple Consumers (Shared Production)", () => {
    it("should correctly handle multiple consumers of the same item", () => {
      ...
      const smelterNodes = result.nodes.filter(
        (n) => n.buildingId === "smelter",
      );
      ...
      const oreNodes = result.nodes.filter(
        (n) => n.buildingId === "ore_excavator",
      );
    ...
    it("should consolidate edges between the same producer and consumer", () => {
      ...
      const edgesFromSmelter = result.edges.filter(
        (e) => e.from === smelterNodeId,
      );

      // Should have exactly 2 edges: one to beam fabricator, one to sheet fabricator
      expect(edgesFromSmelter).toHaveLength(2);

      const beamEdge = edgesFromSmelter.find((e) =>
        e.to.includes("titanium_beam"),
      );
      const sheetEdge = edgesFromSmelter.find((e) =>
        e.to.includes("titanium_sheet"),
      );
  ...
  describe("Raw Material Handling", () => {
    ...
    it("reports raw material deficits when raw production is disabled and inputs are insufficient", () => {
      ...
      expect(
        result.nodes.find(
          (n) => n.outputItem === "ore_titanium" && n.nodeType === "production",
        ),
      ).toBeUndefined();
      expect(result.rawMaterialDeficits).toBeDefined();
  ...
  describe("Custom Inputs and Raw Deficits", () => {
    ...
    it("uses input building snapshots as external sources without custom input conversion", () => {
      ...
      const customInputNode = result.nodes.find(
        (n) => n.nodeType === "input" && n.outputItem === "ore_titanium",
      );
      ...
      const edgeFromCustom = result.edges.find(
        (e) =>
          e.from.includes("input_snapshot_1") && e.to.includes("bar_titanium"),
      );
    ...
    it("uses input buildings for raw materials even when rawProductionDisabled is false", () => {
      ...
      const edgeFromCustom = result.edges.find(
        (e) => e.from.includes("input_1") && e.to.includes("bar_titanium"),
      );
      expect(edgeFromCustom).toBeDefined();
      expect(edgeFromCustom!.amount).toBe(90);
    ...
    it("ignores input buildings for the target item and still builds production", () => {
      ...
      const customInputNode = result.nodes.find(
        (n) => n.nodeType === "input" && n.outputItem === "bar_titanium",
      );
    ...
    it("propagates intermediate custom-input allocation to upstream demand", () => {
      ...
      const beamNode = result.nodes.find(
        (n) => n.nodeType === "production" && n.outputItem === "titanium_beam",
      );
      expect(beamNode).toBeUndefined();

      // Smelter should only produce bars needed for sheets (30/min), not full theoretical 90/min.
      const smelterNode = result.nodes.find(
        (n) => n.outputItem === "bar_titanium" && n.nodeType === "production",
      );
      expect(smelterNode).toBeDefined();
      expect(smelterNode!.buildingCount).toBe(0.5); // 30/60

      const edgesFromSmelter = result.edges.filter(
        (e) => e.from === "smelter_0_bar_titanium",
      );
      const totalFromSmelter = edgesFromSmelter.reduce(
        (sum, e) => sum + e.amount,
        0,
      );
    ...
    it("does not count allocations to pruned consumers in custom-input utilization", () => {
      ...
      const sheetProducer = result.nodes.find(
        (n) => n.nodeType === "production" && n.outputItem === "titanium_sheet",
      );
      expect(sheetProducer).toBeUndefined();

      // Bar input should only be used for beam branch (60/min), not pruned sheet branch.
      const customBarNode = result.nodes.find(
        (n) => n.nodeType === "input" && n.baseBuildingId === "bar_input",
      );
    ...
    it("allocates shared custom inputs to remaining live consumers after branch pruning", () => {
      ...
      const stabilizerProducer = result.nodes.find(
        (n) => n.nodeType === "production" && n.outputItem === "stabilizer",
      );
      expect(stabilizerProducer).toBeUndefined();

      // Rod custom input must still feed impeller directly, so no rod producer is required.
      const rodProducer = result.nodes.find(
        (n) => n.nodeType === "production" && n.outputItem === "titanium_rod",
      );
      expect(rodProducer).toBeUndefined();

      const rodEdgeToImpeller = result.edges.find(
        (e) =>
          e.from.includes("rod_input") &&
          e.to.includes("impeller") &&
          e.itemId === "titanium_rod",
      );
    ...
    it("should show input node when it is actually consumed by another node", () => {
      ...
      const fabricatorNode = result.nodes.find(
        (n) =>
          n.buildingId === "fabricator" && n.outputItem === "titanium_beam",
      );
  ...
  describe("Deterministic Input Allocation", () => {
    it("should allocate input buildings deterministically in source order", () => {
      ...
      const sourceA = customNodesR1.find(
        (n) => n.baseBuildingId === "source_a",
      );
      const sourceB = customNodesR1.find(
        (n) => n.baseBuildingId === "source_b",
      );
    ...
    it("uses earlier sources first based on actual edge allocation amounts", () => {
      ...
      const fromA = result.edges
        .filter(
          (e) => e.from.includes("source_a") && e.itemId === "bar_titanium",
        )
        .reduce((sum, e) => sum + e.amount, 0);
      const fromB = result.edges
        .filter(
          (e) => e.from.includes("source_b") && e.itemId === "bar_titanium",
        )
        .reduce((sum, e) => sum + e.amount, 0);
    ...
    it("should not double-count input supply across multiple consumers", () => {
      ...
      const edgesFromCustom = result.edges.filter((e) =>
        e.from.includes("source_1"),
      );
      const totalFromCustom = edgesFromCustom.reduce(
        (sum, e) => sum + e.amount,
        0,
      );
    ...
    it("should handle partial input satisfaction correctly", () => {
      ...
      const edgesToFabricator = result.edges.filter(
        (e) => e.to === fabricatorNodeId,
      );

      expect(edgesToFabricator.length).toBe(2); // One from input, one from smelter

      const totalToFabricator = edgesToFabricator.reduce(
        (sum, e) => sum + e.amount,
        0,
      );
  ...
  describe("Flow Conservation Invariants", () => {
    ...
    it("satisfies each consumer input amount from incoming edges", () => {
      ...
      const recipeByOutput = new Map<
        string,
        { inputs: Array<{ id: string; amount_per_minute: number }> }
      >();
      buildings.forEach((b) => {
        (b.recipes || []).forEach((r) =>
          recipeByOutput.set(r.output.id, { inputs: r.inputs }),
        );
      });
  ...
  describe("Pure Demand Calculation", () => {
    ...
    it("should produce identical results for identical inputs (determinism)", () => {
      ...
      for (let i = 0; i < result1.nodes.length; i++) {
        expect(result1.nodes[i].buildingCount).toBe(
          result2.nodes[i].buildingCount,
        );
        expect(result1.nodes[i].buildingCount).toBe(
          result3.nodes[i].buildingCount,
        );
      }
    });
  });
  ...
  describe("Node ID Generation", () => {
    it("should generate consistent and unique node IDs", () => {
      ...
      const nodeIds = result.nodes.map(
        (node) => `${node.buildingId}_${node.recipeIndex}_${node.outputItem}`,
      );
