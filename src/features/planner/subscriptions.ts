...
import {
  buildRecipeOptionsForOutputItems,
  collectRecipeOutputItems,
} from "./recipe-options";
import { getMultiTargetWarning } from "./target-conflicts";
import { getItemName } from "@/utils/itemUtils";

export const registerPlannerSubscriptions: UkladModule<
  UkladRegistrar<AppContracts>
> = (registrar) => {
  registrar.regRootSub(
    appIds.subscriptions.PLANNER_TABS,
    stateKeys.plannerTabs,
  );
  registrar.regRootSub(
    appIds.subscriptions.PLANNER_ACTIVE_TAB_ID,
    stateKeys.plannerActiveTabId,
  );
  registrar.regRootSub(
    appIds.subscriptions.PLANNER_TAB_CREATION,
    stateKeys.plannerTabCreation,
  );
  registrar.regSub(
    ...
    ([plannerTabs, plannerActiveTabId]) =>
      getActivePlannerTab({ plannerTabs, plannerActiveTabId }),
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_ACTIVE_VIEW,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.activeView ?? "graph",
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_GROUP_BY_STAGE,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.groupByStage ?? false,
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_FLOW_DIRECTION,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.flowDirection ?? "LR",
  );
  registrar.regSub(
  ...
  registrar.regSub(
    ...
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.multiTargets ?? [],
  );
  registrar.regRootSub(
    appIds.subscriptions.PLANNER_TARGET_WARNING,
    stateKeys.plannerTargetWarning,
  );
  registrar.regSub(
  ...
  registrar.regSub(
    appIds.subscriptions.PLANNER_SELECTED_ITEM_ID,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.selectedItemId ?? null,
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_SELECTED_CORPORATION_LEVEL,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.selectedCorporationLevel ?? null,
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_SINGLE_RECIPE_SELECTIONS,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => (tab?.mode === "single" ? tab.recipeSelections : {}),
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_MULTI_RECIPE_SELECTIONS,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => (tab?.mode === "multi" ? tab.recipeSelections : {}),
  );
  registrar.regSub(
  ...
  registrar.regRootSub(
    ...
    stateKeys.pinnedRecipeSelections,
  );
  registrar.regRootSub(
    appIds.subscriptions.RECIPE_ALTERNATIVE_PRESETS,
    stateKeys.recipeAlternativePresets,
  );
  registrar.regSub(
    appIds.subscriptions.PLANNER_TARGET_AMOUNT,
    () => [[appIds.subscriptions.PLANNER_ACTIVE_TAB]],
    ([tab]) => tab?.targetAmount ?? 60,
  );
  ...
  registrar.regSub(
    ...
    (
      ...
      const outputItems =
        mode === "multi" && productionFlow.nodes.length === 0
  ...
  registrar.regSub(
    ...
    (
      [productionFlow, items, targetIds, direction, groupByStage],
      ..._params
    ) => {
      void _params;
  ...
  registrar.regSub(
    ...
    ([selectedItem, productionFlow], ..._params) => {
      ...
      const buildingNodes = productionFlow.nodes.filter(
        (node) => node.nodeType !== "target",
      );
      return {
        totalBuildings: buildingNodes.reduce(
          (sum, node) => sum + Math.ceil(node.buildingCount),
          0,
        ),
        totalEnergy: buildingNodes.reduce(
          (sum, node) => sum + node.totalPower,
          0,
        ),
        totalHotness: buildingNodes.reduce(
  ...
  registrar.regSub(
    ...
    ([productionFlow, items, selectedItemId], ..._params) => {
      ...
      for (const edge of productionFlow.edges) {
        requiredRates.set(
          edge.itemId,
          (requiredRates.get(edge.itemId) ?? 0) + edge.amount,
        );
      }
      ...
      for (const node of buildingNodes) {
        ...
        outputRates.set(
          node.outputItem,
          (outputRates.get(node.outputItem) ?? 0) +
      ...
      for (const itemId of itemIds) {
        ...
        itemsByType.get(type)!.push({
          ...
          type,
          requiredRate:
            requiredRates.get(itemId) ?? outputRates.get(itemId) ?? 0,
        });
      }
      itemsByType.forEach((entries) =>
      ...
      return {
        productionGroups: ["launcher", "target", ...[...sortedTypes].reverse()]
          .map((type) => ({
            type,
            nodes: productionFlow.nodes
              .filter((node) => {
                if (node.nodeType === "launcher") return type === "launcher";
                if (targetNodeIds.has(getFlowNodeId(node)))
                  return type === "target";
                return (
                  type ===
                  (items.find((item) => item.id === node.outputItem)?.type ||
                    "unknown")
                );
              })
              .sort(
                (a, b) =>
                  getItemName(a.outputItem, items).localeCompare(
                    getItemName(b.outputItem, items),
                  ) ||
        ...
        buildingStats: Array.from(buildingMap.values()).sort(
          (a, b) => b.count - a.count,
        ),
        totalEnergy: buildingNodes.reduce(
          (sum, node) => sum + node.totalPower,
