...
const nodeId = (
  ...
  suffix
    ? `${buildingId}_${recipeIndex}_${outputItem}_${suffix}`
    : `${buildingId}_${recipeIndex}_${outputItem}`;
...
interface BuilderContext {
  ...
  flows: Map<
    string,
    { from: string; to: string; itemId: string; amount: number }
  >;
...
export function createRecipeResolver(
  ...
  recipeOptionsCache.forEach((options) => {
    options.sort((a, b) => {
      const alternativeDiff =
        Number(a.recipe.variant === "alternative") -
        Number(b.recipe.variant === "alternative");
      if (alternativeDiff !== 0) return alternativeDiff;
      const rateDiff =
        a.recipe.output.amount_per_minute - b.recipe.output.amount_per_minute;
  ...
  const getRecipe = (itemId: string): RecipeInfo | null => {
    ...
    if (selectedKey) {
      const selected = options.find(({ building, recipe, recipeIndex }) =>
...
export function buildProductionFlow(
  params: ProductionFlowParams,
  buildings: Building[],
): ProductionFlowResult {
  return buildFlow(params, buildings, [
    { itemId: params.targetItemId, amount: params.targetAmount ?? 60 },
  ]);
}
...
export function buildMultiTargetProductionFlow(
  ...
  for (const producer of flow.nodes) {
    ...
    const recipe = buildings.find(
      (building) => building.id === producer.buildingId,
    )?.recipes?.[producer.recipeIndex];
    if (!recipe?.inputs.length) continue;
    const target: TargetFlowNode = {
      nodeType: "target",
      outputItem: producer.outputItem,
      amount,
    };
    targetNodes.push(target);
    targetEdges.push({
      from: getFlowNodeId(producer),
      to: getFlowNodeId(target),
      itemId: producer.outputItem,
      amount,
    });
  }
  return targetNodes.length
    ? {
        ...flow,
        nodes: [...flow.nodes, ...targetNodes],
        edges: [...flow.edges, ...targetEdges],
      }
    : flow;
}
...
function buildFlow(
  ...
  const sourceStateById = new Map(
    sources.map((s) => [s.baseBuildingId, { ...s, remaining: s.available }]),
  );
  ...
  const getOrCreateInputNode = (trackerId: string): FlowNode | null => {
    if (ctx.inputNodeBySource.has(trackerId))
      return ctx.inputNodeBySource.get(trackerId)!;
  ...
  const allocateInputToConsumer = (
    ...
    for (const sourceId of sourceIds) {
      ...
      ctx.usedByInputSource.set(
        sourceId,
        (ctx.usedByInputSource.get(sourceId) ?? 0) + amount,
      );
  ...
  const fulfillDemand = (
    ...
    if (raw) {
      ctx.rawRequiredByItem.set(
        itemId,
        (ctx.rawRequiredByItem.get(itemId) ?? 0) + requested,
      );
      ctx.rawAvailableByItem.set(
        itemId,
        (ctx.rawAvailableByItem.get(itemId) ?? 0) + allocatedFromInput,
      );
    ...
    const additionalBuildings =
      remaining / info.recipe.output.amount_per_minute;
    producerNode.buildingCount = round(
    ...
    const producerId = nodeId(
      producerNode.buildingId,
      producerNode.recipeIndex,
      producerNode.outputItem,
    );
    if (consumerId) {
  ...
  if (includeLauncher) {
    ...
    if (launcherBuilding) {
      ...
      if (targetNode) {
        flowEdges.push({
          from: nodeId(
            targetNode.buildingId,
            targetNode.recipeIndex,
            targetNode.outputItem,
          ),
          to: nodeId(
            launcherNode.buildingId,
            launcherNode.recipeIndex,
            launcherNode.outputItem,
          ),
          itemId: targetItemId,
          amount: targetAmount,
        });
