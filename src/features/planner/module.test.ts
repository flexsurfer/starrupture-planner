...
describe("planner Uklad module", () => {
  it("totals shared ingredient demand and scales item rates with the target", () => {
      ...
      buildingsList: [
        {
          ...
          recipes: [
            ...
            {
              output: { id: "product", amount_per_minute: 10 },
              inputs: [
    ...
    try {
      ...
      expect(
        harness
          .getSubscriptionValue([appIds.subscriptions.PLANNER_STATS_DETAILED])
          .productionGroups.map((group) => ({
            type: group.type,
            items: group.nodes.map((node) => node.outputItem),
          })),
      ).toEqual([
      ...
      harness.dispatchSync([
        ...
        { corporationId: "corp", level: 1 },
      ]);
      expect(
        harness
          .getSubscriptionValue([appIds.subscriptions.PLANNER_STATS_DETAILED])
          .productionGroups.map((group) => group.type),
  ...
  it("sets planner state and its default target rate through typed events", () => {
    ...
    ).toEqual([{ id: "iron-plate", name: "Iron Plate", type: "processed" }]);
