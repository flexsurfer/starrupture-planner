...
function validateTargets(
  ...
  tab: PlannerTab,
  ids: string[],
  selections = tab.recipeSelections,
): boolean {
  state.plannerTargetWarning = getMultiTargetWarning(
    ids,
    state.buildingsList,
    selections,
    state.itemsList,
...
export const registerPlannerEvents: UkladModule<
...
> = (registrar) => {
  ...
  registrar.regEvent(
    ...
    ({ draftState }, id, name, mode, view = "graph") => {
      const trimmedName = name.trim();
      if (
        !id.trim() ||
        !trimmedName ||
        !["single", "multi"].includes(mode) ||
        !["graph", "table"].includes(view) ||
        draftState.plannerTabs.some((tab) => tab.id === id)
      )
        return;
      ...
      if (request?.itemId) {
        if (mode === "single") {
          ...
          tab.selectedCorporationLevel = request.corporationLevel
            ? { ...request.corporationLevel }
            : null;
          tab.targetAmount = getSlowestOutputRateForItem(
            draftState.buildingsList,
            request.itemId,
          );
        } else {
          const info = createRecipeResolver(
            draftState.buildingsList,
            tab.recipeSelections,
          )(request.itemId);
          if (info?.recipe.inputs.length) {
            tab.multiTargets = [
              {
                itemId: request.itemId,
                amount: info.recipe.output.amount_per_minute,
              },
            ];
          }
        }
  ...
  registrar.regEvent(appIds.events.PLANNER_CLOSE_TAB, ({ draftState }, id) => {
    ...
    if (id === activeId) {
      draftState.plannerActiveTabId =
        draftState.plannerTabs[
          Math.min(index, draftState.plannerTabs.length - 1)
        ]?.id ?? null;
      draftState.plannerTargetWarning = null;
    }
  });
  registrar.regEvent(
    appIds.events.PLANNER_SET_ACTIVE_VIEW,
    ({ draftState }, view) => {
  ...
  registrar.regEvent(
    appIds.events.PLANNER_SET_GROUP_BY_STAGE,
    ({ draftState }, enabled) => {
      const tab = getActivePlannerTab(draftState);
  ...
  registrar.regEvent(
    ...
    ({ draftState }, direction) => {
      const tab = getActivePlannerTab(draftState);
      if (tab && ["LR", "RL", "TB", "BT"].includes(direction))
        tab.flowDirection = direction;
    },
  );
  registrar.regEvent(
  ...
  registrar.regEvent(
    ...
    ({ draftState }, itemId) => {
      ...
      if (
        !validateTargets(draftState, tab, [
          ...tab.multiTargets.map((t) => t.itemId),
          itemId,
        ])
      )
        return;
      tab.multiTargets.push({
        itemId,
        amount: info.recipe.output.amount_per_minute,
      });
    },
  ...
  registrar.regEvent(
    ...
    ({ draftState }, itemId, corporationLevel) => {
      draftState.plannerTabCreation = {
        itemId,
        ...(corporationLevel
  ...
  registrar.regEvent(
    ...
    ({ draftState }, corporationLevel) => {
      const tab = getActivePlannerTab(draftState);
      if (tab?.mode === "single")
        tab.selectedCorporationLevel = corporationLevel
          ? { ...corporationLevel }
          : null;
    },
  );
  ...
  registrar.regEvent(
    ...
    ({ draftState }, presetId) => {
      if (!presetId) return;
      draftState.recipeAlternativePresets =
        draftState.recipeAlternativePresets.filter(
  ...
  registrar.regEvent(
    appIds.events.PLANNER_SET_TARGET_AMOUNT,
    ({ draftState }, targetAmount) => {
      const tab = getActivePlannerTab(draftState);
      if (
        tab?.mode === "single" &&
