...
describe("planner production stage layout", () => {
  it("aligns raw materials, processed items, and targets despite different chain lengths", () => {
    const flow = buildMultiTargetProductionFlow(
      [
        { itemId: "short", amount: 60 },
        { itemId: "long", amount: 60 },
      ],
      buildings,
    );
    ...
    const positions = Object.fromEntries(
      graph.nodes.map((node) => [node.flowNode.outputItem, node.position]),
    );
    ...
    expect(
      Math.abs(positions.short.y - positions.long.y),
  ...
  it.each([
    ...
    "orients stages and connection handles for %s",
    (direction, axis, sign, sourceHandle, targetHandle) => {
      const flow = buildMultiTargetProductionFlow(
        [
          { itemId: "short", amount: 60 },
          { itemId: "long", amount: 60 },
      ...
      const targetNodes = graph.nodes.filter((node) =>
        ["short", "long"].includes(node.flowNode.outputItem),
      );
      expect(targetNodes.every((node) => node.type === "default")).toBe(true);
      expect(
        targetNodes.every(
          (node) => node.style?.borderColor === "var(--color-primary)",
        ),
      ).toBe(true);
      ...
      for (const edge of graph.edges) {
        ...
        const target = graph.nodes.find((node) => node.id === edge.target)!;
        expect(
          (target.position[axis] - source.position[axis]) * sign,
        ).toBeGreaterThan(0);
      }
    },
  );
  ...
  it.each(["LR", "RL", "TB", "BT"] as const)(
    "can use automatic layout in %s without changing production",
    (direction) => {
      const flow = buildMultiTargetProductionFlow(
        [
          { itemId: "short", amount: 60 },
      ...
      const grouped = buildPlannerFlowGraph(
        flow.nodes,
        flow.edges,
        items,
        ["short", "long"],
      ...
      const automatic = buildPlannerFlowGraph(
        flow.nodes,
        flow.edges,
        items,
        ["short", "long"],
      ...
      expect(automatic.nodes.map((node) => node.position)).not.toEqual(
        grouped.nodes.map((node) => node.position),
      );
  ...
  it.each(["LR", "RL", "TB", "BT"] as const)(
    "keeps nested producers before target endpoints in %s",
    (direction) => {
      const targets = [
        { itemId: "plate", amount: 15 },
        { itemId: "part", amount: 30 },
        { itemId: "long", amount: 60 },
      ];
      ...
      for (const groupByStage of [false, true]) {
        ...
        const specialTargets = graph.nodes.filter(
          (node) => node.flowNode.nodeType === "target",
        );
        expect(specialTargets.map((node) => node.flowNode.outputItem)).toEqual([
          "plate",
          "part",
        ]);
        expect(specialTargets.every((node) => node.type === "output")).toBe(
          true,
        );
        const terminalProducer = graph.nodes.find(
          (node) => node.flowNode.outputItem === "long",
        )!;
        ...
        expect(
          graph.nodes
            .filter(
              (node) =>
                node.flowNode.nodeType === "production" &&
                ["plate", "part"].includes(node.flowNode.outputItem),
            )
            .every((node) => !node.style?.borderColor),
        ).toBe(true);
        if (groupByStage)
          expect(
            new Set(endpoints.map((node) => node.position[axis])).size,
          ).toBe(1);
        expect(graph.edges).toHaveLength(flow.edges.length);
        for (const edge of graph.edges) {
          const source = graph.nodes.find((node) => node.id === edge.source)!;
  ...
  it("places delivery after the single target and supports empty flows", () => {
    const flow = buildProductionFlow(
      { targetItemId: "long", includeLauncher: true },
      buildings,
    );
    const graph = buildPlannerFlowGraph(
      flow.nodes,
    ...
    const target = graph.nodes.find(
      (node) =>
        node.flowNode.nodeType === "production" &&
        node.flowNode.outputItem === "long",
    )!;
    const launcher = graph.nodes.find(
      (node) => node.flowNode.nodeType === "launcher",
    )!;
