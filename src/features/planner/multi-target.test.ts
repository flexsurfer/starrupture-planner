...
import {
  buildMultiTargetProductionFlow,
  buildProductionFlow,
...
const buildings: Building[] = [
  {
    ...
    recipes: [
      ...
      {
        output: { id: "plate", amount_per_minute: 20 },
        inputs: [{ id: "ore", amount_per_minute: 30 }],
      },
      {
        output: { id: "a", amount_per_minute: 10 },
        inputs: [{ id: "plate", amount_per_minute: 10 }],
      },
      {
        output: { id: "b", amount_per_minute: 10 },
        inputs: [{ id: "plate", amount_per_minute: 10 }],
      },
      {
        variant: "alternative",
        output: { id: "b", amount_per_minute: 10 },
        inputs: [{ id: "a", amount_per_minute: 10 }],
      },
      {
        output: { id: "c", amount_per_minute: 10 },
        inputs: [{ id: "a", amount_per_minute: 10 }],
      },
    ],
  },
...
describe("multi-target planner tabs", () => {
  it.each(["a", "missing"])(
    "waits for game data before validating restored target %s",
    (itemId) => {
      ...
      try {
        ...
        if (itemId === "missing") {
          expect(warning()).toContain(
            "missing has no usable production recipe",
          );
          expect(
            harness.getSubscriptionValue([
  ...
  it("shares production capacity and rounds combined building counts", () => {
    ...
    const plate = flow.nodes
      .filter((n): n is FlowNode => n.nodeType === "production")
      .filter((n) => n.outputItem === "plate");
    expect(plate).toHaveLength(1);
    expect(plate[0].buildingCount).toBe(1);
    expect(plate[0].totalPower).toBe(10);
    expect(
      flow.edges.filter((e) => e.itemId === "plate").map((e) => e.amount),
    ).toEqual([10, 10]);
    expect(flow.nodes.some((n) => n.nodeType === "launcher")).toBe(false);
    expect(flow.nodes.some((n) => n.nodeType === "target")).toBe(false);
    const single = buildProductionFlow(
      { targetItemId: "a", targetAmount: 10 },
      buildings,
    );
    const multi = buildMultiTargetProductionFlow(
      [{ itemId: "a", amount: 10 }],
      buildings,
    );
    ...
    expect(
      buildMultiTargetProductionFlow([{ itemId: "a", amount: NaN }], buildings)
        .nodes,
    ).toEqual([]);
  });

  it.each([false, true])(
    "fulfills nested target demand independently of target order (reversed: %s)",
    (reversed) => {
      const targets = [
        { itemId: "plate", amount: 5 },
        { itemId: "a", amount: 10 },
        { itemId: "c", amount: 10 },
      ];
      const flow = buildMultiTargetProductionFlow(
      ...
      const producers = flow.nodes.filter(
        (node): node is FlowNode => node.nodeType === "production",
      );
      expect(
        Object.fromEntries(
          producers.map((node) => [
      ...
      expect(
        producers.reduce((sum, node) => sum + Math.ceil(node.buildingCount), 0),
      ).toBe(6);
      expect(producers.reduce((sum, node) => sum + node.totalPower, 0)).toBe(
        60,
      );
      for (const target of targets.filter((target) => target.itemId !== "c")) {
        const id = `target:${target.itemId}`;
        expect(flow.nodes.find((node) => getFlowNodeId(node) === id)).toEqual({
          nodeType: "target",
          outputItem: target.itemId,
          amount: target.amount,
        });
        expect(flow.edges.filter((edge) => edge.to === id)).toHaveLength(1);
        expect(flow.edges.find((edge) => edge.to === id)).toMatchObject({
      ...
      expect(
        flow.nodes.some((node) => getFlowNodeId(node) === "target:c"),
      ).toBe(false);
      ...
      for (const producer of producers) {
        const outgoing = flow.edges.filter(
          (edge) => edge.from === getFlowNodeId(producer),
        );
        ...
        expect(
          outgoing.reduce((sum, edge) => sum + edge.amount, 0),
  ...
  it("adds and removes only the nested endpoint when recipes or targets change", () => {
    ...
    expect(independent.nodes.some((node) => node.nodeType === "target")).toBe(
      false,
    );
    const nested = buildMultiTargetProductionFlow(targets, buildings, {
      b: "factory:4",
    });
    expect(nested.nodes.filter((node) => node.nodeType === "target")).toEqual([
      { nodeType: "target", outputItem: "a", amount: 10 },
    ]);
    expect(buildMultiTargetProductionFlow(targets, buildings, {})).toEqual(
      independent,
    );
    expect(
      buildMultiTargetProductionFlow([targets[0]], buildings, {
        b: "factory:4",
      }),
    ).toEqual(
      buildProductionFlow({ targetItemId: "a", targetAmount: 10 }, buildings),
    );
  });
  ...
  it("allows dependencies in both directions and through selected alternatives", () => {
    ...
    expect(
      getMultiTargetWarning(["a", "b"], buildings, { b: "factory:4" }, []),
    ).toBeNull();
  });
  ...
  it("does not add demand for invalid or raw targets already used as ingredients", () => {
    ...
    expect(flow).toEqual(
      buildProductionFlow({ targetItemId: "c", targetAmount: 10 }, buildings),
    );
  });
  ...
  it("warns about actual recipe cycles, including cycles below a target", () => {
    ...
    cyclicBuildings[0].recipes!.push({
      variant: "alternative",
      output: { id: "a", amount_per_minute: 10 },
      inputs: [{ id: "b", amount_per_minute: 10 }],
    });
    const selections = { a: "factory:6", b: "factory:4" };
    expect(
      getMultiTargetWarning(["a", "b"], cyclicBuildings, selections, []),
    ).toContain("circular production dependency");
    expect(
      getMultiTargetWarning(["c"], cyclicBuildings, selections, []),
    ).toContain("circular production dependency");
    expect(
      getMultiTargetWarning(["c"], cyclicBuildings, { a: "factory:6" }, []),
    ).toBeNull();
  });
  ...
  it("accepts overlapping additions and recipe changes while preserving tabs and updating stats", () => {
    ...
    harness.restoreState({
      ...
      itemsList: ["ore", "plate", "a", "b", "c"].map((id) => ({
        id,
        name: id.toUpperCase(),
    ...
    try {
      ...
      const rates = () =>
        Object.fromEntries(
          [
            ...
            .flat()
            .map((item) => [item.id, item.requiredRate]),
        );
      expect(rates()).toEqual({ ore: 30, plate: 20, a: 10, b: 10 });
      expect(
        harness.getSubscriptionValue([
          appIds.subscriptions.PLANNER_STATS_DETAILED,
        ]).productionGroups[0].nodes,
      ).toHaveLength(2);
      ...
      const stats = harness.getSubscriptionValue([
        appIds.subscriptions.PLANNER_STATS_DETAILED,
      ]);
      expect(stats.productionGroups[0].nodes).toHaveLength(4);
      expect(
        Object.fromEntries(
          stats.productionGroups[0].nodes.map((node) => [
            node.outputItem,
            node.nodeType,
          ]),
        ),
      ...
      expect(
        stats.productionGroups.slice(1).flatMap((group) => group.nodes),
      ).toHaveLength(3);
      expect(stats.buildingStats).toEqual([
        {
          buildingId: "factory",
      ...
      harness.dispatchSync([appIds.events.PLANNER_ADD_TARGET, "a"]);
      expect(harness.getState().plannerTargetWarning).toContain(
        "already a target",
      );
      harness.dispatchSync([
      ...
      const flow = harness.getSubscriptionValue([
        appIds.subscriptions.PLANNER_PRODUCTION_FLOW,
      ]);
      expect(
        flow.nodes.some(
          (node) => node.nodeType === "target" && node.outputItem === "a",
        ),
      ).toBe(false);
      expect(
        flow.nodes.some(
          (node) => node.nodeType === "production" && node.outputItem === "a",
        ),
      ).toBe(true);
