...
export const PlannerProductionTable = () => {
  ...
  if (!stats.productionGroups.length)
    return (
    ...
    <div className="h-full overflow-auto p-2 sm:p-3">
      <table className="w-full table-fixed border border-base-300">
        <caption className="sr-only">
          Production cards ordered by target and item category
        </caption>
        {stats.productionGroups.map((group) => (
          ...
          >
            <tr>
              <th
                scope="rowgroup"
                className="px-2 py-1 text-left text-xs font-semibold bg-base-200 sm:px-4 sm:py-2 sm:text-sm"
              >
                {group.type === "target"
                  ? "Targets"
                  : group.type === "launcher"
                  ...
                  {group.nodes.map((node) => (
                    ...
                    >
                      <NodeCard
                        ...
                        node={node}
                        items={items}
                        outputColor={getItemColor(node.outputItem, items)}
                      />
                    </div>
                  ))}
                </div>
