...
export function DiagramSettings() {
    ...
    return <div className="absolute right-2 top-2 z-20 flex flex-col items-end gap-2"
        ...
        <button type="button" className="btn btn-sm btn-square border-base-300 bg-base-200 shadow"
            aria-label={t("Diagram settings")} title={t("Diagram settings")} aria-expanded={open} aria-controls={panelId}
        ...
        {open && <div id={panelId} role="group" aria-label={t("Diagram options")}
            className="flex flex-col gap-3 rounded-lg border border-base-300 bg-base-200 p-3 text-sm shadow-lg">
            <label className="flex items-center justify-between gap-4">{t("Direction")}<select className="select select-sm select-bordered w-auto" aria-label={t("Diagram direction")}
                    value={direction}
                    onChange={event => {
                    ...
                    <option value="LR">{t("Left → right")}</option>
                    <option value="RL">{t("Right → left")}</option>
                    <option value="TB">{t("Top → bottom")}</option>
                    <option value="BT">{t("Bottom → top")}</option>
                </select>
            </label>
            <label className="flex cursor-pointer items-center justify-between gap-4">{t("Group by production stage")}<input type="checkbox" className="toggle toggle-sm" checked={groupByStage}
