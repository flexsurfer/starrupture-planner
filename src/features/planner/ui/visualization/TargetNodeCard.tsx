...
export function TargetNodeCard({ node, items, outputColor, compactOnMobile = false }: {
    ...
    return <div className={`relative text-center ${compactOnMobile ? 'p-1.5 pt-4 space-y-1 sm:p-2 sm:pt-4 sm:space-y-2' : 'p-2 pt-4 space-y-2'}`}>
        <div className="absolute -top-2 left-1/2 -translate-x-1/2 rounded border border-primary/40 bg-base-200 px-1.5 text-[10px] leading-4 text-primary">{t("Target")}</div>
        ...
        <div className="flex items-center justify-center gap-1.5">
            ...
            <div className={`min-w-0 font-semibold leading-tight break-words tabular-nums ${compactOnMobile ? 'text-lg sm:text-xl' : 'text-xl'}`} style={{ color: outputColor }} aria-label={t("Target amount per minute")}>
                {node.amount.toFixed(1)}<span className="block text-xs font-normal">{t("/min")}</span>
            </div>
        </div>
    </div>;
}
