...
vi.mock("@/app/uklad/bindings", () => ({
  useRuntime: () => ({ dispatch }),
  useSubscription: vi.fn(([id]: [string]) =>
    id === appIds.subscriptions.ITEMS_RECIPES_BY_OUTPUT_ITEM_ID
      ? recipes
...
describe("NodeRecipeModal", () => {
  it("routes embedded graph selections to the owning plan", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Select smelter Alternative recipe" }),
    );
    expect(onSelectRecipe).toHaveBeenCalledExactlyOnceWith(
      "plate",
      "smelter:1",
    );
  ...
  it.each([undefined, "plate-alternative"])(
    "passes the recipe key to the selection callback (recipe ID: %s)",
    (id) => {
      ...
      expect(
        screen.getByRole("button", {
          name: "Selected smelter Standard recipe",
        }),
      ).toBeDisabled();
      fireEvent.click(
        screen.getByRole("button", {
          name: "Select smelter Alternative recipe",
        }),
      );
      expect(onSelectRecipe).toHaveBeenCalledExactlyOnceWith(
        "plate",
  ...
  it.each(recipes)(
    "highlights only the exact $recipeType recipe",
    (selected) => {
      render(
        <NodeRecipeModal
          item={{ id: "plate", name: "Plate", type: "processed" }}
      ...
      sections.forEach((section, index) => {
        expect(section.getAttribute("aria-current")).toBe(
          recipes[index] === selected ? "true" : null,
        );
      });
  ...
  it("keeps recipes inside a full-screen dialog and consumes Escape before closing the parent", () => {
    ...
    render(
      <dialog open aria-label="Plan diagram">
        <NodeRecipeButton
          item={{ id: "plate", name: "Plate", type: "processed" }}
          node={node}
        />
    ...
    expect(parent).toContainElement(
      screen.getByRole("dialog", { name: "Recipes for Plate" }),
    );
    ...
    expect(
      screen.queryByRole("dialog", { name: "Recipes for Plate" }),
    ).not.toBeInTheDocument();
    expect(parent).toHaveAttribute("open");
    expect(
      screen.getByRole("button", { name: "Recipes for Plate" }),
    ).toHaveFocus();
  });
});
