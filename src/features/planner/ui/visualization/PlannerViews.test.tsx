...
vi.mock("@/app/uklad/bindings", () => ({
  useRuntime: () => ({
    dispatch: ([, view]: [string, "graph" | "table"]) => {
      planner.view = view;
  ...
  useSubscription: function useSubscription([id]: [string]) {
    const view = useSyncExternalStore(
      (listener) => {
        planner.listeners.add(listener);
        return () => {
          planner.listeners.delete(listener);
        };
      },
      () => planner.view,
    );
    ...
    if (id === appIds.subscriptions.PLANNER_MULTI_TARGET_WARNING)
...
vi.mock("./PlannerFlowDiagram", () => ({
  PlannerFlowDiagram: () => (
    <input aria-label="Graph viewport" defaultValue="initial" />
  ),
}));
vi.mock("./PlannerProductionTable", () => ({
  PlannerProductionTable: () => (
    <div data-testid="table-scroll">
      <input aria-label="Table state" defaultValue="initial" />
    </div>
  ),
}));
afterEach(() => {
...
it("supports keyboard navigation between views", () => {
  render(<PlannerViews />);
  fireEvent.keyDown(screen.getByRole("tab", { name: "Graph" }), {
    key: "ArrowRight",
  });
...
it("pauses invalid multi-target views and resumes when repaired or switched to single mode", () => {
  planner.mode = "multi";
  planner.warning =
    "Iron Plate has no usable production recipe in the current game data.";
