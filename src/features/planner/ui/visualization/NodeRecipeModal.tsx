        ...
        <div className="modal-box flex max-h-[calc(100dvh-0.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-lg p-0 sm:max-h-[calc(100dvh-2rem)] sm:rounded-xl">
            <div className="flex items-center justify-between gap-2 border-b border-base-300 px-3 py-2 shrink-0 sm:px-4 sm:py-3">
                <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-base-content/60 sm:text-xs">{t("{count} recipes · Rates per building", { count: recipes.length })}</p>
                </div>
                <button type="button" className="btn btn-sm btn-circle btn-ghost size-9 shrink-0" aria-label={t("Close recipes")} onClick={onClose}>✕</button>
            </div>
            <div className="min-h-0 overflow-y-auto overscroll-contain space-y-2 p-2 sm:space-y-3 sm:p-3">
                {recipes.length === 0 && <p className="text-sm text-base-content/60">{t("No production recipes available for this item.")}</p>}
                {recipes.map(({ recipe, building, recipeIndex, recipeType }) => {
                    ...
                    return (
                        <section
                            ...
                            aria-label={`${building.name} — ${typeLabel}${selected ? t(" — Used in this node") : ''}`}
                            ...
                            <RecipeCard
                                ...
                                action={onSelectRecipe ? (
                                    <button
                                        ...
                                        disabled={selected}
                                        aria-label={t("{value} {name} {typeLabel} recipe", { value: selected ? t("Selected") : t("Select"), name: building.name, typeLabel: typeLabel })}
                                        onClick={() => {
                                    ...
                                    >
                                        {selected ? t("Selected") : t("Use recipe")}
                                    </button>
                                ) : selected ? <span className="shrink-0 text-xs font-medium text-primary">{t("In use")}</span> : undefined}
                            />
                        </section>
                    );
                })}
