...
export const NodeCard: React.FC<NodeCardProps> = ({
  ...
  const availableRate =
    node.outputAmount * (node.nodeType === "input" ? 1 : buildingCount);
  const usagePercent =
    availableRate > 0
      ? Math.min(100, Math.max(0, (usedRate / availableRate) * 100))
      : 0;
  ...
  return (
    <div className="flex h-full flex-col text-center">
      <div className="absolute -top-2 left-1/2 -translate-x-1/2 flex items-center gap-1">
        {node.nodeType === "input" && (
          <div className="rounded border border-teal-400/20 bg-base-200 px-1.5 text-[10px] leading-4 text-teal-300/70">
            input
          </div>
        )}
      </div>
      ...
      <div
        className={`${compactOnMobile ? "p-1.5 space-y-1 sm:p-2 sm:space-y-2" : "p-2 space-y-2"} ${node.nodeType === "input" ? "pt-4 sm:pt-4" : ""}`}
      >
        <div className="flex items-start justify-between gap-1.5">
          <div
            className={`min-w-0 flex-1 font-normal leading-tight break-words ${compactOnMobile ? "text-xs sm:text-base" : "text-base"}`}
          >
            {getItemName(node.outputItem, items)}
          </div>
          {item && !inputRequirement && (
            <NodeRecipeButton
              item={item}
              node={node}
              onSelectRecipe={
                node.nodeType === "input" ? undefined : onSelectRecipe
              }
            />
          )}
        </div>
        <div className="relative flex items-center justify-center gap-1.5">
          ...
          <div
            className={`shrink-0 ${compactOnMobile ? "max-sm:[&>div]:size-10 max-sm:[&_img]:size-10" : ""}`}
          >
            <ItemImage itemId={node.outputItem} size="medium" />
          </div>
          <div
            className={`min-w-0 font-semibold leading-tight break-words tabular-nums ${compactOnMobile ? "text-lg sm:text-xl" : "text-xl"}`}
            style={{ color: outputColor }}
            aria-label={
              inputRequirement
                ? "Required input per minute"
                : "Total output per minute"
            }
          >
            {usedRate.toFixed(1)}
            <span className="block text-xs font-normal">
              /min{inputRequirement ? " needed" : ""}
            </span>
          </div>
        </div>
      </div>
      ...
      {inputRequirement ? (
        <div
          className={`mt-auto p-2 text-xs ${showMissingInput ? "text-error" : "text-base-content/60"}`}
        >
          {showMissingInput
            ? inputRequirement.available > 0
        ...
        <>
          ...
          <div
            className={`shrink-0 bg-base-content/5 rounded-b ${compactOnMobile ? "p-1.5 sm:p-2" : "p-2"}`}
          >
            <div
              className={`mb-1 leading-tight text-base-content/60 break-words ${compactOnMobile ? "text-[11px] sm:text-sm" : "text-sm"}`}
            >
              {node.buildingName}
            </div>
            <div className="flex items-center gap-1.5">
              <BuildingImage
                ...
                size="small"
                className={`!w-8 !h-8 shrink-0 ${compactOnMobile ? "max-sm:!w-6 max-sm:!h-6" : ""}`}
              />
              <div className="min-w-0 flex-1 text-left text-[10px] leading-tight break-words space-y-0.5">
                <div
                  className={`${compactOnMobile ? "text-[10px] sm:text-xs" : "text-xs"} text-base-content/55`}
                >
                  {node.outputAmount.toFixed(1)}/min
                </div>
              </div>
              <span
                className={`shrink-0 rounded border py-0.5 font-semibold ${compactOnMobile ? "px-1 text-xs sm:px-1.5 sm:text-sm" : "px-1.5 text-sm"} ${buildingCount > 1 ? "border-secondary/40 bg-secondary/15 text-secondary" : "border-base-content/15 text-base-content/75"}`}
                title={`${buildingCount} buildings required`}
              >
                ×{buildingCount}
              </span>
            </div>
