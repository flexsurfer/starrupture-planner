...
export const PlannerFlowDiagram: React.FC = () => {
  ...
  const onSelectRecipe = useCallback(
    (itemId: string, recipeKey: string) => {
      runtime.dispatch([
  ...
  // State subscriptions
  const selectedItemId = useSubscription([
    appIds.subscriptions.PLANNER_ACTIVE_TARGET_IDS,
  ]);
  const groupByStage = useSubscription([
    appIds.subscriptions.PLANNER_GROUP_BY_STAGE,
  ]);
  const direction = useSubscription([
  ...
  const renderedNodes = useMemo<Node[]>(
    () =>
      flowGraph.nodes.map(({ flowNode, outputColor, ...node }) => ({
        ...node,
        style: {
          ...node.style,
          padding: 0,
      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-base-content/60 mt-2">
            Choose any processed item, component, or ammo to see the required
            buildings and resource flow.
          </p>
        </div>
      </div>
    );
