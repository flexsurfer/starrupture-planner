  ...
  return (
    <div className="planner-tab-bar">
      <div
        ...
        className="planner-tab-list"
      >
        {tabs.map((tab, index) => (
          <div
            key={tab.id}
            className={`planner-tab ${tab.id === activeTab?.id ? "is-active" : ""}`}
          >
            <button
              type="button"
              role="tab"
              id={`planner-tab-${tab.id}`}
              aria-controls="planner-tab-panel"
              aria-selected={tab.id === activeTab?.id}
              tabIndex={tab.id === activeTab?.id ? 0 : -1}
              className="planner-tab-select"
              title={`${tab.name} — ${tab.mode === "single" ? "Single target" : "Multi-target"}`}
              onClick={() =>
                runtime.dispatch([appIds.events.PLANNER_SELECT_TAB, tab.id])
              }
              onKeyDown={(event) => {
                if (event.key === "Delete") {
                  ...
                  event.key === "Home"
                    ...
                    : event.key === "End"
                      ...
                      : (index +
                        ...
                        tabs.length;
                runtime.dispatch([
                  appIds.events.PLANNER_SELECT_TAB,
                  tabs[next].id,
                ]);
                list.current
      ...
      <button
        type="button"
        className="planner-tab-add"
        aria-label="Create new planner tab"
        title="New planner tab"
        onClick={() =>
          runtime.dispatch([appIds.events.PLANNER_REQUEST_TAB_CREATION])
        }
      >
        <svg
          aria-hidden="true"
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        event.preventDefault();
        if (!name.trim()) return;
        const view = window.matchMedia?.("(max-width: 639px)").matches
          ? "table"
          : "graph";
        runtime.dispatch([
          ...
          {(["single", "multi"] as const).map((value) => (
            ...
            >
              ...
              <span className="mt-2 block text-xs text-base-content/60">
                {value === "single"
                  ? "Plan production for one item."
                  : "Plan several items together."}
              </span>
            </label>
          ))}
        </div>
        <p className="mt-2 text-xs text-base-content/60">
          The mode cannot be changed after creating the tab.
        </p>
      </fieldset>
...
export function PlannerTabCreation({ empty = false }: { empty?: boolean }) {
  ...
  const titleId = useId();
  const cancel = () =>
    runtime.dispatch([appIds.events.PLANNER_CANCEL_TAB_CREATION]);
  const initialName = request?.itemId
    ? (items[request.itemId]?.name ?? request.itemId)
    : "";
      ...
      <div className="flex flex-1 items-center justify-center overflow-auto p-4 sm:p-8">
        <section className="w-full max-w-md rounded-lg border border-base-300 bg-base-200 p-6 shadow-sm">
          <h1 className="text-xl font-semibold">
            Create your first planner tab
          </h1>
          <p className="mt-2 mb-6 text-sm text-base-content/60">
            Give your plan a name and choose a mode. Your tabs and settings are
            saved automatically.
          </p>
          <CreateTabForm
            key={request?.itemId ?? "empty"}
