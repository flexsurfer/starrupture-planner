...
export function PlannerTabs() {
    ...
    const close = (tab: PlannerTab) => runtime.dispatch([
        ...
        t("Close “{name}”?", { name: tab.name }),
        t("This will permanently delete this tab and all its planner settings, targets, amounts, and recipes."),
        () => runtime.dispatch([appIds.events.PLANNER_CLOSE_TAB, tab.id]),
        { confirmLabel: t("Delete tab"), confirmButtonClass: 'btn-error' },
    ]);

    return <div className="planner-tab-bar">
        <div ref={list} role="tablist" aria-label={t("Planner tabs")} className="planner-tab-list">
            {tabs.map((tab, index) => <div key={tab.id} className={`planner-tab ${tab.id === activeTab?.id ? 'is-active' : ''}`}>
                <button type="button" role="tab" id={`planner-tab-${tab.id}`}
                    ...
                    className="planner-tab-select" title={`${tab.name} — ${tab.mode === 'single' ? t("Single target") : t("Multi-target")}`}
                ...
                <button type="button" className="planner-tab-close" aria-label={t("Close {name}", { name: tab.name })} title={t("Close tab")}
                    onClick={() => close(tab)}>
                    <svg aria-hidden="true" viewBox="0 0 16 16" className="h-4 w-4" fill="none" stroke="currentColor"><path d="m4 4 8 8m0-8-8 8" /></svg>
                </button>
            </div>)}
        </div>
        <button type="button" className="planner-tab-add" aria-label={t("Create new planner tab")} title={t("New planner tab")}
...
function CreateTabForm({ initialName, onCancel }: { initialName: string; onCancel?: () => void }) {
    ...
    return <form className="flex flex-col gap-5" onSubmit={event => {
        ...
        <label className="flex flex-col gap-2 text-sm" htmlFor={`${id}-name`}>{t("Tab name")}<input id={`${id}-name`} className="input input-bordered w-full" placeholder={t("e.g. Steel production")}
                autoFocus required value={name} onChange={event => setName(event.target.value)} />
        </label>
        <fieldset>
            <legend className="mb-2 text-sm">{t("Planner mode")}</legend>
            <div className="grid grid-cols-2 gap-3">
                {(['single', 'multi'] as const).map(value => <label key={value}
                    ...
                    <span className="flex items-center gap-2">
                        ...
                        {value === 'single' ? t("Single target") : t("Multi-target")}
                    </span>
                    <span className="mt-2 block text-xs text-base-content/60">
                        {value === 'single' ? t("Plan production for one item.") : t("Plan several items together.")}
                    </span>
                </label>)}
            </div>
            <p className="mt-2 text-xs text-base-content/60">{t("The mode cannot be changed after creating the tab.")}</p>
        </fieldset>
        <div className="flex justify-end gap-2">
            {onCancel && <button type="button" className="btn btn-ghost" onClick={onCancel}>{t("Cancel")}</button>}
            <button type="submit" className="btn btn-primary" disabled={!name.trim()}>{t("Create tab")}</button>
        </div>
    </form>;
}
...
export function PlannerTabCreation({ empty = false }: { empty?: boolean }) {
    ...
    if (empty) return <div className="flex flex-1 items-center justify-center overflow-auto p-4 sm:p-8">
        <section className="w-full max-w-md rounded-lg border border-base-300 bg-base-200 p-6 shadow-sm">
            <h1 className="text-xl font-semibold">{t("Create your first planner tab")}</h1>
            <p className="mt-2 mb-6 text-sm text-base-content/60">{t("Give your plan a name and choose a mode. Your tabs and settings are saved automatically.")}</p>
    ...
    return <dialog ref={dialog} className="modal" aria-labelledby={titleId} onCancel={cancel}
        ...
        <div className="modal-box max-w-md">
            <h2 id={titleId} className="mb-5 text-lg font-semibold">{t("Create planner tab")}</h2>
