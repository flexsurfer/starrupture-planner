...
it("requires a nonblank name, creates fixed-mode tabs, and supports + and keyboard switching", async () => {
  const harness = setup();
  expect(
    screen.getByRole("heading", { name: "Create your first planner tab" }),
  ).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Create tab" })).toBeDisabled();
  fireEvent.change(screen.getByLabelText("Tab name"), {
    target: { value: "   " },
  });
  expect(screen.getByRole("button", { name: "Create tab" })).toBeDisabled();
  fireEvent.change(screen.getByLabelText("Tab name"), {
  ...
  await waitFor(() =>
    expect(screen.getByRole("tab", { name: "Shared plan" })).toHaveAttribute(
      "aria-selected",
      "true",
    ),
  );
  expect(
    harness.getSubscriptionValue([appIds.subscriptions.PLANNER_MODE]),
  ).toBe("multi");
  expect(screen.queryByRole("radio")).not.toBeInTheDocument();
  fireEvent.click(
    screen.getByRole("button", { name: "Create new planner tab" }),
  );
  await screen.findByRole("dialog", { name: "Create planner tab" });
  fireEvent.change(screen.getByLabelText("Tab name"), {
    target: { value: "Single plan" },
  });
  fireEvent.click(screen.getByRole("button", { name: "Create tab" }));
  await waitFor(() =>
    expect(screen.getByRole("tab", { name: "Single plan" })).toHaveAttribute(
      "aria-selected",
      "true",
    ),
  );
  ...
  fireEvent.keyDown(screen.getByRole("tab", { name: "Single plan" }), {
    key: "ArrowLeft",
  });
  await waitFor(() =>
    expect(screen.getByRole("tab", { name: "Shared plan" })).toHaveAttribute(
  ...
  await waitFor(() =>
    expect(screen.getByLabelText("Target items per minute")).toHaveValue(125),
  );
});
...
it("does not delete until confirmed, preserves an inactive tab on cancel, and shows creation after the last close", async () => {
  ...
  expect(
    harness.getSubscriptionValue([appIds.subscriptions.PLANNER_ACTIVE_TAB_ID]),
  ).toBe("two");
  fireEvent.click(screen.getByRole("button", { name: "Cancel" }));
  await waitFor(() =>
    expect(screen.queryByText("Close “One”?")).not.toBeInTheDocument(),
  );
  expect(
    harness.getSubscriptionValue([appIds.subscriptions.PLANNER_TABS]),
  ).toHaveLength(2);
...
it("renames only the active plan in settings and exports that plan", async () => {
  ...
  runtimes[runtimes.length - 1].registerModule((registrar) => {
    registrar.regEffect(appIds.effects.downloadArchive, (archive) => {
      exports.push(archive);
    });
  });
  fireEvent.click(screen.getByRole("button", { name: "Plan settings" }));
