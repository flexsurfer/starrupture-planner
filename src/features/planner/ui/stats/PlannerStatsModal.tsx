...
export const PlannerStatsModal: React.FC<PlannerStatsModalProps> = ({
  ...
  const [activeTab, setActiveTab] =
    useState<(typeof STAT_TABS)[number]>("buildings");
  const closeModal = () => {
    setActiveTab("buildings");
    onClose();
  };
  const tabId = useId();
  const itemCount = stats.sortedTypes.reduce(
    (sum, type) => sum + (stats.itemsByType.get(type)?.length ?? 0),
    0,
  );
    ...
    <div className="modal modal-open">
      <div className="modal-box w-[95vw] max-w-6xl max-h-[95vh]">
        ...
        <div className="flex items-center justify-between mb-4 lg:mb-6">
          <h3 className="text-lg lg:text-xl font-bold pr-4">
        ...
        <div
          role="tablist"
          aria-label="Production statistics views"
          className="tabs tabs-border mb-4"
        >
        ...
        >
          <p className="mb-2 text-xs text-base-content/60">
            Required amounts per minute at the current production target.
          </p>
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-sm font-semibold tabular-nums whitespace-nowrap">
                            {item.requiredRate.toLocaleString(undefined, {
