...
export function PlannerTargetAlert() {
  ...
  return (
    <dialog
      ...
      onClose={() => {
        if (warning)
          runtime.dispatch([appIds.events.PLANNER_DISMISS_TARGET_WARNING]);
      }}
    >
      <div className="modal-box max-w-md">
        ...
        <p id={messageId} className="mt-3 text-sm">
          {warning}
        </p>
        <form method="dialog" className="modal-action">
          <button type="submit" autoFocus className="btn btn-sm btn-primary">
            OK
          </button>
        </form>
      </div>
