 ...
 */
export const RecipeAlternativesDropdown: React.FC<
  RecipeAlternativesDropdownProps
> = ({
  options,
  onSelectRecipe,
  onApplySelections,
  ...
  const itemsById =
    useSubscription([appIds.subscriptions.ITEMS_BY_ID_MAP]) ??
    EMPTY_ITEMS_BY_ID;
  const defaultSelections =
    useSubscription([appIds.subscriptions.PINNED_RECIPE_SELECTIONS]) ??
    EMPTY_PINNED_SELECTIONS;
  const presets =
    useSubscription([appIds.subscriptions.RECIPE_ALTERNATIVE_PRESETS]) ??
    EMPTY_PRESETS;
  ...
  const selectedNonDefault = options.filter(
    (entry) => entry.selectedKey !== entry.defaultKey,
  ).length;

  const normalizedOptions = options.map((entry) => {
    const selectedOption =
      entry.options.find((option) => option.key === entry.selectedKey) ??
      entry.options[0]!;
  ...
  const isCurrentDefault =
    ...
    sameSelections(currentSelections, defaultSelections);
  ...
  const handleToggleDefault = () => {
    runtime.dispatch([
      ...
      isCurrentDefault ? {} : currentSelections,
    ]);
  };
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`.trim()}>
      ...
      {isOpen && (
        ...
        >
          <div className="relative z-20 shrink-0 rounded-t-md border-b border-base-300 bg-base-100 px-3 py-2">
            <div className="mb-2 flex items-center justify-between sm:hidden">
              <span className="text-sm font-semibold">Recipe Alternatives</span>
              <button
                type="button"
                className="btn btn-ghost min-h-11 min-w-11"
                aria-label="Close recipe alternatives"
                onClick={() => {
                  setIsOpen(false);
                  setIsLoadOpen(false);
                }}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="hidden sm:inline text-xs font-semibold text-base-content/80 mr-auto">
                Recipe Alternatives
              </span>
                    ...
                    {presets.map((preset) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={(event) =>
                            handleDeletePreset(event, preset.id)
                          }
                        >
                          ✕
                        </button>
            ...
            <div className="mt-1 text-[11px] leading-snug text-base-content/60">
              Save and load named sets of alternatives, or make the current set
              the default machines pre-selected for every new plan.
            </div>
          </div>
          ...
          <div className="min-h-0 overflow-y-auto overscroll-contain p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
            {normalizedOptions.map(({ entry, selectedOption }) => (
              <div
                key={entry.itemId}
                className="rounded-md border border-base-300 bg-base-200/40 p-2 mb-2 last:mb-0"
              >
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            onClick={() =>
                              onSelectRecipe(entry.itemId, option.key)
                            }
                          >
                            <div
