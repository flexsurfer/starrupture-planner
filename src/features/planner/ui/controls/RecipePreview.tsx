...
const RecipeIngredient = ({ itemId, amount, item }: RecipeIngredientProps) => (
  <div className="flex w-12 shrink-0 flex-col items-center gap-0.5">
    ...
    <ItemImage itemId={itemId} item={item} size="small" className="!h-8 !w-8" />
    <span
      className="w-full text-center text-[9px] leading-tight text-base-content/80"
...
export const RecipePreview = ({ option, itemsById }: RecipePreviewProps) => {
  ...
  return (
    <div aria-label="Selected recipe" className="min-w-0">
      <div className="flex items-end gap-1.5">
        ...
        />
        <span
          className="shrink-0 self-center text-base-content/50"
          aria-hidden="true"
        >
          ←
        </span>
        <div className="flex min-w-0 flex-wrap items-end gap-1">
          {recipe.inputs.length > 0 ? (
            ...
            <span className="py-3 text-[10px] italic text-base-content/60">
              No inputs
            </span>
          )}
        </div>
