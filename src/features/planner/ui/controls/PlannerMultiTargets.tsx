...
export function PlannerMultiTargets() {
  ...
  return (
    <>
      {warning && (
        <div role="alert" className="alert alert-warning w-full text-sm">
          <div>
            <p>{warning}</p>
            <p>
              Production is paused. Change recipes or remove a target to
              continue.
            </p>
          </div>
        </div>
      ...
      {targets.map((target) => {
        const name =
          items.find((item) => item.id === target.itemId)?.name ??
          target.itemId;
        return (
          <div
            key={target.itemId}
          ...
          >
            <span className="min-w-0 max-w-48 truncate text-sm" title={name}>
              {name}
            </span>
            <TargetRate
            ...
            <button
              ...
              aria-label={`Remove ${name} target`}
              title={`Remove ${name} target`}
              onClick={() =>
                runtime.dispatch([
      ...
      <select
        className="select select-sm select-bordered w-50 min-w-0 max-w-full text-xs sm:text-sm"
        aria-label="Add production target"
        value=""
        onChange={(event) => {
