...
function TargetRate({ itemId, name, amount }: { itemId: string; name: string; amount: number }) {
    ...
    return <label className="flex shrink-0 items-center gap-1 text-sm">
        <input
            ...
            aria-label={t("{name} target items per minute", { name: name })}
            value={draft ?? amount}
            onChange={event => {
...
export function PlannerMultiTargets() {
    ...
    return <>
        {warning && <div role="alert" className="alert alert-warning w-full text-sm">
            <div>
                <p>{warning && translateText(t, warning)}</p>
                <p>{t("Production is paused. Change recipes or remove a target to continue.")}</p>
            </div>
        </div>}
        {targets.map(target => {
            ...
            return <div key={target.itemId} className="flex min-w-0 max-w-full items-center gap-2">
                ...
                <button type="button" className="btn btn-sm btn-square btn-ghost shrink-0" aria-label={t("Remove {name} target", { name: name })} title={t("Remove {name} target", { name: name })}
        ...
        <select className="select select-sm select-bordered w-50 min-w-0 max-w-full text-xs sm:text-sm" aria-label={t("Add production target")} value=""
            onChange={event => {
                if (event.target.value) runtime.dispatch([appIds.events.PLANNER_ADD_TARGET, event.target.value]);
            }}>
            <option value="">{t("Add target…")}</option>
