...
export function getTargetNodeIds(nodes: PlannerFlowNode[], targetItemIds: string[]): Set<string> {
    const targets = new Set(targetItemIds);
    const separateTargets = new Set(nodes.filter(node => node.nodeType === 'target').map(node => node.outputItem));
    return new Set(nodes.filter(node => node.nodeType === 'target'
        || (node.nodeType === 'production' && targets.has(node.outputItem) && !separateTargets.has(node.outputItem)))
        .map(getFlowNodeId));
}
