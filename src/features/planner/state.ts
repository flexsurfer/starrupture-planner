...
export function getActivePlannerTab(
  state: Pick<PlannerFeatureState, "plannerTabs" | "plannerActiveTabId">,
): PlannerTab | null {
  return (
    state.plannerTabs.find((tab) => tab.id === state.plannerActiveTabId) ??
    state.plannerTabs[0] ??
    null
  );
}
