...
export function buildProductionStageLayout(
  ...
  const stages: number[] = flowNodes.map((node) =>
    itemTypes.get(node.outputItem) === "raw"
      ? 0
      : itemTypes.get(node.outputItem) === "processed"
        ? 1
        : 2,
  );
  const pendingInputs = flowNodes.map(
    (_, index) => graph.inEdges(`node_${index}`)?.length ?? 0,
  );
  const ready = pendingInputs.flatMap((count, index) =>
    count === 0 ? [index] : [],
  );
  let visited = 0;
  ...
  if (visited === flowNodes.length) {
    const isTarget = (index: number) =>
      targetNodeIds.has(getFlowNodeId(flowNodes[index]));
    const targetStage =
      Math.max(
        1,
        ...stages.filter(
          (_, index) =>
            !isTarget(index) && flowNodes[index].nodeType !== "launcher",
        ),
      ) + 1;
    ...
    const maxRows = Math.max(
      0,
      ...[...grouped.values()].map((indices) => indices.length),
    );
    for (const [stage, indices] of grouped) {
      indices.sort(
        (a, b) =>
          graph.node(`node_${a}`).y - graph.node(`node_${b}`).y || a - b,
      );
