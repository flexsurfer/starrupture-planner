...
function applyMatchInputs(draftState: AppState): void {
  ...
  const maxAmount = calculateMaxTargetFromInputs({
    selectedItemId,
    inputBuildings: getSelectedFlowInputBuildings(
      base,
      selectedInputIds,
...
function computeUsedInputSnapshots(
  ...
  return usedInputIdSet.size === 0
    ? []
    : inputBuildings.filter((inputBuilding) =>
...
function saveProductionPlan(draftState: AppState): void {
  ...
  const usedInputSnapshots = (
    draftState.basesMode === "planning"
      ? base.buildings.filter(
          (input) =>
            modal.selectedInputIds.includes(input.id) &&
  ...
  if (editSectionId) {
    const section = base.productions.find(
      (candidate) => candidate.id === editSectionId,
    );
    if (!section) return;
    ...
    for (const output of base.buildings) {
      if (
        ...
        output.planningOwnerPlanId === section.id
      ) {
  ...
  if (
    draftState.basesMode === "planning" &&
    draftState.buildingsList.some(
      (building) => building.id === PACKAGE_DISPATCHER_BUILDING_ID,
    )
  ) {
    base.buildings.push({
      ...createBaseBuilding({
        buildingTypeId: PACKAGE_DISPATCHER_BUILDING_ID,
        sectionType: "outputs",
        sourceProductionId: newSection.id,
        allocationMode: "auto",
        name: `${newSection.name} output`,
        selectedItemId,
      }),
...
export function selectAddedProductionPlanInputs(
  draftState: AppState,
  baseId: string,
  inputIds: string[],
): void {
  ...
  modal.selectedInputIds.push(
    ...inputIds.filter((id) => !modal.selectedInputIds.includes(id)),
  );
  const base = getBaseById(draftState.basesList, baseId);
  const inputs = getSelectedFlowInputBuildings(
    base,
    modal.selectedInputIds,
    draftState.basesList,
  );
  modal.recipeSelections = sanitizeRecipeSelectionsForInputItems(
    modal.recipeSelections,
    inputs,
  );
...
export const registerProductionPlanModalEvents: UkladModule<
...
> = (registrar) => {
  registrar.regEvent(
    ...
    ({ draftState }) => {
      draftState.productionPlanModalState =
        createProductionPlanModalFeatureState().productionPlanModalState;
    },
  );
    ...
    ({ draftState }, itemId) => {
      ...
      if (itemId) {
        if (!modalState.name.trim()) {
          const itemName =
            draftState.itemsList.find((item) => item.id === itemId)?.name ||
            itemId;
          modalState.name = `${itemName} Production`;
        }
        modalState.targetAmount = getSlowestOutputRateForItem(
          draftState.buildingsList,
          itemId,
        );
        applyMatchInputs(draftState as AppState);
  ...
  registrar.regEvent(
    ...
    ({ draftState }, itemId, recipeKey) => {
      if (!itemId) return;

      const modalState = draftState.productionPlanModalState;
      const base = modalState.baseId
        ? getBaseById(draftState.basesList, modalState.baseId)
        : undefined;
      const selectedInputBuildings = getSelectedFlowInputBuildings(
        base,
  ...
  registrar.regEvent(
    ...
    ({ draftState }, selections) => {
      ...
      modalState.recipeSelections = sanitizeRecipeSelectionsForInputItems(
        { ...(selections || {}) },
        selectedInputBuildings,
      );
      applyMatchInputs(draftState as AppState);
  ...
  registrar.regEvent(
    ...
    ({ draftState }, baseBuildingId) => {
      ...
      if (index >= 0) {
        ...
        const input = base?.buildings.find(
          (input) => input.id === baseBuildingId,
        );
        if (!input) return;
        if (
          draftState.basesMode === "planning" &&
  ...
  registrar.regEvent(
    ...
    (
      ...
      const targetPlan = targetBase.productions.find(
        (plan) => plan.id === modalState.editSectionId,
      );
      if (
        planning &&
      ...
      const targetBuilding = targetBuildingTypeId
        ? draftState.buildingsList.find(
            (building) => building.id === targetBuildingTypeId,
          )
        : undefined;
      const inputBuildingTypeId = planning
        ? PACKAGE_RECEIVER_BUILDING_ID
        : targetBuilding &&
      ...
      if (
        !inputBuildingTypeId ||
        !draftState.buildingsList.some(
          (building) => building.id === inputBuildingTypeId,
        )
      )
        return;

      const existingLinkedInput = targetBase.buildings.find(
        (building) =>
      ...
      if (planning && targetPlan)
        linkedInput.planningOwnerPlanId = targetPlan.id;
