...
export const InputsSelector: React.FC = () => {
  ...
  const { inputItems, selectedInputIds } = useSubscription([
        ...
        {inputItems.length === 0 ? (
          <div className="rounded-lg border border-base-300 bg-base-200/40 px-3 py-2">
            <div className="flex items-start gap-2 text-base-content/65">
              ...
              <span className="text-xs">
                {planning
                  ? "Use Add external item to supply resources from another plan or base."
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div
              ...
              id={scrollId}
              role="region"
              aria-label="Production plan inputs"
              tabIndex={0}
              className="flex min-w-0 flex-1 flex-nowrap items-stretch gap-2 overflow-x-auto pb-1"
            >
              {inputItems.map((inputItem) => {
                const isSelected = selectedInputIds.includes(
                  inputItem.baseBuildingId,
                );
                const hasLinkError =
                  !!inputItem.linkedOutput &&
                  inputItem.linkedOutput.status !== "ok";
                ...
                return (
                  <button
                    ...
                    aria-label={
                      planning
                        ? `${isSelected ? "Remove" : "Use"} input ${inputItem.item.name} from ${linkLabel || inputItem.name}`
                        : undefined
                    }
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        style={{
                          color: getItemCategoryColor(inputItem.item.type),
                        }}
                      >
                        {formatQuantity(inputItem.ratePerMinute)}
                        <span className="font-normal text-xs">/min</span>
                      </span>
                    </div>
                    <div
                      className="flex items-center gap-1.5 text-base-content/45"
                      title={inputItem.description}
                    >
                      {!planning && (
                        <BuildingImage
                      ...
                      {!planning && (
                        <span className="text-xs max-w-64 truncate">
                          {inputItem.name || inputItem.building.name}
                        </span>
                      )}
                      {inputItem.linkedOutput && (
                        <span
