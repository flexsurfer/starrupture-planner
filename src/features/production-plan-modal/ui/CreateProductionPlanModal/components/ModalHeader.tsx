...
export const ModalHeader: React.FC = () => {
    ...
    const returnLabel = selectedTab === 'plans' ? t("Plans") : selectedTab === 'buildings' && advanced ? t("Buildings") : t("Production");

    return (
        <NavigationHeader title={isEditMode ? t("Edit Plan") : t("New Plan")}
            breadcrumbs={[
                { label: t("My Bases"), onClick: () => {
                    handleClose();
                    runtime.dispatch([appIds.events.BASES_SET_SELECTED_BASE, null]);
                } },
                { label: base?.name || t("Base"), onClick: () => goToBase('base') },
                { label: t("Plans"), onClick: () => goToBase('plans') },
                { label: defaultName || t("New Plan") },
            ]}
            back={{ label: t("Back to {returnLabel}", { returnLabel: returnLabel }), onClick: handleClose }}
