...
const ManageInputsModal = ({ baseId, onClose }: { baseId: string; onClose: () => void }) => {
    ...
    return (
        <div className="modal modal-open">
            <div role="dialog" aria-modal="true" aria-label={t("Manage inputs")}
                ...
                <div className="border-b border-base-300 px-6 py-4">
                    <div className="flex items-center justify-between gap-3">
                        <h2 className="text-lg font-bold">{t("Manage inputs")}</h2>
                        <button type="button" className="btn btn-sm btn-circle btn-ghost"
                            aria-label={t("Close manage inputs")} onClick={onClose}>✕</button>
                    </div>
                    <p className="mt-1 text-sm text-base-content/65">{t("All input buildings in this base. Changes apply immediately and can affect other plans using these inputs.")}</p>
                </div>
                <div className="min-h-0 flex-1 overflow-y-auto p-4">
                    {buildings.length > 0 ? (
                        ...
                        <p className="rounded-lg border border-dashed border-base-300 p-5 text-sm text-base-content/65">{t("No input buildings in this base.")}</p>
                    )}
                </div>
                <div className="flex justify-end border-t border-base-300 px-6 py-3">
                    <button type="button" className="btn btn-sm" onClick={onClose}>{t("Done")}</button>
                </div>
            </div>
...
export const ManageInputsButton = () => {
    ...
    return <>
        <button type="button" className="btn btn-sm btn-outline h-8 min-h-8 shrink-0 gap-1 px-2 text-xs"
            ...
            <SectionIcon name="buildings" className="size-3.5" />{t("Manage inputs")}</button>
