...
export const LinkOutputModal: React.FC<LinkOutputModalProps> = ({
  isOpen,
  onClose,
  onSelect,
  ...
  return (
    <div className="modal modal-open">
      <div
        role="dialog"
        aria-modal="true"
        aria-label={planning ? "Add external item" : "Link Output"}
        className="modal-box max-w-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden"
      >
        <div className="px-6 pt-5 pb-3 border-b border-base-300">
          <div className="flex flex-wrap items-center gap-3">
            <h3 className="mr-auto font-bold text-lg">
              {planning ? "Add external item" : "Link Output"}
            </h3>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredOutputs.length === 0 ? (
            <div className="rounded-lg border border-dashed border-base-300 bg-base-200/40 px-4 py-5 text-sm text-base-content/65">
              {planning
                ? "No available outputs found. Create a plan in another base, or configure a free output in Advanced mode."
