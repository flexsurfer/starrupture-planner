...
import {
  canSelectPlanningInput,
  canUsePlanningOutput,
} from "@/features/production-plans/planning-endpoints";
...
const isLauncherEnabled = (
  corporationLevel: CreateProductionPlanModalState["selectedCorporationLevel"],
): boolean => corporationLevel !== null && corporationLevel !== undefined;

export const registerProductionPlanModalSubscriptions: UkladModule<
  UkladRegistrar<AppContracts>
> = (registrar) => {
  registrar.regRootSub(
    appIds.subscriptions.PRODUCTION_PLAN_MODAL_STATE,
    stateKeys.productionPlanModalState,
  );
  ...
  registrar.regSub(
    ...
    ([modalState, buildings, basesById]: [
      CreateProductionPlanModalState,
      DbBuilding[],
      BasesById,
    ]) => {
      ...
      const base = modalState.baseId
        ? basesById[modalState.baseId] || null
        : null;
      const inputBuildings = getSelectedFlowInputBuildings(
        base,
        modalState.selectedInputIds || [],
        Object.values(basesById),
      );
      return buildProductionFlow(
        {
          targetItemId: modalState.selectedItemId,
          targetAmount:
            modalState.targetAmount > 0 ? modalState.targetAmount : 1,
          inputBuildings,
          rawProductionDisabled: true,
          includeLauncher: isLauncherEnabled(
            modalState.selectedCorporationLevel,
          ),
  ...
  registrar.regSub(
    ...
    ([modalState, productionFlow, buildings, itemsById, basesById]: [
      CreateProductionPlanModalState,
      ProductionFlowResult,
      DbBuilding[],
      Record<string, Item>,
      ...
      productionFlow.nodes.forEach((node) => {
        if (
          ...
          !inputItemIds.has(node.outputItem)
        )
      ...
      corporations.forEach((corporation) =>
        corporation.levels.forEach((level) =>
          level.components.forEach((component) => {
            if (component.id === modalState.selectedItemId)
              levels.push({
  ...
  registrar.regSub(
    ...
    ([basesById, buildingsById, itemsMap, allBases, modalState, mode]) => {
      if (!modalState.baseId || !basesById[modalState.baseId])
        return { inputItems: [], selectedInputIds: [] };
      const base = basesById[modalState.baseId];
      const targetPlan = base.productions.find(
        (plan) => plan.id === modalState.editSectionId,
      );
      const inputBase =
        mode === "planning"
          ? {
              ...
              buildings: base.buildings.filter(
                (input) =>
                  modalState.selectedInputIds.includes(input.id) ||
                  canSelectPlanningInput(allBases, input, base.id, targetPlan),
              ),
            }
          : base;
      const inputItems: BaseInputItem[] = collectConfiguredSectionItems(
        inputBase,
        buildingsById,
        itemsMap,
        "inputs",
      ...
      return {
        inputItems,
        selectedInputIds: modalState.selectedInputIds || [],
      };
    },
  );
  ...
  registrar.regSub(
    ...
    ([bases, buildingsById, itemsMap, modalState, mode]) => {
      const linkableOutputs: LinkableOutputItem[] = [];
      const targetPlan = bases
        .find((base) => base.id === modalState.baseId)
        ?.productions.find((plan) => plan.id === modalState.editSectionId);
      bases.forEach((base) =>
        collectConfiguredSectionItems(
          ...
          if (mode === "planning") {
            const output = base.buildings.find(
              (building) => building.id === entry.baseBuildingId,
            )!;
            if (
              !targetPlan ||
              !canUsePlanningOutput(
          ...
          if (!isLogisticsExcludedOutputBuildingId(entry.building.id))
            linkableOutputs.push({
              baseId: base.id,
              baseName: base.name,
              isCurrentBase: base.id === modalState.baseId,
              baseBuildingId: entry.baseBuildingId,
              item: entry.item,
              ratePerMinute: entry.ratePerMinute,
              building: entry.building,
      ...
      return linkableOutputs.sort((left, right) =>
        left.isCurrentBase !== right.isCurrentBase
          ? left.isCurrentBase
            ? -1
            : 1
  ...
  registrar.regSub(
    ...
    ([productionFlow, items]: [
      ...
      (productionFlow.rawMaterialDeficits || []).map((deficit) => ({
        ...deficit,
  ...
  registrar.regSub(
    ...
    ([modalState, selectedItemId]: [CreateProductionPlanModalState, string]) =>
      !!(
        modalState.name.trim() &&
        selectedItemId &&
        Number.isFinite(modalState.targetAmount) &&
