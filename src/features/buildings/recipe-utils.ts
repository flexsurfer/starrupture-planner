...
export function getRecipeDisplayType(
  ...
  return buildings.some(({ upgrade }) => upgrade === building.id)
    ? "upgrade"
    : "standard";
}

/** Keeps alternatives last, then applies the production-flow rate/name ordering. */
export function compareItemRecipesForDisplay(
  a: ItemRecipe,
  b: ItemRecipe,
): number {
  const typeDiff =
    RECIPE_TYPE_ORDER[a.recipeType] - RECIPE_TYPE_ORDER[b.recipeType];
