  ...
  registrar.regSub(
    ...
    ([buildings, helperMaps], ..._params) => {
      ...
      const productionBuildings = buildings.filter(
        (building) => building.type === "production",
      );
      const sorted = [...productionBuildings].sort((a, b) => {
        ...
        const minLevelA =
          usageA.length > 0
            ? Math.min(...usageA.map((usage) => usage.level))
            : Infinity;
        const minLevelB =
          usageB.length > 0
            ? Math.min(...usageB.map((usage) => usage.level))
            : Infinity;
      ...
      const buildingsById = new Map<string, Building>(
        sorted.map((building) => [building.id, building]),
      );
      ...
      for (const building of sorted) {
        if (emittedIds.has(building.id) || upgradeTargetIds.has(building.id))
          continue;
        result.push(building);
        emittedIds.add(building.id);

        const upgradedBuilding = building.upgrade
          ? buildingsById.get(building.upgrade)
          : undefined;
