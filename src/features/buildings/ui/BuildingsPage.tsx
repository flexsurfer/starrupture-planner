...
const BuildingsPage = () => {
  const sortedBuildings = useSubscription([
  ...
  const totalRecipes = sortedBuildings.reduce(
        ...
        {sortedBuildings.map((building) => {
          const displayRecipes = (building.recipes ?? [])
            .map((recipe, recipeIndex) => ({
              recipe,
              recipeIndex,
              recipeType: getRecipeDisplayType(
                recipe,
                building,
                sortedBuildings,
              ),
            ...
            .sort(
              (a, b) =>
                Number(a.recipeType === "alternative") -
                  Number(b.recipeType === "alternative") ||
            ...
            <ExpandableSection
              ...
              summary={
                <>
                  ...
                  {findBuildingCorporationUsage(building.name).map(
                    (usage, index) => (
                      <CorporationUsageBadge
                        key={index}
                        usage={usage}
                        corporationId={getCorporationId(usage.corporation)}
                      />
                    ),
                  )}
                </>
