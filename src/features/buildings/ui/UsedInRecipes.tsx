...
export const UsedInRecipes = ({ itemId, itemName }: UsedInRecipesProps) => {
  ...
  return (
    <details className="mt-3 border-t border-base-300 pt-2 sm:mt-4 sm:pt-3">
      <summary
        className="cursor-pointer py-1 text-xs font-medium text-base-content/70 sm:text-sm"
        title={`${itemName} is used in ${recipes.length} recipes`}
      >
      ...
      <div className="mt-2 space-y-2 sm:space-y-3">
        {recipes.map((entry) => (
          <RecipeCard
            key={`${entry.building.id}:${entry.recipe.id ?? entry.recipeIndex}`}
            recipe={entry.recipe}
