...
import {
  findItemRecipes,
  findRecipesUsingInput,
} from "@/features/buildings/recipe-utils";
import {
...
export const registerItemsSubscriptions: UkladModule<
...
> = (registrar) => {
  ...
  registrar.regRootSub(
    ...
    stateKeys.itemsSelectedCategory,
  );
  registrar.regRootSub(
    appIds.subscriptions.ITEMS_SELECTED_BUILDING,
    stateKeys.itemsSelectedBuilding,
  );
  registrar.regRootSub(
    appIds.subscriptions.ITEMS_SEARCH_TERM,
    stateKeys.itemsSearchTerm,
  );
  registrar.regRootSub(
    appIds.subscriptions.ITEMS_CATEGORIES,
    stateKeys.itemsCategories,
  );
  registrar.regSub(
    ...
    ([filteredItems, buildings, corporations], ..._params) => {
      ...
      for (const building of buildings) {
        for (const recipe of building.recipes || []) {
          ...
          if (
            ...
            recipe.output.amount_per_minute < existingRate
          ) {
      ...
      return filteredItems.map((item) => ({
        ...
        producingBuildings: Array.from(
          (
            producingBuildingsMap.get(item.id) || new Map<string, number>()
          ).entries(),
        )
