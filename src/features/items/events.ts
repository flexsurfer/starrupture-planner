...
export const registerItemsEvents: UkladModule<UkladRegistrar<AppContracts>> = (
  registrar,
) => {
  registrar.regEvent(
    appIds.events.ITEMS_SET_SELECTED_CATEGORY,
    ({ draftState }, category) => {
  ...
  registrar.regEvent(
    appIds.events.ITEMS_SET_SELECTED_BUILDING,
    ({ draftState }, building) => {
