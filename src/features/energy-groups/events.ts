...
> = (registrar) => {
  ...
  registrar.regEvent(
    appIds.events.ENERGY_GROUP_DELETE,
    ({ draftState }, groupId) => {
      if (!draftState.energyGroups.some((group) => group.id === groupId))
        return;

      draftState.energyGroups = draftState.energyGroups.filter(
  ...
  registrar.regEvent(
    ...
    ({ draftState }, groupId, name) => {
      const group = draftState.energyGroups.find(
      ...
      const normalizedName = normalizeEnergyGroupName(name);
      if (!normalizedName) return;
      const duplicateByName = draftState.energyGroups.find(
        (candidate) =>
