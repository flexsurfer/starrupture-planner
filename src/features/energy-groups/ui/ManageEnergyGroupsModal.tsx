...
export const ManageEnergyGroupsModal: React.FC<
  ManageEnergyGroupsModalProps
> = ({ isOpen, onClose }) => {
  const runtime = useRuntime();
  const energyGroups = useSubscription([
  ...
  const linkedBasesCount = useMemo(() => {
    return bases.reduce(
      (total, base) => total + (base.energyGroupId ? 1 : 0),
      0,
    );
  }, [bases]);
    ...
    <div className="modal modal-open" role="dialog" aria-modal="true">
      <div className="modal-box max-w-lg">
        ...
        <p className="text-sm text-base-content/70 mt-1 mb-4">
          {energyGroups.length} group{energyGroups.length !== 1 ? "s" : ""}{" "}
          across {linkedBasesCount} linked base
          {linkedBasesCount !== 1 ? "s" : ""}.
        </p>
