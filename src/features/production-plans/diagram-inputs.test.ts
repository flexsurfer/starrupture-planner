...
const buildings: Building[] = [
  {
    ...
    recipes: [
      ...
      {
        output: { id: "bar", amount_per_minute: 30 },
        inputs: [{ id: "water", amount_per_minute: 90 }],
      },
    ],
  },
  {
    ...
    recipes: [
      {
        ...
        inputs: [
          { id: "bar", amount_per_minute: 30 },
          { id: "ore", amount_per_minute: 20 },
        ],
      },
    ],
  },
...
it.each([0, 40, 80, 100])(
  "connects only unmet demand to requirement cards with %s/min input supply",
  (supply) => {
    const inputBuildings: BaseBuilding[] = supply
      ? [
          {
            id: "ore-input",
            buildingTypeId: "cargo",
            sectionType: "inputs",
            selectedItemId: "ore",
            ratePerMinute: supply,
          },
        ]
      : [];
    ...
    expect(diagram.requirements.get("required-input:ore")).toMatchObject({
      required: 80,
      available: supply,
      missing: 80 - supply,
    });
    const newEdges = diagram.edges.slice(flow.edges.length);
    expect(newEdges.reduce((sum, edge) => sum + edge.amount, 0)).toBeCloseTo(
      80 - supply,
    );
    for (const [to, expected] of [
      ...
      expect(
        diagram.edges
          .filter((edge) => edge.to === to && edge.itemId === "ore")
          .reduce((sum, edge) => sum + edge.amount, 0),
      ).toBeCloseTo(expected);
    }
    expect(
      diagram.nodes.filter((node) => node.baseBuildingId === "ore-input"),
    ).toEqual(flow.nodes.filter((node) => node.baseBuildingId === "ore-input"));
  },
);
  ...
  const flow = buildProductionFlow(
    {
      ...
      targetAmount: 15,
      rawProductionDisabled: true,
      recipeSelections: { bar: "smelter:1" },
    },
    buildings,
  );
  const diagram = addDiagramInputRequirements(flow, buildings);
  expect([...diagram.requirements.keys()].sort()).toEqual([
    "required-input:ore",
