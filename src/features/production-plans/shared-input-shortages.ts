...
function resolveInputAvailability(
  ...
  const currentInput = planBase.buildings.find(
    (building) => building.id === input.id,
  );
  ...
  if (!currentInput.ratePerMinute || currentInput.ratePerMinute <= 0)
    return null;
...
function buildPlanInputUsageByBuildingId(
  ...
  const inputSnapshotsById = new Map(
    inputSnapshots.map((input) => [input.id, input]),
  );
  ...
  for (const node of productionFlow.nodes) {
    if (node.nodeType !== "input" || !node.baseBuildingId || !node.outputItem)
      continue;
    ...
    const availability = resolveInputAvailability(
      inputSnapshot,
      planBase,
      allBases,
    );
...
export function calculateSharedInputShortages(
  ...
  const planEntriesForCheck = basesForCheck.flatMap((candidateBase) =>
    (candidateBase.productions || [])
      .filter(
        (plan) =>
          plan.active ||
          (candidateBase.id === base.id && plan.id === sectionId),
      )
  ...
  for (const entry of planEntriesForCheck) {
    const usage = buildPlanInputUsageByBuildingId(
      entry.base,
      entry.plan,
      buildings,
      basesById,
    ...
    usage.forEach((usageEntry) => {
      requiredBySourceKey.set(
        ...
        (requiredBySourceKey.get(usageEntry.sourceKey) || 0) +
          usageEntry.requiredPerMinute,
      );
      if (!availableBySourceKey.has(usageEntry.sourceKey)) {
        availableBySourceKey.set(
          usageEntry.sourceKey,
          usageEntry.availablePerMinute,
        );
      }
    });
  }
  ...
  currentUsage.forEach((entry, baseBuildingId) => {
    const availablePerMinute =
      availableBySourceKey.get(entry.sourceKey) ?? entry.availablePerMinute;
