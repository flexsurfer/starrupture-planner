...
export function canUsePlanningOutput(
  bases: Base[],
  sourceBase: Base,
  output: BaseBuilding,
  targetBaseId: string,
  targetPlan: Production,
  existingInputId?: string,
): boolean {
  if (
    output.sectionType !== "outputs" ||
    isLogisticsExcludedOutputBuildingId(output.buildingTypeId)
  )
    return false;
  const resolved = resolveOutputBuilding(output, sourceBase);
  if (
    !resolved.selectedItemId ||
    resolved.selectedItemId === targetPlan.selectedItemId ||
    ...
    bases.some((base) =>
      base.buildings.some(
        (input) =>
          ...
          input.linkedOutput?.baseId === sourceBase.id &&
          input.linkedOutput.buildingId === output.id,
      ),
    )
  )
    return false;
  ...
  const dependsOnTarget = (base: Base, planId: string): boolean => {
    ...
    return (plan?.inputs ?? []).some((snapshot) => {
      ...
      const upstream = bases.find(
        (candidate) => candidate.id === snapshot.linkedOutput?.baseId,
      );
      const endpoint = upstream?.buildings.find(
        (candidate) => candidate.id === snapshot.linkedOutput?.buildingId,
      );
      return !!(
        upstream &&
        endpoint?.sourceProductionId &&
        dependsOnTarget(upstream, endpoint.sourceProductionId)
      );
    });
  };
  return (
    !output.sourceProductionId ||
    !dependsOnTarget(sourceBase, output.sourceProductionId)
  );
}

/** Existing receivers may retain their own output reservation, but must obey the same connection rules. */
export function canSelectPlanningInput(
  bases: Base[],
  input: BaseBuilding,
  targetBaseId: string,
  targetPlan: Production | undefined,
): boolean {
  if (input.sectionType !== "inputs") return false;
  if (input.planningOwnerPlanId && input.planningOwnerPlanId !== targetPlan?.id)
    return false;
  if (!input.linkedOutput) return true;
  const sourceBase = bases.find(
    (base) => base.id === input.linkedOutput?.baseId,
  );
  const output = sourceBase?.buildings.find(
    (building) => building.id === input.linkedOutput?.buildingId,
  );
  return !!(
    sourceBase &&
    output &&
...
export function removeOwnedPlanningInput(
  base: Base,
  inputId: string,
  planId: string,
): void {
  ...
  if (
    base.productions.some(
      (plan) =>
        plan.id !== planId &&
        plan.inputs?.some((candidate) => candidate.id === inputId),
    )
  ) {
    ...
    base.buildings = base.buildings.filter(
      (building) => building.id !== inputId,
    );
  }
  const owner = base.productions.find((plan) => plan.id === planId);
  if (owner)
    owner.inputs = owner.inputs?.filter(
