...
const { fitView, dispatch } = vi.hoisted(() => ({
  fitView: vi.fn(),
  dispatch: vi.fn(),
}));
...
vi.mock("@/app/uklad/bindings", () => ({
  ...
  useSubscription: ([id]: [string]) => {
    switch (id) {
      case appIds.subscriptions.UI_THEME:
        return "dark";
      case appIds.subscriptions.ITEMS_LIST:
        return items;
      case appIds.subscriptions.BUILDINGS_LIST:
        return buildings;
      case appIds.subscriptions.BASES_MODE:
        return basesMode;
      case appIds.subscriptions.ITEMS_BY_ID_MAP:
        return Object.fromEntries(items.map((item) => [item.id, item]));
      case appIds.subscriptions.BASES_LIST:
        return [];
      case appIds.subscriptions.PRODUCTION_PLAN_SECTION_FLOW_BY_ID:
        return productionFlow;
      case appIds.subscriptions.PRODUCTION_PLAN_SECTION_VIEW_MODEL_BY_ID:
        return {
          selectedBaseId: "base",
          section: {
            id: "plan",
          ...
          itemName: "Plate",
          stats: { buildingCount: 1, totalHeat: 0, totalPowerConsumption: 0 },
...
const productionFlow: ProductionFlowResult = {
  nodes: ["ore", "plate", "water"].map((outputItem, index) => ({
    ...
    buildingId: index === 1 ? "smelter" : "cargo",
...
it("uses the new edge style and lets users pin connections in saved base plans", () => {
  ...
  expect(diagramProps.nodes?.[1].style?.borderColor).toBe(
    "var(--color-primary)",
  );
  expect(diagramProps.nodes?.[0].style?.borderColor).not.toBe(
    "var(--color-primary)",
  );
  ...
  expect(
    diagramProps.nodes?.find((node) => node.id === "node_1")?.position,
  ).toEqual({ x: 250, y: 120 });
  fireEvent.click(
    screen.getAllByRole("button", {
      name: "Pin node to highlight connections",
    })[0],
  );
  expect(
    screen.getByRole("button", { name: "Unpin node" }),
  ).toBeInTheDocument();
});
...
it("expands the diagram and returns to the inline view after closing or native dismissal", () => {
  render(<ProductionPlanSection baseId="base" sectionId="plan" />);
  fireEvent.click(
    screen.getByRole("button", { name: "Expand Plate plan diagram" }),
  );
...
it("keeps plan actions independent from diagram collapse", () => {
  ...
  expect(
    screen.queryByRole("region", { name: "Plate plan diagram" }),
  ...
  expect(screen.getByRole("button", { name: "Plate plan" })).toHaveAttribute(
    "aria-expanded",
    "false",
  );
  fireEvent.click(screen.getByRole("button", { name: "Plate plan" }));
  expect(
    screen.getByRole("region", { name: "Plate plan diagram" }),
  ).toBeVisible();
});
...
it("lays out saved plans vertically on narrow screens", () => {
  ...
  expect(diagramProps.nodes?.[1].position.y).toBeGreaterThan(
    diagramProps.nodes?.[0].position.y ?? 0,
  );
});
...
it("clears a pin when the production flow changes even if node IDs are reused", () => {
  ...
  rerender(
    <EmbeddedFlowDiagram
      productionFlow={{
        ...productionFlow,
        nodes: productionFlow.nodes.map((node) => ({
          ...node,
          outputAmount: 120,
        })),
      }}
  ...
  expect(
    screen.queryByRole("button", { name: "Unpin node" }),
  ).not.toBeInTheDocument();
...
it("omits pin controls when diagram interaction is disabled", () => {
  render(
    <EmbeddedFlowDiagram productionFlow={productionFlow} interactive={false} />,
  );
  expect(
    screen.queryByRole("button", { name: "Pin node to highlight connections" }),
  ).not.toBeInTheDocument();
...
it("shows missing inputs connected to their consumers and only marks them red in Advanced mode", () => {
  const flow: ProductionFlowResult = {
    nodes: [productionFlow.nodes[1]],
    edges: [],
    rawMaterialDeficits: [
      { itemId: "ore", required: 60, available: 0, missing: 60 },
    ],
  };
  ...
  expect(diagramProps.edges).toEqual([
  ...
  expect(diagramProps.nodes).toHaveLength(2);
  expect(diagramProps.nodes?.[1].style?.borderColor).not.toBe(
    "var(--color-error)",
  );
});
