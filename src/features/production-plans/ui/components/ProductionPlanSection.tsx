...
const ProductionFlowDiagram: React.FC<ProductionFlowDiagramProps> = ({
  baseId,
  sectionId,
  name,
  targetItemId,
...
export const ProductionPlanSection: React.FC<ProductionPlanSectionProps> = ({
  ...
  const outputSummary =
    base && section ? getPlanOutputAllocationSummary(base, section.id) : null;
  ...
  const statusColor = hasError
    ? "text-error"
    : section.active
      ? allRequirementsSatisfied && !hasMaterialShortage
        ? "text-success"
        : "text-warning"
      : "text-base-content/60";
  const warningLabels = [
    showBuildingWarning && "buildings",
    showMaterialWarning && "materials",
    showInputWarning && "inputs",
  ].filter(Boolean);

  return (
    <section
      className={`min-w-0 rounded-lg border bg-base-100 ${hasError ? "border-error/50" : "border-base-300"}`}
    >
      <header className="rounded-t-lg border-b border-base-300 bg-base-200 p-2 sm:p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="min-w-0 flex-1 basis-full sm:basis-auto">
            <button
              type="button"
              className="flex w-full min-w-0 items-center gap-2 rounded text-left focus-visible:outline-2 focus-visible:outline-primary"
            ...
            >
              <svg
                aria-hidden="true"
                className={`size-4 shrink-0 text-base-content/50 transition-transform ${isCollapsed ? "" : "rotate-90"}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <span className="shrink-0 [&>div]:size-7 [&_img]:size-7">
                <ItemImage
                  itemId={section.selectedItemId}
                  item={item}
                  size="small"
                />
              </span>
              <span className="min-w-0 text-sm font-semibold leading-snug break-words sm:text-base">
                {section.name}
              </span>
              {advanced && (
                <span
                  className={`flex shrink-0 items-center gap-1 text-[11px] font-medium ${statusColor}`}
                >
                  <span
                    aria-hidden="true"
          ...
          <div className="flex flex-wrap items-center gap-1">
            {advanced && showManageButton && (
              <button
                type="button"
                onClick={() => setShowRequirementsModal(true)}
                className="btn btn-sm btn-ghost h-8 min-h-8 px-2 text-xs"
                title="Manage production buildings"
              >
            ...
            {advanced && (
              <button
                type="button"
                className="btn btn-sm btn-outline h-8 min-h-8 px-2 text-xs"
                onClick={section.active ? handleDeactivate : handleActivate}
                disabled={!section.active && hasError}
                title={
                  !section.active && hasError
                    ? "Cannot activate: inputs are insufficient"
                    : undefined
                }
              >
            ...
            <button
              type="button"
              className="btn btn-sm btn-primary h-8 min-h-8 px-2 text-xs"
              onClick={handleEditProductionPlan}
            >
              Edit
            </button>
            <button
              type="button"
              className="btn btn-sm btn-ghost h-8 min-h-8 w-8 p-0 text-base-content/50 hover:text-error"
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-base-content/65 tabular-nums sm:text-xs">
          <span
            ...
            aria-label={`${stats.buildingCount} ${stats.buildingCount === 1 ? "building" : "buildings"}`}
            title="Buildings"
          >
            <SectionIcon name="buildings" className="size-4" />
            {stats.buildingCount}
          ...
          {stats.totalHeat > 0 && (
            <span title="Heat">🔥 {stats.totalHeat}</span>
          )}
          {stats.totalPowerConsumption > 0 && (
            <span title="Power consumption">
              ⚡ {stats.totalPowerConsumption} MW
            </span>
          )}
          {corporationName && (
            <span>
          ...
          {advanced && hasLinkedOutputs && outputSummary && (
            <span
              className={
                outputSummary.remainingRatePerMinute > 0 ? "text-warning" : ""
              }
              title={`${formatRatePerMinute(outputSummary.remainingRatePerMinute)}/min remaining`}
            >
              Outputs {formatRatePerMinute(outputSummary.assignedRatePerMinute)}
              /min assigned
            </span>
          )}
        </div>
      </header>
      {advanced && warningLabels.length > 0 && (
        <details className="border-b border-base-300 px-2 py-1.5 text-xs sm:px-3">
          <summary className="cursor-pointer text-warning">
            Requirements need attention: {warningLabels.join(", ")}
          </summary>
          <ul className="mt-2 space-y-1 pb-1 text-base-content/75">
            {showBuildingWarning && (
              <li>
                Not enough production buildings in base. Use Manage to add them.
              </li>
            )}
            {showMaterialWarning && <li>Missing materials for this plan.</li>}
            {sharedInputShortages.map((shortage) => (
              <li key={shortage.baseBuildingId}>
                Not enough resources from input &quot;{shortage.inputName}&quot;
                ({shortage.itemName}):{" "}
                {formatRatePerMinute(shortage.availablePerMinute)}/min
                available, {formatRatePerMinute(shortage.requiredPerMinute)}/min
