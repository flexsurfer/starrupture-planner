...
const isNarrowScreen = () =>
  window.matchMedia?.(portraitQuery).matches ?? false;
...
export const PlanDiagram = ({
  ...
  return (
    <>
      <div
        className="relative h-[clamp(320px,60dvh,720px)] overflow-hidden rounded-b-lg"
        role="region"
      ...
      >
        ...
        <button
          type="button"
          className="btn btn-sm absolute top-2 right-2 z-10 h-8 min-h-8 gap-1.5 border-base-300 bg-base-200 px-2 text-xs"
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-base-300 bg-base-200 p-2 sm:px-4">
          ...
          <button
            type="button"
            className="btn btn-sm btn-ghost h-8 min-h-8 shrink-0 px-2 text-xs"
