...
export const BuildingRequirementsModal: React.FC<
  BuildingRequirementsModalProps
> = ({
  isOpen,
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {req.linkedOutput && (
                          <div className="text-[11px] text-base-content/60 truncate">
                            {req.linkedOutput.baseName} /{" "}
                            {req.linkedOutput.outputName}
                          </div>
                        )}
                      </div>
                      <span className="text-xs text-base-content/60 flex-shrink-0">
                        {req.ratePerMinute}/min
                      </span>
                    </div>
                    <div className="flex items-center gap-2 ml-2">
                      <span
                        className={`badge badge-sm ${req.isSatisfied ? "badge-success" : "badge-error"}`}
                      >
                        {req.isSatisfied ? "✓" : "✗"}
                      </span>
                    </div>
            ...
            {!allRequirementsSatisfied && (
              <>
                ...
                <p className="text-xs text-base-content/60 text-center">
                  Note: Input buildings must be added manually in the Buildings
                  tab
                </p>
              </>
            )}
