...
import { ProductionPlanSection as ProductionPlanSectionComponent } from "./components";
...
export const BasePlansView: React.FC = () => {
  ...
  const sectionIds =
    useSubscription([appIds.subscriptions.PRODUCTION_PLAN_SECTION_IDS]) || [];
    ...
    <div className="space-y-2 sm:space-y-3">
      ...
      {sectionIds.length === 0 && (
        <div className="rounded-lg border border-dashed border-base-300 px-4 py-8 text-center text-base-content/60">
          <p className="text-sm">
            No production plans yet. Create a production plan to calculate the
            buildings needed to produce items at a specific rate.
          </p>
        </div>
      )}
    </div>
