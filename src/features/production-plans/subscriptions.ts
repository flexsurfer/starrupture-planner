...
const EMPTY_PRODUCTION_FLOW: ProductionFlowResult = {
  nodes: [],
  edges: [],
  rawMaterialDeficits: [],
};
...
): boolean => corporationLevel !== null && corporationLevel !== undefined;

export const registerProductionPlansSubscriptions: UkladModule<
...
> = (registrar) => {
  registrar.regSub(
    ...
    ([selectedBase], ..._params) => {
      void _params;
      return selectedBase
        ? selectedBase.productions.map((section) => section.id)
        : [];
    },
  );
  ...
  registrar.regSub(
    ...
    ([base], _baseId, sectionId) =>
      base && sectionId
        ? base.productions.find((section) => section.id === sectionId) || null
        : null,
  );
  ...
  registrar.regSub(
    ...
    (values) => {
      const [section, buildings, allBases] = values as [
        Production | null,
        DbBuilding[],
        Base[],
      ];
      if (!section || !section.selectedItemId) return EMPTY_PRODUCTION_FLOW;
      const inputBuildings = getFlowInputBuildings(
        section.inputs || [],
        allBases,
      );
      return buildProductionFlow(
        {
          ...
          recipeSelections: sanitizeRecipeSelectionsForInputItems(
            section.recipeSelections,
            inputBuildings,
          ),
        },
        buildings,
  ...
  registrar.regSub(
    ...
    (values) => {
      const [productionFlow, section] = values as [
      ...
      return {
        buildingCount:
          productionFlow.nodes
            .filter((node) => node.nodeType !== "input")
            .reduce((sum, node) => sum + Math.ceil(node.buildingCount), 0) +
          (section?.inputs || []).length,
        totalHeat: productionFlow.nodes.reduce(
          (sum, node) => sum + node.totalHeat,
          0,
        ),
        totalPowerConsumption: productionFlow.nodes.reduce(
          (sum, node) => sum + node.totalPower,
          0,
        ),
      };
    },
  ...
  registrar.regSub(
    ...
    (values) => {
      ...
      const corporationName = section.corporationLevel
        ? corporations.find(
            (corporation) =>
              corporation.id === section.corporationLevel?.corporationId,
          )?.name || null
        : null;
      const requiredBuildings =
        productionFlow.nodes.length > 0
          ? computeRequiredBuildings(productionFlow)
          : section.requiredBuildings || [];
      const sectionInputSnapshots = section.inputs || [];
      const sectionInputs = sectionInputSnapshots.map((input) =>
        resolveInputBuilding(input, allBases),
      );
      let totalHeat = 0;
      ...
      const stats: ProductionPlanSectionStats = {
        buildingCount:
          requiredBuildings.reduce((sum, building) => sum + building.count, 0) +
          sectionInputs.length,
        totalHeat,
        totalPowerConsumption,
      };
      const occupancyFromOtherPlans = buildActivePlanOccupancy(base, {
        excludePlanId: section.id,
      });
      const totalBuildingsMap = new Map<string, number>();
      base.buildings.forEach((baseBuilding) =>
        totalBuildingsMap.set(
          baseBuilding.buildingTypeId,
          (totalBuildingsMap.get(baseBuilding.buildingTypeId) || 0) + 1,
        ),
      );
      ...
      requiredBuildings.forEach(({ buildingId, count }) => {
        ...
        const available = Math.max(
          0,
          total -
            (occupancyFromOtherPlans.occupiedBuildingTypeCounts.get(
              buildingId,
            ) || 0),
        );
      ...
      buildingRequirements.sort((left, right) =>
      ...
      const baseBuildingsById = new Map(
        base.buildings.map((building) => [building.id, building]),
      );
      ...
      sectionInputSnapshots.forEach((inputBuilding: BaseBuilding) => {
        ...
        const itemId =
          ...
          inputBuilding.linkedOutput?.itemIdSnapshot ||
          "";
        const item = itemId
          ? itemsMap[itemId] || { id: itemId, name: itemId, type: "unknown" }
          : null;
        const ratePerMinute =
          resolvedInput.ratePerMinute ||
          inputBuilding.linkedOutput?.ratePerMinuteSnapshot ||
          0;
        ...
        const linkedOutputSourceBuilding = linkedOutputResolution?.sourceOutput
          ...
          : null;
        const linkedOutputStatus =
          resolvedInput.linkedOutputStatus || linkedOutputResolution?.status;
        const isLinkedInputSatisfied = !!(
          inputBuilding.linkedOutput &&
          matchingBaseBuilding &&
          matchingBaseBuilding.sectionType === "inputs" &&
          matchingBaseBuilding.linkedOutput?.baseId ===
            inputBuilding.linkedOutput.baseId &&
          matchingBaseBuilding.linkedOutput?.buildingId ===
        ...
        const isManualInputSatisfied = !!(
          ...
          matchingBaseBuilding &&
          matchingBaseBuilding.sectionType === "inputs" &&
          matchingBaseBuilding.selectedItemId ===
            inputBuilding.selectedItemId &&
          matchingBaseBuilding.ratePerMinute === inputBuilding.ratePerMinute &&
          building &&
          item
        );
        const isSatisfied = inputBuilding.linkedOutput
          ? isLinkedInputSatisfied
          : isManualInputSatisfied;
        ...
        inputRequirements.push({
          ...
          linkedOutput: inputBuilding.linkedOutput
            ? {
                ...
                baseName:
                  linkedOutputResolution?.sourceBase?.name || "Missing base",
                outputName:
      ...
      const sharedInputShortages: SharedInputShortage[] =
        calculateSharedInputShortages(
          ...
          const matchingBaseInput = baseInputBuildingsById.get(
            shortage.baseBuildingId,
          );
          const itemId =
            shortage.itemId || matchingBaseInput?.selectedItemId || "";
          return {
            ...shortage,
            inputName:
              (matchingBaseInput?.name || "").trim() ||
              (itemId ? itemsMap[itemId]?.name || itemId : "Unknown input"),
            itemId,
            itemName: itemId
              ? itemsMap[itemId]?.name || itemId
              : "Unknown input",
          };
        });
      const hasRawMaterialShortage =
        (productionFlow.rawMaterialDeficits || []).length > 0;
      const hasMaterialShortage =
        sharedInputShortages.length > 0 || hasRawMaterialShortage;
      const planStatus =
        section.status || (section.active ? "active" : "inactive");
      return {
        ...
        planStatus,
        hasError: planStatus === "error" || !allInputsSatisfied,
        showManageButton:
          buildingRequirements.length > 0 || inputRequirements.length > 0,
      };
    },
  );
  ...
  registrar.regSub(
    ...
    (values): ProductionPlanRequirementsStatus => {
