...
import { getSectionTypeForBuilding } from "@/features/bases/building-section";
...
function buildAvailableBuildingCountByType(
  ...
  for (const baseBuilding of base.buildings) {
    totals.set(
      ...
      (totals.get(baseBuilding.buildingTypeId) || 0) + 1,
    );
  }
  ...
  totals.forEach((totalCount, buildingTypeId) => {
    available.set(
      buildingTypeId,
      Math.max(0, totalCount - (occupied.get(buildingTypeId) || 0)),
    );
  });
  return available;
}

export const registerProductionPlansEvents: UkladModule<
  UkladRegistrar<AppContracts>
> = (registrar) => {
  registrar.regEvent(
    appIds.events.PRODUCTION_PLAN_ACTIVATE_SECTION,
    ({ draftState }, baseId, sectionId) => {
      const section = getBaseById(
        draftState.basesList,
        baseId,
      )?.productions.find((plan) => plan.id === sectionId);
      if (section) {
        section.active = true;
  ...
  registrar.regEvent(
    ...
    ({ draftState }, baseId, planId, mode) => {
      ...
      const planInputBuildings = getFlowInputBuildings(
        plan.inputs || [],
        draftState.basesList,
      );
      ...
      for (const { buildingId, count: requiredCount } of requiredBuildings) {
        ...
        const existingCount =
          mode === "missing" ? existingCountByType.get(buildingId) || 0 : 0;
        const countToAdd =
          mode === "missing"
      ...
      for (const { buildingId, count } of buildingCountsToAdd) {
        ...
        for (let index = 0; index < count; index += 1) {
          ...
          if (
            buildingId === ORBITAL_CARGO_LAUNCHER_BUILDING_ID &&
            plan.selectedItemId
          ) {
