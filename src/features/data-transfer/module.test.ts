...
function setup() {
  ...
  runtime.registerModule(
    createHeadlessEffects({ onExport: (archive) => exports.push(archive) }),
  );
...
describe("archive events and persistence", () => {
  it("previews before import, cancels, then imports repeated copies without changing existing work", async () => {
    ...
    try {
      ...
      expect(state.plannerTabs.slice(0, 2)).toEqual(
        archiveFixture().plannerTabs,
      );
  ...
  it("imports only selected entries, detaches omitted bases, and appends Copy without changing the originals", async () => {
    ...
    try {
      ...
      harness.dispatchSync([
        appIds.events.DATA_TRANSFER_CONFIRM_IMPORT,
        {
          baseIds: [preview.bases[1].id],
          planIds: [preview.plans[1].id],
        },
      ]);
      const state = harness.getState();
      ...
      for (const input of [
        imported.buildings[0],
        imported.productions[0].inputs![0],
      ]) {
  ...
  it("keeps links between selected bases and omits energy groups when importing only plans", async () => {
    ...
    try {
      ...
      harness.dispatchSync([
        appIds.events.DATA_TRANSFER_CONFIRM_IMPORT,
        { baseIds: [], planIds: [preview.plans[0].id] },
      ]);
