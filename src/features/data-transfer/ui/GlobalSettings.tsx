...
import { useTranslation, translateText, message, UiMessageError } from '@/shared/i18n';
...
export function GlobalSettingsButton({ onClick }: { onClick: () => void }) {
    ...
    return <button type="button" className="btn btn-ghost btn-sm btn-square" title={t("Global settings")} aria-label={t("Global settings")} onClick={onClick}>
        <SettingsIcon />
    </button>;
}

function ArchiveChecklist({ title, selectAllLabel, emptyLabel, entries, selected, onChange }: {
    title: string;
    ...
    return <fieldset className="min-w-0 rounded-lg border border-base-300 p-3">
        ...
        <label className="mb-2 flex cursor-pointer items-center gap-2 text-sm">
            <input ref={checkbox} type="checkbox" className="checkbox checkbox-sm rounded-none border-base-content/30 shadow-none checked:border-base-content/60 checked:bg-base-content/10 checked:text-base-content indeterminate:bg-base-content/10" checked={all} disabled={!entries.length}
                onChange={event => onChange(event.target.checked ? entries.map(entry => entry.id) : [])} />{selectAllLabel}</label>
        <div className="max-h-48 space-y-1 overflow-y-auto">
            ...
            {!entries.length && <p className="py-2 text-xs text-base-content/60">{emptyLabel}</p>}
        </div>
    </fieldset>;
}
...
function ImportPreview({ archive, dataVersion }: { archive: PlannerArchive; dataVersion: string }) {
    ...
    return <section aria-label={t("Import preview")} className="mt-3 rounded-lg border border-base-300 p-3">
        ...
        {archive.dataVersion !== dataVersion && <p className="mt-3 text-sm text-warning" role="status">{t("This file uses game data “{dataVersion}”; you are using “{dataVersion2}”. Some items or recipes may be unavailable. You can change the game version in the header.", { dataVersion: archive.dataVersion, dataVersion2: dataVersion })}</p>}
        <div className="mt-3 flex flex-wrap justify-end gap-2">
            <button type="button" className="btn btn-sm btn-ghost" onClick={() => runtime.dispatch([appIds.events.DATA_TRANSFER_CANCEL_IMPORT])}>{t("Cancel import")}</button>
            <button type="button" className="btn btn-sm btn-primary" disabled={!selectedCount}
                onClick={() => runtime.dispatch([appIds.events.DATA_TRANSFER_CONFIRM_IMPORT, { baseIds, planIds }])}>{t("Import selected ({selectedCount})", { selectedCount: selectedCount })}</button>
        </div>
    </section>;
}
    ...
    return <dialog ref={dialog} className="modal" aria-labelledby={titleId} onCancel={close}
        ...
        <div className="modal-box max-w-2xl">
            <div className="mb-5 flex items-center justify-between gap-3">
                <h2 id={titleId} className="text-xl font-semibold">{t("Global settings")}</h2>
                <button type="button" className="btn btn-sm btn-ghost btn-square" aria-label={t("Close settings")} onClick={close}>✕</button>
            </div>
            <section className="mb-5 border-b border-base-300 pb-5" aria-labelledby={`${titleId}-appearance`}>
                <h3 id={`${titleId}-appearance`} className="mb-3 font-semibold">{t("Appearance")}</h3>
                <label className="mb-3 flex items-center justify-between gap-4 text-sm">
                    {t("Language")}
                    <select className="select select-sm select-bordered w-32" value={locale}
                        onChange={event => runtime.dispatch([appIds.events.UI_SET_LOCALE, event.target.value])}>
                        {Object.entries(locales).map(([code, language]) => <option key={code} value={code} lang={code}>{language.name}</option>)}
                    </select>
                </label>
                <label className="flex items-center justify-between gap-4 text-sm">{t("Theme")}<select className="select select-sm select-bordered w-32" value={theme}
                        onChange={event => {
            ...
            <section aria-labelledby={`${titleId}-export`}>
                <h3 id={`${titleId}-export`} className="font-semibold">{t("Export bases and plans")}</h3>
                <p className="mt-1 mb-4 text-sm text-base-content/65">{t("Save a JSON file to back up or share your work. Bases include their buildings, production plans, and energy groups. Planner plans are your saved planner tabs.")}</p>
                <div className="grid gap-3 sm:grid-cols-2">
                    <ArchiveChecklist title={t("Bases")} selectAllLabel={t("Select all bases")} emptyLabel={t("No bases yet.")} entries={bases} selected={baseIds} onChange={setBaseIds} />
                    <ArchiveChecklist title={t("Planner plans")} selectAllLabel={t("Select all planner plans")} emptyLabel={t("No planner plans yet.")} entries={plans} selected={planIds} onChange={setPlanIds} />
                </div>
                <div className="mt-3 flex flex-wrap justify-end gap-2">
                    <button type="button" className="btn btn-sm btn-outline" disabled={!bases.length && !plans.length}
                        onClick={() => runtime.dispatch([appIds.events.DATA_TRANSFER_EXPORT, { baseIds: null, planIds: null }])}>{t("Export all")}</button>
                    <button type="button" className="btn btn-sm btn-primary" disabled={!selectedCount}
                        onClick={() => runtime.dispatch([appIds.events.DATA_TRANSFER_EXPORT, { baseIds, planIds }])}>{t("Export selected ({selectedCount})", { selectedCount: selectedCount })}</button>
                </div>
            </section>
            <section className="mt-6 border-t border-base-300 pt-5" aria-labelledby={`${titleId}-import`}>
                <h3 id={`${titleId}-import`} className="font-semibold">{t("Import bases and plans")}</h3>
                <p className="mt-1 mb-3 text-sm text-base-content/65">{t("Imports add new copies and keep your existing work. Links between selected bases stay connected. Links to bases you leave out become standalone inputs with their saved item and rate.")}</p>
                <label className="flex flex-col gap-2 text-sm">{t("Choose a Rupture Planner export")}<input type="file" accept=".json,application/json" className="file-input file-input-bordered w-full" disabled={reading}
                        onChange={async event => {
                            ...
                            setReading(true);
                            try {
                                if (file.size > MAX_ARCHIVE_BYTES) throw new UiMessageError('Choose an export smaller than 20 MB.');
                                ...
                                if (mounted.current) runtime.dispatch([appIds.events.DATA_TRANSFER_SET_STATUS, {
                                    kind: 'error', message: error instanceof UiMessageError ? error.uiMessage : message('Could not read the file.'),
                                }]);
                            } finally { if (mounted.current) setReading(false); }
                        }} />
                </label>
                {reading && <p role="status" className="mt-2 text-sm">{t("Reading export…")}</p>}
                {preview && <ImportPreview key={`${preview.bases[0]?.id ?? ''}:${preview.plans[0]?.id ?? ''}`} archive={preview} dataVersion={dataVersion} />}
            </section>
            {status && <p role={status.kind === 'error' ? 'alert' : 'status'} className={`mt-4 rounded-md p-3 text-sm ${status.kind === 'error' ? 'bg-error/10 text-error' : 'bg-success/10 text-success'}`}>{translateText(t, status.message)}</p>}
        </div>
    </dialog>;
}
