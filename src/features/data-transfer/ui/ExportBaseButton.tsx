...
export function ExportBaseButton({ baseId, name, iconOnly = false }: { baseId: string; name: string; iconOnly?: boolean }) {
    ...
    return <button type="button" className={`btn btn-sm btn-ghost shrink-0 ${iconOnly ? "btn-square h-8 min-h-8 w-8 p-0 text-base-content/35 hover:text-base-content/65" : "gap-1 px-2 text-xs"}`} aria-label={`Export ${name}`} title="Export base"
