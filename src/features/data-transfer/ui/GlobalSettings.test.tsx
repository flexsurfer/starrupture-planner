...
vi.mock('@/platform/web/archive-transfer', () => ({ downloadArchive: vi.fn(), MAX_ARCHIVE_BYTES: 20 * 1024 * 1024 }));
...
function setup(empty = false) {
    ...
    render(<UkladProvider runtime={runtime}><GlobalSettings onClose={() => {}} /><ExportBaseButton baseId="source" name="Smelting" /></UkladProvider>);
    return harness;
}
function upload(text: string) {
    const file = new File([text], 'planner.json', { type: 'application/json' });
    Object.defineProperty(file, 'text', { value: async () => text });
    fireEvent.change(screen.getByLabelText('Choose a Rupture Planner export'), { target: { files: [file] } });
}

it('exports all, selected collections, and one base through the browser adapter', async () => {
    setup();
    fireEvent.click(screen.getByRole('checkbox', { name: 'Select all planner plans' }));
    ...
    expect(vi.mocked(downloadArchive).mock.calls[0][0]).toMatchObject({ bases: [{ id: 'target' }], plans: [] });
    ...
    expect(vi.mocked(downloadArchive).mock.calls[2][0]).toMatchObject({ bases: [{ id: 'source' }], plans: [] });
});

it('previews the file, warns about a different game version, and imports only after confirmation', async () => {
    const harness = setup();
    const archive = createArchive(archiveFixture(), { baseIds: ['source'], planIds: ['multi'] });
...
it('keeps import available with no saved data and reports invalid files without changes', async () => {
    ...
    expect(screen.getByLabelText('Choose a Rupture Planner export')).toBeEnabled();
    upload('{bad file');
...
it('lets users select import entries independently of export and disables an empty import', async () => {
    const harness = setup();
    upload(JSON.stringify(createArchive(archiveFixture(), { baseIds: null, planIds: null })));
    const preview = within(await screen.findByRole('region', { name: 'Import preview' }));
    expect(preview.getByRole('button', { name: 'Import selected (4)' })).toBeEnabled();
    fireEvent.click(preview.getByRole('checkbox', { name: 'Select all bases' }));
    fireEvent.click(preview.getByRole('checkbox', { name: 'Select all planner plans' }));
    expect(preview.getByRole('button', { name: 'Import selected (0)' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Export selected (4)' })).toBeEnabled();
...
it('resets import selections when another file is chosen', async () => {
    ...
    fireEvent.click(preview.getByRole('checkbox', { name: 'Select all planner plans' }));
    expect(preview.getByRole('button', { name: 'Import selected (0)' })).toBeDisabled();
    upload(JSON.stringify(createArchive(archiveFixture(), { baseIds: ['source'], planIds: [] })));
    expect(await screen.findByRole('button', { name: 'Import selected (1)' })).toBeEnabled();
    const nextPreview = within(screen.getByRole('region', { name: 'Import preview' }));
    expect(nextPreview.getByRole('checkbox', { name: 'Smelting' })).toBeChecked();
    expect(nextPreview.queryByRole('checkbox', { name: 'Assembly' })).not.toBeInTheDocument();
});
