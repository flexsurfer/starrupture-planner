...
export function createArchive(
  ...
  const bases = copy(
    state.basesList.filter(
      (base) =>
        selection.baseIds === null || selection.baseIds.includes(base.id),
    ),
  );
  for (const base of bases) {
    for (const building of [
      ...base.buildings,
      ...base.productions.flatMap((plan) => plan.inputs ?? []),
    ]) {
  ...
  return {
    format: "rupture-planner",
    version: 1,
    dataVersion: state.appDataVersion,
    basesMode: state.basesMode,
    bases,
    plans: copy(
      state.plannerTabs.filter(
        (plan) =>
          selection.planIds === null || selection.planIds.includes(plan.id),
      ),
    ),
    energyGroups: copy(
      state.energyGroups.filter((group) => groupIds.has(group.id)),
    ),
  };
}

/** Select from a prepared preview without changing its IDs or runtime-owned data. */
export function selectArchiveImports(
  archive: PlannerArchive,
  selection: ArchiveSelection,
): PlannerArchive {
  const bases = copy(
    archive.bases.filter(
      (base) =>
        selection.baseIds === null || selection.baseIds.includes(base.id),
    ),
  );
  ...
  for (const base of bases) {
    ...
    for (const input of [
      ...
      if (input.linkedOutput && !includedBaseIds.has(input.linkedOutput.baseId))
        detachLinkedOutput(input);
    }
  }
  ...
  return {
    ...
    plans,
    energyGroups: copy(
      archive.energyGroups.filter((group) => groupIds.has(group.id)),
    ),
  };
}
...
function detachLinkedOutput(input: BaseBuilding) {
  ...
  input.selectedItemId =
    input.linkedOutput.itemIdSnapshot ?? input.selectedItemId;
  input.ratePerMinute =
    input.linkedOutput.ratePerMinuteSnapshot ?? input.ratePerMinute;
  delete input.linkedOutput;
}

function check(
  condition: unknown,
  message = "The file contains invalid or incomplete planner data.",
): asserts condition {
...
function ids(value: unknown): asserts value is Record<string, unknown>[] {
  ...
  for (const entry of value) {
    object(entry);
    check(
      string(entry.id) && !seen.has(entry.id),
      "The file contains missing or duplicate IDs.",
    );
    seen.add(entry.id);
  }
}
...
function building(value: unknown) {
  ...
  optional(value, "allocationMode", (item) =>
    check(item === "auto" || item === "fixed"),
  );
...
export function parseArchive(text: string): PlannerArchive {
  check(
    text.length <= MAX_ARCHIVE_BYTES,
    "Choose an export smaller than 20 MB.",
  );
  let value: unknown;
  try {
    value = JSON.parse(text.replace(/^\uFEFF/, ""), (key, entry: unknown) => {
      check(
        ...
        "The file contains an unsupported property.",
      );
      return entry;
    });
  } catch (error) {
    if (error instanceof SyntaxError)
      throw new Error(
        "This is not a valid JSON file. Choose a Rupture Planner export.",
      );
    throw error;
  }
  object(value);
  check(
    value.format === "rupture-planner",
    "Choose a file exported from Rupture Planner.",
  );
  check(
    value.version === 1,
    "This export version is not supported. Update Rupture Planner and try again.",
  );
  check(string(value.dataVersion));
  check(
    value.basesMode === null ||
      value.basesMode === "planning" ||
      value.basesMode === "advanced",
  );
  ids(value.bases);
  ids(value.plans);
  ids(value.energyGroups);
  check(
    value.bases.length + value.plans.length > 0,
    "This file contains no bases or planner plans.",
  );
  for (const group of value.energyGroups) check(string(group.name));
  for (const base of value.bases) {
    check(string(base.name));
    optional(base, "coreLevel", (level) =>
      check(
        number(level) && Number.isInteger(level) && level >= 0 && level <= 7,
      ),
    );
    ...
    for (const plan of base.productions) {
      check(
        string(plan.name) &&
          string(plan.selectedItemId) &&
          positive(plan.targetAmount),
      );
  ...
  for (const plan of value.plans) {
    check(
      string(plan.name) && (plan.mode === "single" || plan.mode === "multi"),
    );
    ...
    check(
      typeof plan.flowDirection === "string" &&
        ["LR", "RL", "TB", "BT"].includes(plan.flowDirection),
    );
...
export function prepareArchiveImport(
  archive: PlannerArchive,
  prefix: string,
): PlannerArchive {
  ...
  const groupIds = new Map(
    result.energyGroups.map((group) => [group.id, nextId()]),
  );
  ...
  for (const base of result.bases) {
    const planIds = new Map(
      base.productions.map((plan) => [plan.id, nextId()]),
    );
    const remapBuilding = (entry: BaseBuilding) => {
      entry.id = buildingIds.get(base.id)!.get(entry.id)!;
      if (entry.sourceProductionId)
        entry.sourceProductionId = planIds.get(entry.sourceProductionId);
      if (entry.planningOwnerPlanId)
        entry.planningOwnerPlanId = planIds.get(entry.planningOwnerPlanId);
      if (!entry.linkedOutput) return;
      const reference = entry.linkedOutput;
      const sourceBase = archive.bases.find(
        (source) => source.id === reference.baseId,
      );
      const outputExists = sourceBase?.buildings.some(
        (output) =>
          output.id === reference.buildingId &&
          output.sectionType === "outputs",
      );
      if (outputExists) {
        reference.buildingId = buildingIds
          .get(reference.baseId)!
          .get(reference.buildingId)!;
    ...
    if (base.energyGroupId)
      base.energyGroupId = groupIds.get(base.energyGroupId);
    base.id = baseIds.get(base.id)!;
  }
  result.energyGroups.forEach((group) => {
