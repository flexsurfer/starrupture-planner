...
import {
  ...
  extractCategories,
  parseCorporations,
} from "@/features/app-shell/game-data";
import {
...
export const registerAppShellEvents: UkladModule<
...
> = (registrar) => {
  ...
  registrar.regEvent(
    ...
    ({ draftState }, title, message, onConfirm, options) => {
  ...
  registrar.regEvent(
    ...
    ({ draftState }, version) => {
      if (
        version === draftState.appDataVersion ||
        draftState.uiGameDataLoadPending
      )
        return;
