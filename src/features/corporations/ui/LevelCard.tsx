...
export const LevelCard = ({
  ...
  return (
    <section
      ...
      className="min-w-0 overflow-hidden rounded-lg border border-base-300 bg-base-200"
    >
      ...
      {components.length > 0 && (
        <div className="p-2 sm:p-3">
          <p className="mb-1.5 text-[10px] font-medium text-base-content/60 sm:text-xs">
            {components.length > 1
              ? "Supply options · choose one to plan"
              : "Supply item"}
          </p>
          <div
            className={`grid gap-1.5 ${components.length > 1 ? "xl:grid-cols-2" : ""}`}
          >
            {components.map((component, index) => (
              ...
              >
                ...
                <span
                  className="ml-auto shrink-0 text-[10px] text-base-content/50"
                  aria-hidden="true"
                >
                  Plan →
                </span>
      ...
      {rewards.length > 0 && (
        <div className="border-t border-base-300 px-2 py-2 sm:px-3">
          <h4 className="mb-1.5 text-[10px] font-medium text-base-content/60 sm:text-xs">
            Rewards
          </h4>
          <ul className="flex flex-wrap gap-1">
            {rewards.map((reward, index) => (
              <li
                key={index}
                className="max-w-full rounded border border-transparent bg-base-content/5 px-1.5 py-1 text-[11px] leading-snug text-base-content/85 break-words sm:text-xs"
              >
                {reward.name}
              </li>
            ))}
