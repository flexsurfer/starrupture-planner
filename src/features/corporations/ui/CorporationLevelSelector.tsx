 ...
 */
export const CorporationLevelSelector: React.FC<
  CorporationLevelSelectorProps
> = ({
  corporationLevels,
  selectedLevel,
  onChange,
  ...
  const selectedLevelInfo = selectedLevel
    ? corporationLevels.find(
        (l) =>
          l.corporationId === selectedLevel.corporationId &&
          l.level === selectedLevel.level,
      )
    : null;
          ...
          <div className="absolute top-full left-0 right-0 mt-1 z-20 bg-base-200 border border-base-300 rounded-lg shadow-lg max-h-60 overflow-auto">
            <ul className="menu p-3 gap-1">
              ...
              {corporationLevels.map((levelInfo) => {
                const isSelected =
                  selectedLevel?.corporationId === levelInfo.corporationId &&
