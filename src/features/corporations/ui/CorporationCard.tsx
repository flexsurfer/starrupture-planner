...
export const CorporationCard = ({
  ...
  const totalRewards = corporation.levels.reduce(
    (sum, level) => sum + level.rewards.length,
    0,
  );
    ...
    <ExpandableSection
      ...
      summary={
        <>
          ...
          {totalRewards > 0 && (
            <span>
              {totalRewards} {totalRewards === 1 ? "reward" : "rewards"}
            </span>
          )}
          {totalCost > 0 && (
            <span className="text-info">{totalCost.toLocaleString()} G</span>
          )}
        </>
      }
    >
      {corporation.description && (
        <p className="mb-3 text-xs leading-relaxed text-base-content/65 sm:text-sm">
          {corporation.description}
        </p>
      )}
