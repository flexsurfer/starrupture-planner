...
export type OutputResolutionStatus =
  "manual" | "allocated" | "missing-plan" | "unconfigured-output";
...
export function getDefaultOutputCapacityPerMinute(
...
export function resolveOutputCapacityPerMinute(
  output: BaseBuilding,
): number | undefined {
...
export function resolveOutputBuilding(
  output: BaseBuilding,
  sourceBase: Base | undefined,
): ResolvedOutputBuilding {
  if (!output.sourceProductionId) {
    return {
      ...output,
      outputResolutionStatus:
        output.selectedItemId && isPositiveNumber(output.ratePerMinute)
          ? "manual"
          : "unconfigured-output",
  ...
  const sourceProduction = sourceBase?.productions.find(
    (plan) => plan.id === output.sourceProductionId,
  );
  ...
  const producedRatePerMinute =
    sourceProduction.targetAmount > 0 ? sourceProduction.targetAmount : 0;
  let remaining = producedRatePerMinute;

  for (const candidate of getPlanLinkedOutputs(
    sourceBase,
    sourceProduction.id,
  )) {
    const requested = resolveRequestedRate(candidate, remaining);
    const capacity = resolveOutputCapacityPerMinute(candidate);
    const capacityLimit = isPositiveNumber(capacity)
      ? capacity
      : Number.POSITIVE_INFINITY;
    const effective = Math.max(
      0,
      Math.min(requested, capacityLimit, remaining),
    );
    ...
    if (candidate.id === output.id) {
      return {
        ...
        capacityPerMinuteResolved: isPositiveNumber(capacity)
          ? capacity
          : undefined,
...
export function getPlanOutputAllocationSummary(
  base: Base,
  planId: string,
): {
  producedRatePerMinute: number;
  ...
  const outputs = getPlanLinkedOutputs(base, planId).map((output) =>
    resolveOutputBuilding(output, base),
  );
  const assignedRatePerMinute = outputs.reduce(
    (sum, output) => sum + (output.effectiveRatePerMinute || 0),
    0,
  );
  ...
  return {
    ...
    remainingRatePerMinute: Math.max(
      0,
      producedRatePerMinute - assignedRatePerMinute,
    ),
    outputs,
  };
}

export function clearOutputPlanLinksForProduction(
  base: Base,
  planId: string,
): number {
  let clearedCount = 0;
