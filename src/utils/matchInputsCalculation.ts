 ...
 */
export function calculateMaxTargetFromInputs(
  params: MatchInputsParams,
): number | null {
  const {
    selectedItemId,
    inputBuildings,
  ...
  const probeFlow = buildProductionFlow(
    { ...flowParams, targetAmount: 1 },
    buildings,
  );
  if (!probeFlow.nodes.some((n) => n.nodeType === "input")) return null;

  const T_BASE = 0.001;
  const baselineFlow = buildProductionFlow(
    { ...flowParams, targetAmount: T_BASE },
    buildings,
  );
  ...
  const result = !isOverCapacity(rounded)
    ? rounded
