...
export type LinkedOutputStatus =
  | "ok"
  | "missing-base"
  | "missing-output"
  | "missing-plan"
...
export function getProductionInputIds(
  inputs: BaseBuilding[] | undefined,
): string[] {
...
export function resolveLinkedOutput(
  ...
  const sourceOutput = sourceBase.buildings.find(
    (building) => building.id === reference.buildingId,
  );
  ...
  if (
    ...
    reference.itemIdSnapshot &&
    resolvedSourceOutput.selectedItemId &&
    reference.itemIdSnapshot !== resolvedSourceOutput.selectedItemId
  ) {
    return {
      status: "item-changed",
      sourceBase,
...
export function resolveInputBuilding(
  input: BaseBuilding,
  bases: BaseLookup,
): ResolvedInputBuilding {
...
export function getInputAvailabilityKey(
  input: BaseBuilding,
  baseId: string,
): string {
...
export function getFlowInputBuildings(
  inputs: BaseBuilding[] | undefined,
  bases?: BaseLookup,
): BaseBuilding[] {
  if (!inputs || inputs.length === 0) return [];
...
export function getSelectedFlowInputBuildings(
  ...
  return getFlowInputBuildings(
    base.buildings.filter((building) => selectedIds.has(building.id)),
    allBases,
  );
}

export function computeRequiredBuildings(
