...
describe("headless catalog and planner E2E", () => {
  ...
  it("emulates category, building, name/id search, empty-result, and reset filters", async () => {
    ...
    expect(view.value("filtered").map(({ id }) => id)).toEqual([
      "copper-ore",
      "iron-ore",
    ]);
  ...
  it("emulates catalog joins, special building queries, sorting, and corporation summaries", async () => {
    ...
    expect(view.value("categories")).toEqual([
      "all",
      "raw",
      "processed",
      "component",
    ...
    expect(view.value("smelterItems").map(({ id }) => id)).toEqual([
      "iron-plate",
    ]);
    expect(view.value("missingItems")).toEqual([]);
    expect(
      view.value("ironRecipes").map(({ building }) => building.id),
    ).toEqual(["smelter", "smelter_mk2"]);
    expect(view.value("unknownRecipes")).toEqual([]);
    expect(
      view.value("plateRecipes").map(({ building }) => building.id),
    ).toEqual(["smelter", "smelter_mk2"]);
    expect(view.value("unknownOutputRecipes")).toEqual([]);

    const ironRow = view
      .value("tableRows")
      .find(({ item }) => item.id === "iron-plate");
    expect(ironRow).toMatchObject({
      producingBuildings: ["Smelter", "Smelter Mk.2"],
      corporationUsage: [{ corporation: "Miners", level: 1 }],
    });
    expect(view.value("helpers").corporationNameToId.get("Engineers")).toBe(
      "engineers",
    );
    expect(
      view.value("helpers").buildingCorporationUsage.get("Smelter"),
    ).toEqual([{ corporation: "Miners", level: 1 }]);

    const sortedBuildingIds = view.value("sortedBuildings").map(({ id }) => id);
    expect(sortedBuildingIds.indexOf("smelter_mk2")).toBe(
      sortedBuildingIds.indexOf("smelter") + 1,
    );
    expect(sortedBuildingIds.indexOf("smelter")).toBeLessThan(
      sortedBuildingIds.indexOf("assembler"),
    );
    expect(view.value("corporations")).toMatchObject([
      ...
      {
        name: "Engineers",
        stats: { totalLevels: 1, totalComponents: 1, totalCost: 0 },
      },
    ]);
  ...
  it("keeps every production building visible when upgrade metadata forms a cycle", async () => {
    ...
    expect(ids).toHaveLength(
      cyclicUpgradeData.buildings.filter(({ type }) => type === "production")
        .length,
    );
  ...
  it("emulates planner defaults, alternatives, corporation mode, graphs, stats, and resets", async () => {
    ...
    expect(
      view
        ...
        .nodes.some(
          (node) =>
            node.nodeType === "launcher" &&
            node.buildingId === "orbital_cargo_launcher",
        ),
    ).toBe(true);
    ...
    expect(view.value("corporationLevel")).toEqual({
