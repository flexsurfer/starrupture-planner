...
describe("headless production planning E2E", () => {
  ...
  it("emulates form validation, input matching, recipe sanitation, creation, reset, and editing", async () => {
    ...
    expect(autosavedPlan).toMatchObject({
      name: "Iron line",
      targetAmount: 60,
    });
    await app.dispatch([
    ...
    expect(modal.value("levels")).toMatchObject([
      { corporationId: "miners", level: 1 },
    ]);
    await app.dispatch([
      appIds.events.PRODUCTION_PLAN_MODAL_SET_SELECTED_ITEM,
    ...
    expect(modal.value("state").recipeSelections).toEqual({
      "iron-plate": "smelter:0",
    });
    ...
    expect(base.value("plans")).toMatchObject([
      { id: plan.id, name: "Iron line" },
    ]);
  ...
  it("emulates cross-base linkable outputs, fallback input selection, and idempotent linking", async () => {
    ...
    let linkedInputs = bases
      .value("selected")!
      .buildings.filter(
        ({ linkedOutput }) => linkedOutput?.buildingId === sourceOutputId,
      );
    ...
    expect(modal.value("state").selectedInputIds).toEqual([
      linkedInputs[0]!.id,
    ]);
  });
  ...
  it("emulates section requirements, activation, missing/all provisioning, input failure, and deletion", async () => {
    ...
    expect(planView.value("coverage")).toMatchObject([
      { buildingId: "smelter", missing: 1 },
    ]);
    ...
    expect(
      base
        .value("selected")!
        .buildings.filter(({ buildingTypeId }) => buildingTypeId === "smelter"),
    ).toHaveLength(2);
    await app.dispatch([
      appIds.events.PRODUCTION_PLAN_ADD_BUILDINGS_TO_BASE,
      baseId,
    ...
    expect(planView.value("entity")).toMatchObject({
      active: true,
      status: "active",
    });
    expect(planView.value("model")?.planStatus).toBe("active");
    expect(planView.value("productionBuildings")).toEqual(
    ...
    expect(planView.value("inputBuildings")).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: inputId,
          activePlanNames: ["Iron requirements"],
  ...
  it("emulates shared-input shortages and corporation launcher provisioning across plans", async () => {
    ...
    expect(base.value("planRows").map(({ name }) => name)).toEqual([
      "Plan A",
      "Plan B",
    ]);
    ...
    expect(base.value("selected")!.buildings).toEqual(
      expect.arrayContaining([
  ...
  it("emulates prioritized plan-output allocation, normalization, unlinking, and cleanup", async () => {
    ...
    const [firstOutput, secondOutput] = base
      .value("selected")!
      .buildings.filter(({ sectionType }) => sectionType === "outputs");
    await app.dispatch([
    ...
    expect(outputs.value("configured")).toMatchObject([
      {
        baseBuildingId: firstOutput!.id,
        item: { id: "iron-plate" },
        ratePerMinute: 50,
      },
      {
        baseBuildingId: secondOutput!.id,
        item: { id: "iron-plate" },
        ratePerMinute: 70,
      },
    ]);
    expect(outputs.value("logistics")?.outputs).toMatchObject([
      ...
      },
      {
        baseBuildingId: secondOutput!.id,
        ratePerMinute: 70,
        availableCapacityPerMinute: 130,
      },
    ]);
    ...
    expect(
      base
        .value("selected")!
        .buildings.find(({ id }) => id === firstOutput!.id),
    ).not.toMatchObject({
    ...
    expect(
      base.value("selected")!.buildings.find(({ id }) => id === firstOutput!.id)
        ?.sourceProductionId,
    ).toBeUndefined();
    ...
    expect(
      base
        .value("selected")!
        .buildings.find(({ id }) => id === secondOutput!.id),
    ).not.toMatchObject({
      sourceProductionId: expect.anything(),
      allocationMode: expect.anything(),
