...
describe("headless application shell E2E", () => {
  ...
  it("emulates defaults, cached data-version views, navigation, theme, and dialogs", async () => {
    ...
    const app = createApp({
      application: {
        effects: { onThemeChange: (theme) => themes.push(theme) },
      },
    });
    ...
    expect(shell.value("buildings")).toHaveLength(
      TEST_GAME_DATA.buildings.length,
    );
    expect(shell.value("corporations").map(({ name }) => name)).toEqual([
      "Miners",
      "Engineers",
    ]);
  ...
  it("reports failed loads through the default headless error adapter", async () => {
    ...
    try {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        "Failed to load bundled game data:",
        loadError,
      );
    } finally {
