...
import { createHeadlessE2EApp, type HeadlessE2EApp } from "./e2e-support";
...
describe("headless bases and logistics E2E", () => {
  ...
  it("emulates multi-base creation, selection, tabs, collapsed defaults, editing, and deletion", async () => {
    ...
    expect(root.value("list").map(({ name }) => name)).toEqual([
      "Alpha",
      "Beta",
    ]);
    ...
    expect(root.current()).toMatchObject({
      selectedId: alphaId,
      selectedTab: "base",
    });
  ...
  it("emulates building sections, bulk normalization, count reconciliation, stats, and removal guards", async () => {
    ...
    expect(sections.value("defense")).toMatchObject([
    ...
    expect(sections.value("productionStats")).toMatchObject({
      buildingCount: 1,
      totalHeat: 5,
    });
    ...
    expect(sections.value("productionChoices").map(({ id }) => id)).toContain(
      "smelter",
    );
    expect(sections.value("inputChoices").map(({ id }) => id)).toContain(
      "ore_excavator",
    );
    expect(sections.value("energyChoices").map(({ id }) => id)).toContain(
      "power_generator",
    );
    expect(sections.value("outputChoices").map(({ id }) => id)).toContain(
      "package_dispatcher",
    );
    expect(
      sections.value("infrastructureChoices").map(({ id }) => id),
    ).toContain("turret");

    expect(root.value("summary")).toMatchObject({
      totalBases: 1,
      totalBuildings: 6,
    });
    ...
    const smelterId = root
      .value("selected")!
      .buildings.find(({ buildingTypeId }) => buildingTypeId === "smelter")!.id;
    ...
    expect(
      root.value("selected")!.buildings.some(({ id }) => id === smelterId),
    ).toBe(true);
    await app.dispatch([appIds.events.BASES_REMOVE_BUILDING, smelterId]);
  ...
  it("emulates cross-base manual outputs, exclusive input links, relinking, and broken-link views", async () => {
    ...
    expect(detail.value("sourceLogistics")).toMatchObject({
      outputs: [
        {
          baseBuildingId: outputId,
          linkedInputs: [{ baseBuildingId: firstInputId }],
        },
      ],
    });
    ...
    const secondInputId = targetBase.buildings.find(
      ({ name }) => name === "Replacement intake",
    )!.id;
    await app.dispatch([
      appIds.events.BASES_UPDATE_BUILDING_LINKED_OUTPUT,
      targetId,
    ...
    targetBase = root.value("selected")!;
    expect(
      targetBase.buildings.find(({ id }) => id === firstInputId)?.linkedOutput,
    ).toBeUndefined();
    expect(
      targetBase.buildings.find(({ id }) => id === secondInputId),
    ).toMatchObject({
    ...
    const extractorId = root
      .value("selected")!
      .buildings.find(
        ({ buildingTypeId }) => buildingTypeId === "ore_excavator",
      )!.id;
    await app.dispatch([
      appIds.events.BASES_UPDATE_BUILDING_LINKED_OUTPUT,
      targetId,
    ...
    expect(
      root.value("selected")!.buildings.find(({ id }) => id === extractorId)
        ?.linkedOutput,
    ).toBeUndefined();
    ...
    expect(
      root.value("selected")!.buildings.find(({ id }) => id === secondInputId),
    ).toMatchObject({
    ...
    expect(
      detail
        .value("inputs")
        .find(({ baseBuildingId }) => baseBuildingId === secondInputId),
    ).toMatchObject({
      item: { id: "iron-plate" },
    ...
    expect(detail.value("targetLogistics")).toMatchObject({
      incomingInputs: expect.arrayContaining([
        expect.objectContaining({
          baseBuildingId: secondInputId,
          linkedOutputStatus: "missing-base",
        }),
      ]),
    });
  ...
  it("emulates bulk output normalization, add-time linking, exclusive replacement, and clearing", async () => {
    ...
    const planOutput = view
      .value("selected")!
      .buildings.find(({ name }) => name === "Plan dispatcher")!;
    ...
    expect(
      view
        .value("selected")!
        .buildings.find(({ id }) => id === originalInputId),
    ).toMatchObject({
      selectedItemId: "iron-plate",
    ...
    expect(
      view
        .value("selected")!
        .buildings.find(({ name }) => name === "Second plan dispatcher"),
    ).toMatchObject({
      sourceProductionId: planId,
      allocationMode: "auto",
      priority: 1,
    ...
    const replacementInput = view
      .value("selected")!
      .buildings.find(({ name }) => name === "Replacement receiver")!;
    expect(
      view.value("selected")!.buildings.find(({ id }) => id === originalInputId)
        ?.linkedOutput,
    ).toBeUndefined();
    expect(replacementInput.linkedOutput).toMatchObject({
      baseId,
      buildingId: planOutput.id,
    });
    ...
    expect(
      view
        .value("selected")!
        .buildings.find(({ id }) => id === replacementInput.id),
    ).not.toMatchObject({
    ...
    const defaultedOutput = view
      .value("selected")!
      .buildings.find(({ name }) => name === "Defaulted dispatcher")!;
    await app.dispatch([
    ...
    expect(
      view
        .value("selected")!
        .buildings.find(({ id }) => id === defaultedOutput.id),
    ).toMatchObject({
      selectedItemId: "iron-plate",
      sourceProductionId: planId,
      allocationMode: "auto",
