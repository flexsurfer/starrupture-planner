...
} from "@/features/app-shell/data-version";
...
describe("headless bundled game-data compatibility E2E", () => {
  ...
  it("keeps legacy recipes at their indexes and gives new recipes stable IDs", async () => {
    ...
    if (!previousVersion || !latestVersion) {
      throw new Error(
        "Recipe identity compatibility requires at least two data versions",
      );
    }
    ...
    const latestBuildingsById = new Map(
      latest.buildings.map((building) => [building.id, building]),
    );
    ...
    for (const previousBuilding of previous.buildings) {
      ...
      for (const [recipeIndex, previousRecipe] of previousRecipes.entries()) {
        if (previousRecipe.id) {
          const latestRecipeIndex = latestRecipes.findIndex(
            ({ id }) => id === previousRecipe.id,
          );
          expect(
            latestRecipeIndex,
          ...
          if (latestRecipeIndex >= 0) {
            expect(latestRecipes[latestRecipeIndex]?.output.id).toBe(
              previousRecipe.output.id,
            );
            const matchedIndexes =
              matchedRecipeIndexesByBuilding.get(previousBuilding.id) ??
              new Set();
            matchedIndexes.add(latestRecipeIndex);
            matchedRecipeIndexesByBuilding.set(
              previousBuilding.id,
              matchedIndexes,
            );
          }
        ...
        const matchedIndexes =
          matchedRecipeIndexesByBuilding.get(previousBuilding.id) ?? new Set();
    ...
    for (const latestBuilding of latest.buildings) {
      const matchedIndexes =
        matchedRecipeIndexesByBuilding.get(latestBuilding.id) ??
        new Set<number>();
      const seenRecipeIds = new Set<string>();

      for (const [recipeIndex, recipe] of (
        latestBuilding.recipes ?? []
      ).entries()) {
  ...
  it.each(DATA_VERSIONS)(
    "loads %s and completes a real-data planning journey",
    async ({ id: version }) => {
      const app = createApp();
      ...
      expect(Object.keys(catalog.value("itemsById"))).toHaveLength(
        catalog.value("items").length,
      );
      expect(Object.keys(catalog.value("buildingsById"))).toHaveLength(
        catalog.value("buildings").length,
      );
      expect(catalog.value("categories")).toContain("all");
      for (const itemType of new Set(
        catalog.value("items").map(({ type }) => type),
      )) {
        expect(catalog.value("categories")).toContain(itemType);
      }
      expect(catalog.value("corporationSummary").totalCorporations).toBe(
        catalog.value("corporations").length,
      );
      const expectedProductionBuildings = Array.from(
        new Set([
          "all",
          ...catalog
            .value("buildings")
      ...
      expect(catalog.value("productionBuildings")).toEqual(
        expectedProductionBuildings,
      );

      const recipeChoice = catalog
        .value("buildings")
        .flatMap((building) =>
          (building.recipes ?? []).map((recipe) => ({ building, recipe })),
        )
        .find(
          ({ recipe }) =>
            recipe.inputs.length > 0 &&
            !!catalog.value("itemsById")[recipe.output.id],
        );
      ...
      const expectedOptionItemIds = Array.from(
        new Set(
          planner
            .value("flow")
            .nodes.filter(({ nodeType }) => nodeType === "production")
      ...
      if (!createdPlan)
        throw new Error(`Expected a production plan for ${version}`);
      ...
      const buildingsById = catalog.value("buildingsById");
      expect(
        requiredBuildings.some(({ buildingId }) =>
          buildingsById[buildingId]?.recipes?.some(
