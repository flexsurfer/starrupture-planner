...
function isRecipePresetsStorageEnvelope(
  value: unknown,
): value is RecipePresetsStorageEnvelope {
...
export function normalizeRecipePresets(
  raw: unknown,
  ...
  for (const rawPreset of raw) {
    ...
    presets.push({
      id,
      name,
      selections: normalizeSelections(preset.selections),
    });
  }
...
export function readRecipePresetsFromStorage():
  RecipeAlternativePreset[] | null {
  ...
  if (isRecipePresetsStorageEnvelope(raw)) {
    ...
    const hasNormalizationChanges =
      JSON.stringify(raw.presets) !== JSON.stringify(presets);

    if (
      raw.schemaVersion !== RECIPE_PRESETS_SCHEMA_VERSION ||
