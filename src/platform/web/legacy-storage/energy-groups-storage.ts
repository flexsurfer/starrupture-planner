...
function isEnergyGroupsStorageEnvelope(
  value: unknown,
): value is EnergyGroupsStorageEnvelope {
  if (typeof value !== "object" || value === null) return false;

  const envelope = value as EnergyGroupsStorageEnvelope;
  return (
    typeof envelope.schemaVersion === "number" && "energyGroups" in envelope
  );
}
...
export function readEnergyGroupsFromStorage(): EnergyGroup[] | null {
  ...
  if (isEnergyGroupsStorageEnvelope(raw)) {
    ...
    const hasNormalizationChanges =
      JSON.stringify(raw.energyGroups) !== JSON.stringify(groups);
