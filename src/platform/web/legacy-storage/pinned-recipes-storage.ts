...
function isPinnedRecipesStorageEnvelope(
  value: unknown,
): value is PinnedRecipesStorageEnvelope {
  if (typeof value !== "object" || value === null) return false;

  const envelope = value as PinnedRecipesStorageEnvelope;
  return (
    typeof envelope.schemaVersion === "number" &&
    "pinnedRecipeSelections" in envelope
  );
}
 ...
 */
export function normalizePinnedRecipeSelections(
  raw: unknown,
...
function writePinnedRecipesToStorage(
...
export function readPinnedRecipesFromStorage(): Record<string, string> | null {
  ...
  if (isPinnedRecipesStorageEnvelope(raw)) {
    const selections = normalizePinnedRecipeSelections(
      raw.pinnedRecipeSelections,
    );
    const hasNormalizationChanges =
      JSON.stringify(raw.pinnedRecipeSelections) !== JSON.stringify(selections);

    if (
      raw.schemaVersion !== PINNED_RECIPES_SCHEMA_VERSION ||
