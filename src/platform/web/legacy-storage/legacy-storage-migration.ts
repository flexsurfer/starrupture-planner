...
function migrateLegacyValue(
  prefix: string,
  rootKey: string,
  readLegacyValue: () => unknown,
): void {
  try {
    seedLegacyValue(prefix, rootKey, readLegacyValue());
  } catch (error) {
    console.error(
      `Could not migrate legacy local storage for ${rootKey}:`,
      error,
    );
  }
}
...
export function migrateLegacyStorage(prefix: string): void {
  ...
  migrateLegacyValue(prefix, "appDataVersion", () =>
  ...
  migrateLegacyValue(
    ...
    readPinnedRecipesFromStorage,
  );
  migrateLegacyValue(
    prefix,
    "recipeAlternativePresets",
    readRecipePresetsFromStorage,
  );
}
