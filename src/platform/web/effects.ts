...
import {
  ...
  loadGameDataVersion,
} from "@/platform/web/game-data-loader";

export const registerWebEffects: UkladModule<UkladRegistrar<AppContracts>> = (
  registrar,
) => {
  registrar.regEffect(appIds.effects.downloadArchive, (archive, runtime) => {
    try {
      ...
      runtime.dispatch([
        ...
        { kind: "success", message: "Export downloaded." },
      ]);
    } catch {
      runtime.dispatch([
        appIds.events.DATA_TRANSFER_SET_STATUS,
        {
          kind: "error",
          message: "Could not download the export. Please try again.",
        },
      ]);
    }
  });
  registrar.regEffect(appIds.effects.readArchive, (text, runtime) => {
    try {
      const archive = prepareArchiveImport(
        parseArchive(text),
        crypto.randomUUID(),
      );
  ...
  registrar.regEffect(appIds.effects.loadGameData, (version, runtime) => {
    void loadGameDataVersion(version)
      ...
      .catch((error: unknown) => {
        ...
        window.alert(
          "Could not load game data. Check your connection and reload the page.",
        );
