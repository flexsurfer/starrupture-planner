...
describe("saved planner tabs", () => {
  it("restores every tab setting and the active tab across restarts, including edits made without switching tabs", async () => {
    ...
    const expectedTabs = harness.getSubscriptionValue([
    ...
    expect(
      second.harness.getSubscriptionValue([appIds.subscriptions.PLANNER_TABS]),
    ).toEqual(expectedTabs);
    expect(
      second.harness.getSubscriptionValue([
    ...
    expect(
      fourth.harness.getSubscriptionValue([appIds.subscriptions.PLANNER_TABS]),
    ).toEqual([expectedTabs[1]]);
    ...
    expect(
      empty.harness.getSubscriptionValue([appIds.subscriptions.PLANNER_TABS]),
    ).toEqual([]);
    expect(
      empty.harness.getSubscriptionValue([
        appIds.subscriptions.PLANNER_ACTIVE_TAB,
      ]),
    ).toBeNull();
  ...
  it("repairs malformed saved settings and recovers a stale active ID without losing valid tabs", () => {
    ...
    const storage = memoryStorageAdapter({
      [`tabs-test/${stateKeys.plannerTabs}`]: JSON.stringify({
        v: 1,
        data: [
          {
            ...tab,
            ...
            multiTargets: [
              ...
              { itemId: "iron-plate", amount: 30 },
              { itemId: "broken", amount: null },
            ],
            recipeSelections: { "iron-plate": "smelter:0", broken: 5 },
            selectedCorporationLevel: { corporationId: 5, level: "1" },
          },
          tab,
          { ...tab, id: "blank", name: "  " },
          { ...tab, id: "bad-mode", mode: "other" },
          null,
        ],
      }),
      [`tabs-test/${stateKeys.plannerActiveTabId}`]: JSON.stringify({
        v: 1,
        data: "missing",
      }),
    });
    ...
    expect(
      harness.getSubscriptionValue([appIds.subscriptions.PLANNER_TABS]),
    ).toEqual([
      {
        ...tab,
        name: "Saved plan",
        multiTargets: [{ itemId: "iron-plate", amount: 20 }],
        recipeSelections: { "iron-plate": "smelter:0" },
      },
    ]);
    expect(
      harness.getSubscriptionValue([appIds.subscriptions.PLANNER_ACTIVE_TAB])
        ?.id,
    ).toBe("valid");
    runtime.dispose();
  });
