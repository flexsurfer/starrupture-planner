...
describe("base persistence", () => {
  it.each(["planning", "advanced"] as const)(
    "remembers %s mode after reload without changing base data",
    async (mode) => {
      ...
      expect(
        createUkladTestHarness(restored).getSubscriptionValue([
  ...
  it.each([undefined, null, "invalid", true, {}])(
    "asks for a mode when the stored preference is missing or invalid: %s",
    (value) => {
      const storage = memoryStorageAdapter();
      if (value !== undefined)
        storage.setItem(
          `${PERSIST_PREFIX}/${stateKeys.basesMode}`,
          JSON.stringify({ v: 1, data: value }),
        );
      const runtime = createRuntime();
      persist(runtime, {
        storage,
      ...
      expect(
        createUkladTestHarness(runtime).getSubscriptionValue([
          appIds.subscriptions.BASES_MODE,
        ]),
      ).toBeNull();
      runtime.dispose();
  ...
  it.each([undefined, "invalid", true])(
    "defaults or restores expanded details for stored value %s",
    (value) => {
      const storage = memoryStorageAdapter();
      if (value !== undefined)
        storage.setItem(
          `${PERSIST_PREFIX}/${stateKeys.basesDetailsExpanded}`,
          JSON.stringify({ v: 1, data: value }),
        );
      const runtime = createRuntime();
      persist(runtime, {
  ...
  it("persists a base after its energy-group assignment is removed", async () => {
    ...
    expect(
      JSON.parse(storage.getItem(`${PERSIST_PREFIX}/basesList`) || ""),
    ).toEqual({
      v: 1,
      data: [
        {
    ...
    expect(
      createUkladTestHarness(rehydratedRuntime).getState().basesList[0],
    ).not.toHaveProperty("energyGroupId");
