...
export function downloadArchive(archive: PlannerArchive): void {
    const singleName = archive.bases.length + archive.plans.length === 1
        ? (archive.bases[0]?.name ?? archive.plans[0]?.name) : 'backup';
    const name = singleName.replace(/[^\p{L}\p{N}_-]+/gu, '-').slice(0, 80) || 'export';
