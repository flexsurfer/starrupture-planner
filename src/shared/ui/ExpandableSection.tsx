...
export const ExpandableSection = ({
  ...
  return (
    <section className="min-w-0 rounded-lg border border-base-300 bg-base-100">
        ...
        <h2 className="min-w-0 flex-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-semibold leading-tight break-words sm:text-base">
                {title}
              </span>
              <span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-normal leading-tight text-base-content/65 tabular-nums sm:text-xs">
                {summary}
              </span>
            </span>
            <svg
              aria-hidden="true"
              className={`size-4 shrink-0 text-base-content/50 transition-transform ${expanded ? "rotate-90" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      {expanded && (
        <div id={contentId} className="border-t border-base-300 p-2 sm:p-3">
          {children}
        </div>
      )}
    </section>
