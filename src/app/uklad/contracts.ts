...
export interface AppContracts extends UkladContracts {
    ...
    events: {
        ...
        [appIds.events.PLANNER_SET_FLOW_DIRECTION]: [direction: PlannerTab['flowDirection']];
        [appIds.events.PLANNER_REQUEST_TAB_CREATION]: [];
        [appIds.events.PLANNER_CANCEL_TAB_CREATION]: [];
        [appIds.events.PLANNER_CREATE_TAB]: [id: string, name: string, mode: PlannerMode, view?: PlannerView];
        ...
        [appIds.events.PLANNER_SET_MULTI_TARGET_AMOUNT]: [itemId: string, amount: number];
    ...
    subscriptions: {
        ...
        [appIds.subscriptions.PLANNER_MULTI_RECIPE_SELECTIONS]: { params: []; result: Record<string, string> };
        [appIds.subscriptions.PLANNER_SINGLE_RECIPE_SELECTIONS]: { params: []; result: Record<string, string> };
        [appIds.subscriptions.PLANNER_GROUP_BY_STAGE]: { params: []; result: boolean };
        [appIds.subscriptions.PLANNER_FLOW_DIRECTION]: { params: []; result: PlannerTab['flowDirection'] };
        [appIds.subscriptions.PLANNER_TABS]: { params: []; result: PlannerTab[] };
        [appIds.subscriptions.PLANNER_ACTIVE_TAB]: { params: []; result: PlannerTab | null };
        [appIds.subscriptions.PLANNER_ACTIVE_TAB_ID]: { params: []; result: string | null };
        [appIds.subscriptions.PLANNER_TAB_CREATION]: { params: []; result: AppState['plannerTabCreation'] };
